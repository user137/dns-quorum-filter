...
**Closing-advisor (T-174).** Advisor вказав: для NXDOMAIN-сигнатурного voter'а «blocked» ==
«повернув NXDOMAIN» за побудовою, тож приріст живе в зоні, де NXDOMAIN CleanBrowsing може бути
блоком **або** різницею у видимості між рекурсивними резолверами. **Перевірка:** у harness додано
**другий незалежний нефільтрований резолвер** (Quad9 unsec `dns10`, `SECOND_BASELINE_URL` =
`BASELINE_CHAIN[1]`); домен лишається лише якщо **обидва** дають NoError. Перемір із
двобейзлайновим гейтом (n = 106): **0** доменів відкинуто за розбіжність нефільтрованих резолверів
— confound не матеріалізувався. Дельта не впала: **кворум OR Security-tier 76/106 (71.7 %) —
+18 / +17.0 pp над Quad9** (проти +15.9 pp з одним baseline), 17 malware-доменів зловив лише
CleanBrowsing. **Вердикт «підтверджено» стоїть, confound явно протестовано й відкинуто.** Побічно
виправлено caveat-непослідовність, яку advisor теж зауважив (Quad9-й NXDOMAIN-rate мав «upper
bound», CleanBrowsing-й — ні: тепер обидва під тим самим гейтом). Raw-вихід: scratchpad
`t174_advisor_twobaseline_2026-09-05.txt`.
...
впливає). Без CI, без churn тега, без видалення чернетки.

**`v0.3.0` опубліковано 2026-09-06** (рішення користувача «повноцінний реліз, публікуємо як є»):
`gh release edit v0.3.0 --draft=false --latest` → публічна сторінка Releases, постійний
`/releases/tag/v0.3.0`, бейдж `latest`, 7 asset'ів. Test-signed лишається (чесно позначено;
безкоштовний OSS code-signing через SignPath — можлива окрема задача, не блокер). `v0.2.0`
лишається чернеткою.
...
- [x] **T-192** — патч-реліз `v0.3.1` (покриває Батч 3.12 + 3.14; закриває T-186). Коміти `cd0443c`
  ...
  - **Тег `v0.3.1` ЩЕ НЕ ПУШНУТО** (2026-09-08 — користувач не за ПК): ручний чистий прогін MSIX
    ...
    `msix` + `release` cross-path repro) → **чернетка** GitHub-релізу (3 `.exe` + `SHA256SUMS` +
    `.msix` + `.cer` + `Trust-TestCert.ps1`), неопублікована (публікує людина, як `v0.3.0`).
...
### Батч 4.6 — закриття Фази 4, реліз `v0.4.0` (зроблено 2026-09-11, короткий kickoff + closing-advisor перед пушем тега)

**Скоуп:** бамп версії `0.3.1` → `0.4.0` (3 `Cargo.toml.version` + 2 path-dep specs + `Cargo.lock`,
окремий комітом `802c954`) + docs-sync (`91057c8`) + closing-advisor + тег `v0.4.0` →
`release.yml` draft. Kickoff-уточнення (AskUserQuestion): (1) T-190/`v0.3.2` (Батч 3.13's
онбординг-майстер) свідомо **не** тегується окремо — злитий у цей реліз (`0.3.1` → `0.4.0`
одним бампом, не через промижний `0.3.2`); (2) `curate_topn` реально регенерує всі 6 списків
(не тільки smoke-тест).

**Гейти перед комітом:** `fmt --check` / `clippy --workspace --all-targets --locked -D warnings`
/ `test --workspace --lib --bins --locked` (799+37+3 ok) / `--doc --locked` (9 doctest) /
`--examples --locked` / `--test conformance -p dnsqb-service --locked` (18 ok, 2 навмисно
ignored — T-164) — усе зелено локально (windows-gnu), потім підтверджено на реальному
`windows-latest` CI (`34618416547`, увесь `repro`-джоб теж пройшов — cross-path bit-identical
release build). CodeQL (`34618416556`) зелений, **0 відкритих алертів** (перевірено `gh api`
напряму, не лише зелений статус джоба).

**Pre-release перевірки (T-175's мандаторна дисципліна):** `cargo run --example sinkhole_probe`
— усі 5 sinkhole-префіксів (`opendns-familyshield`/`adguard-family`/`adguard`/`dns4eu-{child,
protective}`) досі влучають по канарці й не влучають по контрольному домену; `OK`.
`cargo run --release --example curate_topn -- lists=ua,us,de,pl,gb,global n=1000` — реальна
регенерація всіх 6 CrUX-похідних списків: той самий місяць (`202608`), той самий підрахунок
origin→registrable для кожної країни, **байт-в-байт незмінний вихід** — `git status` показав 0
диффу в `data/topn/`, нічого коммітити для цього кроку. `gov-*.txt`/`edu.txt` (Батч 4.2,
hand-curated) цим інструментом не торкаються за конструкцією.

**Closing-advisor (перед тегом, на вже-durable результат) знайшов 3 блокуючі прогалини, усі
виправлено до тега:**
1. **DECISIONS.md не було оновлено** — TASKS.md's власний запис сам посилався на «DECISIONS.md
   2026-09-08» як джерело плану T-190/`v0.3.2`, а проєктне правило («decisions log overrides
   older specs») означає, що без нового датованого запису найновіша задокументована влада досі
   каже «`v0.3.2` вийде окремо» — цей факт переважав би щойно написаний текст у SPEC.md/CLAUDE.md.
   Виправлено новим записом `## 2026-09-11 — T-190 (v0.3.2) скасовано...` (DECISIONS.md).
2. **TASKS.md сам собі суперечив** — відредаговано Батч-4.6-рядок і «Порядок», але лишилися
   недоторкані посилання на `v0.3.2` за рядками ~490/494/845/896-902, включно з **відкритим**
   чекбоксом `[ ] T-190 — патч-реліз v0.3.2`. Виправлено всі 5 місць — T-190 тепер `[x]`, закритий
   Батчем 4.6, з явним застереженням, що ручний MSIX-прогін (T-186's чек-лист) досі не пройдено.
3. **Не було запису в TASKS-DONE.md** — цей самий запис.

**Verify-крок, що advisor попросив, а не окрема знахідка:** `grep -i "фаз\|v0\.3\|badge" README.md`
дійсно показав застарілий бейдж `Phase 3 complete` + текст «Фаза 3 закрита повністю» без згадки
Фази 4 — README's власна документаційна карта називає фазовий бейдж своїм тригером оновлення.
Виправлено (бейдж → `Phase 4 complete`, статус-абзац переписано на Фазу 4).

**Не закрито, явно передано користувачу (advisor's handoff-пункт, не блокер):** ручний чистий
MSIX-прогін (T-186's чек-лист, розширений T-188/T-189/T-190) на живій машині без сертифіката —
той самий гейт, що чекав на `v0.3.1` (Батч 3.14) і мав би чекати на `v0.3.2`, тепер чекає на
`v0.4.0`. Не закривається кодом чи документацією — потребує людини з реальною Windows-машиною.

**Коміти:** `802c954` (бамп версії) · `91057c8` (docs-sync, kickoff-версія) · докс-фікс-коміт
(closing-advisor: DECISIONS.md + TASKS.md + README.md) · тег `v0.4.0`.

**Фаза 4 закрита повністю.**

### T-219 — MSIX-паковані `dnsqb-service`/`dnsqb-tray` не могли підтвердити довіру сертифіката (виявлено й закрито 2026-09-12, глибокий смоук-тест v0.4.0)

**Знайдено під час продовження смоук-тесту v0.4.0** (`plans/msix-smoke-test-v0.4.0.md`, сценарії
3–9): реально встановлений і запущений пакований (`Add-AppxPackage`) `dnsqb-service` тримав
`GET /admin/cert-status` на `UNKNOWN` 20+ хвилин (20+ тіків 60с-поллінгу
`cert_watch::run_cert_trust_watch`) — не стартовий race, кожен тік публікує результат
беззастережно. Майстер онбордингу (`maybe_offer_onboarding`) ніколи не спрацьовував автоматично;
зелений колір трей-іконки був лише незмінним seed-значенням `TrustState{trusted:true,
confirmed:false}` (T-191), не доказом реальної довіри.

**Root cause, звужено трьома advisor-раундами перед фіксом (жодного кроку без емпіричної
перевірки):** `trust_store::local_cert_thumbprint` передавала спавненому `certutil.exe`
**логічний** шлях (`%LOCALAPPDATA%\dns-quorum-filter\cert.pem`). Сам пакований процес (через
`std::fs`) бачить цей шлях прозоро перенаправленим Windows-файловою віртуалізацією на фізичне
MSIX-розташування — але спавнений дочірній `certutil.exe` цього перенаправлення не отримує,
бачить файл як відсутній (`ERROR_FILE_NOT_FOUND`). Підтверджено: (1) зовнішній `certutil -dump`
на той самий логічний шлях відтворив ту саму помилку; (2) `dnsqb-tray.log` містив точний текст
`TrustStoreError::LocalThumbprint` (не `Spawn` — отже сам `certutil.exe` успішно спавниться,
провалюється лише `-dump`); (3) `certutil -dump` на РЕАЛЬНИЙ фізичний шлях того самого файла
успішно видав повний X.509-дамп.

**Advisor-catch, який врятував від хибного фіксу:** перший гіпотетичний план мав одну фазу
(фіксити читання й запис одночасно, скопіювавши `cert.pem` у `C:\Windows\Temp` для ОБОХ
`-dump`/`-addstore`). Advisor зупинив це — усі `C:\Windows\Temp`-проби йшли ЗОВНІ (unpackaged
PowerShell), ніколи як дочірній процес пакованого сервісу; довести, що `-store`/`-addstore`
дійсно працюють із пакованого child, а не лише що зовнішній certutil бачить цей шлях, потрібен
окремий живий тест. Розв'язальний тест: після Фіксу 1 — зовнішнє `certutil -addstore` в
`CurrentUser\Root` → дочекатись 60с-тіку → `/admin/cert-status` перейшов `NOT_TRUSTED`→`TRUSTED`
— підтвердило, що `-store` (без шляху до файла) справді працює з пакованого дочірнього процесу,
і лише ПІСЛЯ цього Фікс 2 (для `-addstore`) проектувався на підставі знання, не припущення.

**Фікс, дві частини, кожна live-верифікована на перезбудованому/перепакованому/переінстальованому
MSIX (нові ефемерні сертифікати підпису щоразу, окремі UAC-проходи через `Trust-TestCert.ps1`):**
1. `local_cert_thumbprint` тепер обчислює SHA-1-відбиток у чистому Rust
   (`CertificateDer::from_pem_slice` — той самий примітив, що `tls.rs`/`cert.rs` уже
   використовують для цього файла — + вже вжитий `sha1`-крейт), замість `certutil -dump`.
   Звірено емпірично з `.NET X509Certificate2.Thumbprint` на реальному `cert.pem` перед тим, як
   покластися на це. Живо підтверджено: `/admin/cert-status` → `NOT_TRUSTED` (не `UNKNOWN`)
   одразу після переінсталу; майстер онбордингу зʼявився автоматично.
2. `ensure_installed`'s `-addstore` тепер працює проти тимчасової контентно-адресованої копії під
   `%SystemRoot%\Temp` (не `std::env::temp_dir()` — той резолвиться в той самий віртуалізований
   `%LOCALAPPDATA%\Temp` префікс), бо `-addstore` сам мусить відкрити файл. Живо підтверджено:
   «Майстер налаштування» → «Так» → **ground truth** — SHA-1 реального `cert.pem` зʼявився в
   `Cert:\CurrentUser\Root` через сам застосунок (не зовнішньо); тимчасовий файл прибраний
   (best-effort cleanup спрацював). `certutil -store`/`-delstore` (`uninstall`'s код-шлях, без
   аргументу-шляху до файла) підтверджено не зачепленими розв'язальним тестом вище.

**Побічні знахідки, зафіксовані для майбутнього:**
- `certutil -addstore -user Root`/`-delstore` показує власний Windows CryptUI-діалог
  «Попередження системи безпеки» (не UAC) — раніше відкрите питання в doc-коментарі
  `trust_store.rs` («ніколи не пінено для certutil.exe конкретно»), тепер підтверджено живо
  двічі.
- `windows` MCP-сервер (`ui_click` за UIA-іменем) звітував успіх кліку, але не клікав реальну
  кнопку native Win32-діалогу (ймовірно DPI-scaling mismatch) — діалог лишався відкритим.
  `autoit`'s `controlClick` (Windows-повідомлення напряму в handle кнопки) спрацював надійно
  щоразу — саме тому цю сесію зареєстрували `autoit`. Запис у CLAUDE.md's gotchas.

**Гейти перед комітом:** `cargo build`/`test --workspace --lib --bins` (809 тестів, включно з
новим крос-валідаційним тестом `local_cert_thumbprint` проти реального `certutil -dump` того
самого файла) / `clippy --workspace --all-targets -- -D warnings` / `fmt --all -- --check` — усе
зелено. `advisor` — двічі перед реалізацією (звуження діагностики), раз по завершенні кожної
фази верифікації.

**Не виконано в цьому батчі, залишено для наступної сесії:** сценарій 9 (трей-меню «Перевипустити
сертифікат») окремим живим прогоном — логічно покритий тим самим `ensure_installed`-код-шляхом
(сценарій 7) + `uninstall` (розв'язальний тест), але не натиснутий саме через це меню.

**Коміт:** `d90f578` (фікс `trust_store.rs`). Повний план фіксу з усіма advisor-раундами:
`C:\Users\Pa\.claude-work\plans\adaptive-dreaming-torvalds.md`. Живий прогін і оновлені
результати сценаріїв 3–9: `plans/msix-smoke-test-v0.4.0.md`'s «✅ ВИПРАВЛЕНО 2026-09-12» розділ
(локальний файл, не в репо).

### T-136 — merge публічних блок-листів: закрито без коду (2026-09-12, аудит бекложу)

**Закрито без реалізації**, рішення делеговане користувачем агенту. Задача (merge публічних
блок-листів типу EasyList, окрема фіча поза quorum-логікою) обсягово повністю поглинута **T-218**
(запропонована користувачем 2026-09-11: опційний, самооновлюваний режим блокування готовими
наборами списків «під ціль», з набагато детальнішим аналізом джерел/ліцензій/дизайн-питань —
`data/blocklists/CANDIDATES.md`, три відкриті питання advisor-рев'ю щодо ліцензій, мережевої
ввічливості й позиції в SPEC.md §5.3-пайплайні). Тримати T-136 паралельно як окрему задачу додавало
б лише дублювання відстеження одного й того самого скоупу без окремої цінності — якщо T-218 колись
підіймається, її kickoff уже покриває весь скоуп, який T-136 називала. Немає коду, немає коміту —
чисто документаційне закриття в `TASKS.md`.

### T-224 — лог-рядкова кнопка «В allowlist»/«В blocklist» лишалась мертвою після видалення того самого домену зі списку overrides (Батч 4.7.A, 2026-09-12)

**Root cause:** `overrideListItem`'s `removeBtn` click handler (`main.js:429-436`) на успіх
викликав лише `refreshOverrides()`, ніколи `refreshLog()`. Кнопка в рядку лога (T-46,
`main.js:1252-1274`) на успішний `addOverride()` ставить `disabled=true` +
`textContent="✓ Додано"` і ніколи сама цей стан не скидає — розрахована на те, що щось зовні
перебудує лог. Коли той самий домен потім видалявся через «Видалити» у списку overrides, лог не
перебудовувався, і кнопка лишалась мертвою — користувач не міг повторно додати той самий домен з
лога без ручного пересбирання рядків через пошук/фільтр.

**Фікс:** один рядок — `await refreshLog();` одразу після `await refreshOverrides();` у тому
самому `try`-блоці. `refreshLog()` сам ловить власні помилки (`try { renderLog(...) } catch {
renderLogError }`, `main.js:1466-1472`), тож не створює шлях, де провал `getLog()` затер би вже
коректно відрендерений список overrides через `renderOverridesError`. Приймає той самий
trade-off, що вже задокументований коментарем біля add-шляху (T-46): застосовує поточний вміст
`#log-search`/`#log-decision`/`#log-voter`, навіть якщо користувач ще не натиснув «Пошук».

**Перевірено два незалежні `removeBtn.textContent = "Видалити"` в тому самому файлі
(`main.js:758` — GeoIP-країна, `main.js:1646` — власний provider) — жоден не рендерить
allowlist/blocklist, фікс не half-applied.**

**Живо підтверджено в Chrome (chrome-devtools MCP; claude-in-chrome розширення було відключене
цієї сесії), не лише build/clippy/fmt/test:** реальний DoH-запит через сирий wire-format POST
(`t224verify2.example`) → лог-рядок з'явився → клік «В blocklist» → кнопка стала «✓ Додано»
(disabled) → клік «Видалити» у списку BLOCKLIST → лог-рядкова кнопка знову «В blocklist», активна.
Прогнано на скретч-інстансі власного dev-білду на нестандартному порту (9443, окремий
`resolver_config.toml` у скретч-`%LOCALAPPDATA%`) — **не** на живому встановленому MSIX
`v0.4.1`-тест-білді (він лишався запущеним на стандартному порту 8443 увесь час, не зачепленим
цим коміту вартим фіксом). Самовиправлена помилка процесу: перша спроба живої перевірки випадково
вдарила по цьому живому MSIX-інстансу (порт 8443 збігся з дефолтним портом сирого `Start-Process`
без попередньо заданого `resolver_config.toml`) — додала й одразу видалила той самий тестовий
домен у його реальний blocklist; без залишкового стану (запис видалено, лишився лише один
нешкідливий рядок у in-memory лозі, що не персистується за замовчуванням).

**Коміт:** `5fcdda8` (docs+code, один коміт, разом з TASKS.md).

### T-228 — офлайн-гварди на два мережево-залежні контролі `/admin/ui` (Батч 4.7.A, 2026-09-12)

**Root cause:** `#rating-filter-body` (T-127) і `#geoip-maxmind-body` (T-162) обидва мають власний
fetch/render-цикл, деліберативно поза 2с-поллінгом (той самий "не перебудовувати чужий
in-progress ввід" принцип, що й `#overrides-body`/`#geoip-body`) — тож жоден з них ніколи не
бачив `status.network` (те поле живе лише в `AdminStatusResponse`, яку отримує тільки
`render()` на 2с-тіку). Дві дії в цих картках напряму запускають мережевий фетч і мовчки
провалювались офлайн: рейтинг-фільтра «Підтвердити ввімкнення»/«Зберегти зони» (будять
`run_topn_updater` — SPEC.md §5.3) і MaxMind-картки «Зберегти» (POST-time перевірка ключа,
`dispatch.rs`'s власний коментар про save-time probe). Офлайн MaxMind-провал ще й показував
хибний діагноз — `MAXMIND_CHECK_MESSAGES.FAILED` ("MaxMind відхилив креденшели (401/403)"),
хоча причина — відсутність мережі, не погані креденшели.

**Фікс:** новий модульний `let lastNetworkStatus = "ONLINE"`, записуваний рядком
`lastNetworkStatus = status.network;` на початку `render(status)` (2с-поллінг). Обидві картки
читають це значення при рендері (`disabled=true` + пояснювальний `title` на відповідній кнопці)
і повторно перевіряють його в самому click-обробнику (той самий захист, що вже стояв на
success-шляхах цього файлу — стан може застаріти між рендером і кліком). Комбобокс вибору зон і
кнопка «Очистити» MaxMind лишились активними — обидва міняють лише локальний/уже застосований
стан, мережі не потребують.

**Живо підтверджено в Chrome** (chrome-devtools MCP, скретч-інстанс власного dev-білду, порт
9444 — окремий від і 8443 (живий MSIX), і 9443 (використаний для T-224/T-225), щоб уникнути
повторення тієї ж помилки з портом): оскільки `main.js` — звичайний, не `type="module"` скрипт,
його top-level `let`/`function` лишаються bare-глобалами в контексті сторінки — `lastNetworkStatus`
підмінено напряму через `evaluate_script`, обидві картки перемальовано (`refreshRatingFilter()`/
`refreshMaxmind()`), перевірено `disabled`+`title` на всіх трьох кнопках (`.rf-confirm`,
`.rf-save`, MaxMind's «Зберегти») в обох напрямках: OFFLINE → усі три `disabled:true` з
правильним текстом причини; ONLINE → знову `disabled:false`, `title:""`. Не відтворювався
реальний офлайн через hosts-хайджек (як у сценаріях 21/22 попередньої сесії) — непропорційно
важко для точкового UI-фіксу цього масштабу; підміна значення читає той самий live-код-шлях,
...
### T-159 — контекстна довідка полів на `/admin/ui` (Батч 4.7.A, 2026-09-12)

**Задача виникла 2026-08-29** (користувач: "налаштувань купа і можна розгубитися що за що
відповідає"), рекомендація без готового рішення механізму — дві опції: (1) нативний
`<details><summary>` чи (2) власний CSS/JS toggle-патерн. **Користувач обрав (1)** явно, коли
задача піднялась у Батчі 4.7.A.

**Реалізація:** новий `FIELD_HELP` — один плаский об'єкт `{ключ: текст}` у `main.js`, і
`helpDetails(key)` — DOM-хелпер, що повертає `<details class="field-help"><summary>?</summary>
<p>…</p></details>`. Підключено до п'яти карток, названих у первісному описі задачі:
`renderOverrides`/`buildLogFilterRow`/`renderCacheConfig`/`renderProviders` (чотири
DOM-побудовані заголовки, через `cardHeading(text, helpKey)` — сам заголовок і `helpDetails`
братами всередині `<div class="card-heading-row">`) + `renderTimeoutConfig` (єдина картка на
innerHTML-шаблоні — той самий `<div class="card-heading-row">`-патерн вписано рядком HTML
усередині шаблона, той самий підхід, що вже застосований для `configWarning` у тій функції).
**Не** підключено до error-рендерів тих самих карток (`renderCacheConfigError`/
`renderProvidersError`) — підказка пояснює поле, не має сенсу на панелі помилки.

**Advisor-catch на closing-review, виправлено до коміту:** перша реалізація вкладала
`<details>` усередину `<h3>` (`heading.appendChild(helpDetails(...))`) — `<h3>` дозволяє лише
phrasing content, `<details>` це flow content, тож вкладення невалідне HTML. Браузери мовчки
відновлюються (Chrome-перевірка і a11y-дерево показали коректний результат), але це "правда за
спостереженою поведінкою", не "доказово з самого розмітки" — точно той клас, що глобальний
CLAUDE.md's правило про bounds/index safety узагальнює на markup. Виправлено на `cardHeading()`:
`<details>` тепер брат `<h3>`, обидва всередині `<div class="card-heading-row">`
(`display:flex`) — той самий візуальний результат, валідний HTML. `.field-help`'s
`text-transform`/`letter-spacing`-скид у CSS прибрано як зайвий — він був потрібен лише через
невалідне вкладення (спадкування регістру від `.card h3`), тепер `<details>` — сестра, не
дитина, `.card h3`'s стилі на нього не поширюються.

**Текст — власник UI-SPEC.md §3.8** (нова секція, докс-мап-конвенція "текст полів туди й іде, не
вигадується на льоту в коді"), `main.js`'s `FIELD_HELP` дзеркалить її дослівно. П'ять текстів
перевірено на фактичну точність проти реального коду перед записом (не вигадано з голови):
`timeoutMode`'s опис `degraded`-режиму звірено з `timeout.rs`'s власним doc-коментарем
(NOT "останній кеш", як у першому чернетковому варіанті, а "той самий вердикт, що fail_open, але
позначений неповним у лозі" — точне read перед написанням тексту); `logFilters`'s "~1000 запитів
або 24 години" звірено з `query_log.rs`'s `DEFAULT_MAX_ENTRIES`/`DEFAULT_MAX_AGE`; `cache`'s опис
полів звірено з `CacheConfig`'s власних doc-коментарів (clamp_min/max, block_verdict_ttl,
max_capacity).

**i18n-готовність (T-151), явно піднята користувачем під час задачі:** `FIELD_HELP` — деліберативно
один плаский об'єкт, не текст, розкиданий по п'яти функціях рендеру — коли T-151 дійде до цього
екрана, це єдине місце, яке треба замінити на locale-lookup, а не пошук по всьому файлу. Ніякої
реальної i18n-інфраструктури не додано (весь інший текст `main.js`/`index.html` лишається так само
хардкоджено українською) — це була б передчасна абстракція без поточної потреби (CLAUDE.md's
"no speculative features"); зроблено лише мінімальне структурне рішення, що не коштує нічого
зараз, але не ускладнює майбутню роботу.

**CSS:** новий блок `.field-help`/`summary`/`p` у `style.css`. `.card h3` задає
`text-transform: uppercase` — оскільки `<details>` вкладений усередину `<h3>`, без явного скидання
"?" і текст підказки теж стали б капсом; `.field-help` явно скидає `text-transform`/
`letter-spacing`/`font-weight`.

**Живо підтверджено в Chrome** (chrome-devtools MCP, скретч-інстанс власного dev-білду, порт 9445
— окремий від 8443/9443/9444, щоб не повторити T-224's помилку з портом): усі п'ять `.field-help`
присутні з правильним, картці-відповідним текстом (перевірено `evaluate_script`); реальний клік
на "Списки виключень"'s `<details>` показав розгорнутий текст підказки, a11y-дерево коректно читає
елемент як `DisclosureTriangle "Довідка"` (не просто DOM-присутність — справжня взаємодія).
Розширена секція (Провайдери/Timeout/Кеш) перевірена тим самим способом — усі три підказки на
місці з правильним текстом.

**Поза межами цього батчу:** GeoIP-картка, MaxMind-картка, рейтинг-фільтр «бульбашка» — жоден не
названий явно в T-159's власному прикладному переліку карток; додати за тим самим патерном
(`helpDetails`/`FIELD_HELP`/UI-SPEC.md §3.8), коли підніметься окремо.

**Коміт:** `8d44e53`.

### T-220 — `trust_store::NOT_FOUND_EXIT_CODE` було POSIX-truncated значенням, не реальним exit-кодом (Батч 4.7.B, 2026-09-13, plan+advisor)

**Root cause:** `trust_store.rs`'s `NOT_FOUND_EXIT_CODE: i32 = 17` (T-49) — низький байт
`0x80090011`/`NTE_NOT_FOUND`, тобто те, що POSIX-шелл (`$?`) показує після 8-бітного відтинання
реального коду, а не те, що `std::process::ExitStatus::code()` реально повертає на скомпільованому
Windows-білді (`-2146893807`, без відтинання — `0x80090011` як `i32`). `confirmed_thumbprints_
for_common_name` — єдиний спосіб `uninstall_loop` підтвердити "стор порожній" — тому НІКОЛИ не
бачив підтвердженого нуль-збігу і завжди повертав `Err(ListFailed(...))` на завершальній ітерації,
навіть коли `-delstore` щойно реально видалив усе відповідне. Живо відтворено під час смоук-тесту
v0.4.0 (сценарій 9: «Перевипустити сертифікат» → "could not confirm whether any matching
certificates remain"; сценарій 23: сертифікат реально видалено через ручно підтверджений CryptUI-
діалог, звіт усе одно каже "НЕ ВДАЛОСЯ видалити").

**Advisor gate (обов'язковий per проектний Батч 4.7.B стандарт — security-sensitive код без
автоматизованого покриття реального стора):**

- **Advisor заблокував перший накид** (синтетичний `ExitStatus`-тест, що просто підставляє
  `0x80090011` і перевіряє класифікацію) як циркулярний — такий тест довів би лише, що код
  узгоджений сам із собою, те саме було істинним і коли константа була `17`. Замість цього:
  **регресійний тест мусить бити в реальний `certutil`**, той самий клас, що вже дозволений
  `thumbprints_for_common_name`'s власним прецедентом (параметризований `common_name` саме "щоб
  тести могли перевірити реальну логіку без залежності від реального стану проєктного `CommonName`
  у сторі") — `-store` є лише читанням, тож безпечно для CI, на відміну від `-addstore`/
  `-delstore`, які модуль-док прямо забороняє викликати автоматизовано.
- **Test-first, підтверджено падінням проти бага:**
  `confirmed_thumbprints_for_common_name_reports_a_sentinel_cn_as_confirmed_empty` (сентинел-
  CommonName, гарантовано відсутній у реальному сторі) написаний і запущений ПРОТИ старої константи
  (`17`) — впав з `ListFailed(Some(-2146893807))`, підтвердивши баг живо, не за здогадом. Потім
  константу виправлено, той самий тест перезапущено — пройшов.
- Advisor також попередив про `clippy::cast_possible_wrap` для очевидного `0x8009_0011_u32 as
  i32` під `-D warnings` — перевірено: `u32::cast_signed()` (стабільний, `const`-сумісний) не
  тригерить лінт, замінник `as`-каста без `#[allow]`.

**Фікс:** `const NOT_FOUND_EXIT_CODE: i32 = 0x8009_0011_u32.cast_signed();` — той самий bit
pattern, що вже називався в doc-коментарі (`0x80090011 NTE_NOT_FOUND`), тепер видимий і в самому
коді, не захований за непояснюваною десятковою `17`. Doc-коментар константи переписано з повним
root-cause (стара цитата "confirmed empirically 2026-08-29" видалена — вона описувала помилкове
вимірювання).

**Рефакторинг для тестованості:** `confirmed_thumbprints_for_common_name` розбито на тонку обгортку
(спавнить `certutil`) + чисту `classify_confirmed_lookup(&std::process::Output)` (три гілки:
success→парсинг, `NOT_FOUND_EXIT_CODE`→підтверджено-порожньо, інше→`ListFailed`). Три додаткові
синтетичні тести (`ExitStatusExt::from_raw`, без спавну процесу) покривають саме ці три гілки —
**не доводять правильність самої константи** (advisor: це було б тим самим циркулярним аргументом),
лише що класифікаційна логіка навколо неї коректна.

**Блок-радіус, розблокований цим фіксом — прогнозовано, НЕ живо підтверджено цієї сесії** (машина
зараз без встановленого MSIX, T-222/сценарій 23 повністю видалив застосунок): `cert_rotation::
rotate_certificate` (трей "Перевипустити сертифікат" — `uninstall()`'s половина мала завжди
провалюватись саме на підтвердженні порожнього стора) і `local_state::remove_all`'s сертифікатний
артефакт (мав завжди повертати `Failed` в звіті "Повністю видалити", навіть коли видалення реально
відбулось). Обидва не unit-тестовані (реальна мутація стора), тож потребують живого підтвердження
при наступній реальній MSIX-установці — не позначати як "виправлено й перевірено" без цього.
**Уточнення (advisor-catch):** `Removed` у `local_state::remove_all`'s звіті все одно НЕ
гарантовано навіть після цього фіксу — `-delstore` показує власний модальний CryptUI-діалог
(сценарій 23 смоук-тесту), і відхилений діалог дає `TrustStoreError::UninstallFailed`, окремий
варіант, якого цей фікс не торкався (він лікує лише `ListFailed`-гілку підтвердження порожнього
стора). Сценарій 9's текст помилки довів саме `ListFailed` для `rotate_certificate`; сценарій 23's
звіт лише сказав «НЕ ВДАЛОСЯ», ніколи не називаючи варіант — тож майбутній живий тест, що знову
покаже `Failed`, може бути або цим фіксом-ще-не-повністю-live-verified, або зовсім іншим,
раніше не спостереженим `UninstallFailed`-шляхом; не змішувати два в одну знахідку без перевірки
конкретного тексту помилки.

**Не змінено (advisor, свідомо):** старе значення `17` НЕ лишено як додатково прийнятне (недосяжне
на реальному скомпільованому білді — приймати його теж було б захисним кодом для випадку, який не
може статись); `uninstall()` НЕ переведено на лагідну `thumbprints_for_common_name` — модуль-док
явно називає цю асиметрію навмисною (SPEC.md §2's "залишений довірений сертифікат після видалення"
— окремий, вже задокументований клас бага).
...
### T-222 — app-data wipe-хелпер спавнив непакований процес із логічним, а не фізичним шляхом (Батч 4.7.B, 2026-09-13, plan+advisor)

**Root cause:** той самий клас MSIX-віртуалізації, що й T-219, в геть іншій підсистемі, яку T-219's
фікс ніколи не торкався. `spawn_app_data_dir_wipe` (`crates/dnsqb-tray/src/self_uninstall.rs`)
спавнить детачнутий `powershell.exe` (`DETACHED_PROCESS | CREATE_BREAKAWAY_FROM_JOB`) з логічним
`%LOCALAPPDATA%\dns-quorum-filter`. Непакований дочірній процес (жодної package identity)
бачить цей шлях як буквально відсутній — `build_wipe_script_for`'s згенерований скрипт `while
((Test-Path -LiteralPath '{quoted}') -and $j -lt 30) { Remove-Item ... }` бив хибно на першій же
ітерації, `Remove-Item` НІКОЛИ не виконувався, спав самого хелпера все одно репортував `Ok`, тож
лог казав лише "app-data directory wipe scheduled" — без жодного натяку на провал. Живо
підтверджено сценарієм 23 попереднього смоук-тесту (реальний каталог лишався незмінним 60+с;
справжнє стирання того прогону сталося лише через побічний AppX-механізм видалення пакета в
«Параметрах», не через код застосунку).

**Advisor gate:** T-222 — 4.7.B, security/privacy-tier код без автоматизованого покриття
реального пакованого встановлення (той самий стандарт, що T-219/T-220/T-221).

**Фікс:** новий підхід, той самий "дай непакованому дочірньому процесу шлях, який він реально
бачить" прийом, що T-219 вже застосував для `-addstore` (там — тимчасова копія файла в
`%SystemRoot%\Temp`; тут — увесь каталог, копіювання недоречне, тож замість цього обчислюється
РЕАЛЬНИЙ фізичний шлях і передається хелперу):

- `package_family_name(exe_dir_name: &str) -> Option<String>` — чиста функція, парсить
  `PackageFamilyName` (`<Name>_<PublisherHash>`) з install-теки поточного процесу
  (`WindowsApps\<Name>_<Version>_<Arch>__<PublisherHash>\`, стандартна назва теки MSIX-інсталяції).
  **Свідомо ніколи не хардкодить конкретний хеш** — інший підписний сертифікат (ефемерний
  тест-білд `pack-msix.ps1` без `-PfxPath`) дає інший `PublisherHash` щоразу; підтверджено
  емпірично цієї сесії — реальний реліз (`8d78tvs37tgae`) і сценарій-24-тест-білд мали різні хеші.
- `physical_app_data_dir(local_app_data, exe_dir_name, app_data) -> Option<PathBuf>` — чиста,
  будує `%LOCALAPPDATA%\Packages\<PFN>\LocalCache\Local\dns-quorum-filter` з явно переданих
  параметрів, а не читанням середовища напряму (та сама причина, що
  `paths::resolve_app_data_dir`'s власний doc-коментар: `std::env::set_var` — `unsafe fn` на
  цьому тулчейні незалежно від едішена, несумісне з `#![forbid(unsafe_code)]`, тож тестованість
  вимагає явних параметрів, не мутації реального середовища процесу).
- `resolve_target(app_data) -> PathBuf` — нечистий шар: читає `current_exe()`+`%LOCALAPPDATA%`,
  додатково перевіряє, що обчислений фізичний каталог справді існує на диску (`is_dir()`), і лише
  тоді використовує його; в будь-якому іншому випадку (непакований білд, помилка читання
  середовища, чи каталог не існує) — фолбек на `app_data` без змін, той самий, уже коректний шлях
  для непакованого випадку.
- `spawn_app_data_dir_wipe` тепер викликає `resolve_target` і передає результат у
  `build_wipe_script` замість сирого `app_data`. `build_wipe_script_for`'s наявна валідація
  (`dir.starts_with(localappdata)` + `file_name() == "dns-quorum-filter"`) проходить без змін для
  ОБОХ форм шляху — фізичний шлях теж лежить під `%LOCALAPPDATA%` (просто через
  `\Packages\<PFN>\LocalCache\Local\`) і теж закінчується на `dns-quorum-filter`.

**5 нових юніт-тестів** для чистих `package_family_name`/`physical_app_data_dir`: реальна
install-тека, спостережена цієї сесії (`dns-quorum-filter_0.4.1.0_x64__8d78tvs37tgae`) →
коректний `PackageFamilyName`; непакований `debug`/`release` → `None`; деформований вхід (без
`_` перед іменем, без `__` узагалі) → `None`. `resolve_target` (нечистий шар) свідомо НЕ
окремо юніт-тестований — той самий прецедент, що `paths::app_data_dir()` ("thin wrapper... not
separately tested, trivial pass-through").

**НЕ живо перевірено — машина зараз без встановленого MSIX** (T-222/сценарій 23 попереднього
смоук-тесту повністю видалив застосунок з цієї машини; повний build+sign+install цикл із живим
UAC-підтвердженням `Trust-TestCert.ps1` вимагає участі користувача, поза межами автономного
проходу цього батчу). Потребує підтвердження на наступній реальній MSIX-установці: реальне
«Повністю видалити» має справді стерти фізичний каталог, **без** допомоги побічного AppX-механізму
видалення пакета в «Параметрах» (той самий фактор, що замаскував баг у сценарії 23 — якщо
користувач цього разу НЕ доведе видалення пакета до кінця в «Параметрах» одразу, це і буде
дискримінаційний тест: фізичний каталог мусить зникнути сам, без цього кроку).

**Closing-advisor рев'ю виловив два пункти, обидва виправлені перед комітом:**
1. `RUST-GOTCHAS.md`'s T-219-запис закінчувався формулюванням "`certutil -store`/`-delstore`...
   were **not** affected", що після T-222 читалось би як "розрив стосувався лише
   `certutil`-специфічного аргументу шляху" — неправильне узагальнення. Розширено: справжнє
   правило — *будь-який* спавнений непакований дочірній процес бачить логічний
   `%LOCALAPPDATA%`-шлях як відсутній, незалежно від інструмента; T-222 — другий, незалежний
   приклад цього класу в іншій підсистемі. Додано і сам прийом (вивід `PackageFamilyName` з
   `current_exe()`) як перевикористовуваний рецепт, а не лише посилання на T-219-наратив.
2. `resolve_target`'s фолбек-гілка нічого не логувала — у пакованому білді, де парсинг PFN
   спрацював, але `is_dir()` не пройшов, лог виглядав би точно як стара, зламана поведінка
   ("app-data directory wipe scheduled" і нічого більше — та сама фраза, що маскувала баг у
   сценарії 23). Додано `tracing::info!` в обидві гілки `resolve_target` (physical/logical), щоб
   наступна жива MSIX-перевірка могла однозначно розрізнити "фікс спрацював" від "фікс тихо не
   спрацював" від "непакований білд" — саме та відмінність, яку верифікаційний крок вище вимагає
   вміти зробити.

**Повна верифікація:** `cargo build`/`clippy --all-targets -D warnings`/`fmt --check`/
`test --workspace --lib --bins` (`dnsqb-tray`: 11 self_uninstall-тестів, +5 нових; workspace:
809+42+3, без регресій)/`cargo doc -D warnings` — усе зелено, повторно після closing-review фіксів.

### T-223 — MSIX-оновлення відмовляло з `0x80073D02` без ручного виходу з трею (Батч 4.7.B, 2026-09-13, plan+advisor)

**Проблема (запит користувача 2026-09-12, після сценарію 24 попереднього смоук-тесту):**
`Add-AppxPackage` на новішу версію відмовляє з `0x80073D02` («need to be closed»), поки хоча б
один із трьох процесів (`dnsqb-service`/`-tray`/`-watcher`) живий — Windows не закриває класичний
Win32-в-MSIX (Desktop Bridge) застосунок автоматично під час install-оновлення, на відміну від
типової UWP-поведінки. Ціль користувача: оновлення має бути прозорим і тихим — без ручного
«Вийти» через трей, без видимого збою деплою.

**Дослідження (перед проєктуванням — TASKS.md's власний текст явно вимагав це для T-223):**
TASKS.md пропонував напрямок «WinRT `PackageCatalog.PackageUpdating`/`PackageInstalling`-події,
десктоп-бридж процес підписується і сам ініціює graceful-shutdown» — але з трьома явними
невідомими (a/b/c), жодна з яких не перевірна без живого install-циклу. Офіційна MS-документація
(`learn.microsoft.com/.../packagecatalog.packageupdating`) підтверджує лише механіку підписки
(`OpenForCurrentPackage` → події для власного пакета), але **нічого не каже** про: чи ці події
взагалі надійні для Win32-в-MSIX (а не лише UWP), скільки є часу на реакцію, чи взагалі гарантовано
приходять до forced-close. Паралельний пошук виявив кращий, деплой-сторонній варіант:
`Add-AppxPackage`'s `-ForceApplicationShutdown`/`-ForceTargetApplicationShutdown` — документовані
прапорці саме для «оновлення без ручного закриття запущеного застосунку» (offіційна
PowerShell-довідка `Add-AppxPackage`, `UpdateSet`/`AddSet`/`RegisterSet` parameter sets).

**Рішення — deployment-side фікс, не app-side WinRT-підписка:**
- `-ForceTargetApplicationShutdown`, не `-ForceApplicationShutdown` — офіційна довідка розрізняє
  їх точно за обсягом: `-ForceApplicationShutdown` закриває «the package **or its dependencies**»,
  `-ForceTargetApplicationShutdown` — лише «processes associated with the package» саме. Перевірено
  `packaging/AppxManifest.template.xml`: жодного `<PackageDependency>` немає (лише
  `<TargetDeviceFamily>` — це обмеження версії ОС, не залежність-пакет), тож ширший варіант нічого
  зайвого не додає, а MS-технічний форум (techcommunity, "ForceApplicationShutdown and
  ForceTargetApplicationShutdown") явно рекомендує саме вужчий за замовчуванням.
- Без `-Update` — цей параметр стосується semantics *залежних* пакетів («A dependency package is
  removed from the user account when the parent app is removed»), не загального правила
  «новіша версія того самого `PackageFamilyName` = апдейт», яке в довідці описано окремо і працює
  вже на дефолтному `AddSet`, де обидва shutdown-прапорці й так доступні.
- **Розглянуто й відхилено:** `-DeferRegistrationWhenPackagesAreInUse` — єдиний варіант із нульовим
  переривання процесів, але «оновлення застосується при наступному запуску» неправильно для
  безперервно працюючого фільтра, який watchdog тримає живим — деплой рапортував би успіх, а
  оновлення так і не приземлилось би, поки користувач сам не перезапустить усе вручну (гірше за
  статус-кво, не краще).

**Фікс (мінімальний, без нового Rust-коду — весь фікс на рівні команди деплою):**
`packaging/Trust-TestCert.ps1`'s `-Install`-гілка (`Add-AppxPackage -Path $msix`) тепер додає
`-ForceTargetApplicationShutdown` — та сама команда, яку README вже рекомендує як основний шлях
(`.\Trust-TestCert.ps1 -Install`), тепер безшовно апдейтить уже запущену версію. Синхронно оновлено
README.md (обидва шляхи — через `-Install` і «сирий» `Add-AppxPackage` без нього) і
`packaging/README.md`'s «manual equivalent» команда + новий пояснювальний абзац, і
`.github/workflows/release.yml`'s згенерований `notes.md`-текст релізу (обидві гілки signed/test-
signed) — інакше кожен новий GitHub-реліз продовжував би публічно рекомендувати стару, зламану
команду.

**Свідомо звужений обсяг, порівняно з тим, що міг уявити собі користувач:** це закриває T-223 як
deployment-side фікс (прапорець командного рядка інсталяції), а не app-side WinRT-подію, яку
TASKS.md спершу накидав як напрямок дослідження — той напрямок свідомо відкинутий (три невідомі,
жодна не підтверджена документацією) на користь вужчого, повністю задокументованого рішення, яке
не потребує нового коду в `dnsqb-service`/`-tray`/`-watcher`.

**Advisor gate:** T-223 — 4.7.B, plan-рівня задача per TASKS.md's власна вимога «потребує
власного plan+advisor циклу перед проєктуванням». Advisor-review до дизайну виловив: (1) прочитати
`Trust-TestCert.ps1` **перед** рішенням, де саме фікс має жити (там, а не лише в README, інакше
користувач просто міняє одну ручну дію на іншу); (2) не додавати `-Update` (неправильна семантика
залежностей); (3) перевірити маніфест на `<PackageDependency>` перед вибором прапорця (немає —
`-ForceTargetApplicationShutdown` вужчий і правильний); (4) назвати відхилений WinRT-напрямок і
`-DeferRegistrationWhenPackagesAreInUse` явно в записі, а не мовчки опустити.

**НЕ живо перевірено — та сама причина, що й T-222** (машина без встановленого MSIX; повний
build+sign+install-цикл вимагає участі користувача). Потребує підтвердження на наступній реальній
MSIX-установці **разом із T-222** (одна жива сесія, не дві окремі): 0.4.0→новіша версія через
`Trust-TestCert.ps1 -Install` без попереднього ручного виходу з трею дійсно оновлюється, і
фізичний app-data каталог (T-222) дійсно стирається без допомоги AppX-механізму «Параметрів».
**Відкрите питання, яке advisor назвав, але яке навмисно НЕ вирішувалось заздалегідь (потребує
живого спостереження, не спекуляції):** якщо Appx-forced-shutdown послідовний, а не атомарний по
всіх трьох процесах пакета, `dnsqb-watcher` (5-секундний тік, `CREATE_BREAKAWAY_FROM_JOB`) міг би
встигнути респавнити щойно вбитий `dnsqb-service` посеред деплою — потенційно повторний
`0x80073D02` чи напів-зареєстрований пакет. Наявний важіль на цей випадок, якщо підтвердиться:
`quit.flag`, записаний ПЕРЕД деплоєм (сигнал watcher'у «зупинити сервіс і вийти, не респавнити») —
але сам запис цього прапорця з непакованого/елевейтед PowerShell-скрипта наступив би на ту саму
T-219/T-222 MSIX-віртуалізаційну пастку (потрібен фізичний `Packages\<PFN>\LocalCache\Local\`
шлях, і саме LOCALAPPDATA викликача, не елевейтед-сесії) — не реалізовано зараз, лише
задокументовано як залежність, якщо жива перевірка покаже, що це реальна проблема.

**Верифікація (наскільки можлива без живого MSIX):** зміни — PowerShell-скрипт + документація +
`release.yml`, без Rust-коду; `ci.yml`/`codeql.yml` **не** підпадають під `paths-ignore` (той
покриває лише `**/*.md`/`diagrams/**`/`mockups/**`, не `.ps1`/`.yml`), тож CI на цей пуш реально
запуститься — але жодного з його кроків ця зміна не зачіпає, зелений тік тут підтверджує
відсутність регресії в решті workspace, не сам T-223-фікс. `packaging/Trust-TestCert.ps1`
синтаксично перевірено (`[System.Management.Automation.Language.Parser]::ParseFile`, 0 помилок).
`release.yml`'s `notes.md`-шаблон відрендерено окремим скретч-скриптом з обома значеннями
`SIGN_MODE` (`signed`/`test-signed`) — бектики в обох гілках виходять одинарними, текст коректний.

### T-229 — одноразовий near-tray попап замість постійного трей-бейджа (Батч 4.7.B, 2026-09-13, plan+advisor, два раунди)

**Запит користувача 2026-09-12:** постійний знак оклику на трей-іконці, якщо застосунок
встановлено, але браузер довго не вказаний на нього як DoH-провайдер (жодних запитів), з
поясненням у тултипі. **Конфліктувало з чинним правилом проєкту** «An always-on warning is
functionally identical to no warning» (Три Б, SPEC.md §8.1; прецеденти T-56/T-57/T-77) — сталий
бейдж саме та форма, яку це правило забороняє.

**Крок 1 — рішення користувача щодо компромісу:** запропоновано (TASKS.md вже мав цю рекомендацію
записаною наперед) одноразове сповіщення замість беджа, детекція «нуль запитів за вікно» через
`AdminStats.total`, той самий once-per-process+persisted-marker узор, що `onboarding`-модуль уже
використовує. Користувач прийняв цю рекомендацію.

**Крок 2 — уточнення користувача мідсесії:** сповіщення має бути невелике, спливаюче біля трею, не
на весь екран і не по центру — це прямо виключає `rfd`'s центровані нативні діалоги (`onboarding`'s
власний механізм), тож рендер довелось спроєктувати окремо від детекції.

**Детекційна половина** (`crates/dnsqb-tray/src/browser_nudge.rs`, дзеркалить `onboarding.rs`'s
форму): `should_offer_browser_nudge(now, first_seen, total_queries, seen) -> bool` — чиста функція;
`first_seen`/`mark_first_seen_if_absent` — `first-seen.stamp` (епох-мілісекунди текстом, не
файлова mtime — стійко до копіювання/відновлення каталогу); `browser_nudge_seen`/
`mark_browser_nudge_seen` — окремий `browser-nudge.seen`-маркер (незалежний від `onboarding.seen`,
два нагадування спрацьовують незалежно). Поріг `NUDGE_AFTER` = 30 хв від першого запуску —
довільне рішення, не з SPEC.md. 9 нових тестів (щасливий шлях, межа порогу, відсутність доказу,
ненульовий лічильник, майбутній `first_seen` через розбіжність годинника, ідемпотентність
маркерів).

**Advisor-catch #1 (перед рендером):** `maybe_offer_browser_nudge`'s перша версія писала
`browser-nudge.seen` одразу в момент виявлення умови, ще ДО того, як рендер взагалі існував —
це спалило б єдиний живий постріл маркера назавжди на будь-якій машині, що встигла запустити цей
проміжний білд, ще до вибору рендера. Виправлено: маркер пишеться лише в точці, де
`nudge_popup::spawn` реально повернув вікно (успіх рендера), ніколи — просто на умові.

**Дослідження рендер-варіантів (перед проєктуванням, як і вимагало TASKS.md):**
- (a) Windows balloon tip (`Shell_NotifyIcon`/`NIF_INFO`) — `tray-icon` 0.21's
  `TrayIcon::window_handle()` публічний, але приватний `uID`, під яким існуючу іконку
  зареєстровано в Win32, ні (перевірено читанням вихідного коду крейту
  `platform_impl/windows/mod.rs`) — тож balloon не можна причепити до вже наявної іконки без
  окремої, другої `NOTIFYICONDATA`-реєстрації: або новий зовнішній crate (`notify-icon` — вимагає
  `unsafe` у виклику й власного вікна/message loop, конфліктує з наявним `tray-icon`+`tao`), або
  власний `unsafe`-код (проєкт — `#![forbid(unsafe_code)]` всюди).
- (b) Windows Toast (WinRT `Windows.UI.Notifications`) — відхилено окремо: вимагає package
  identity, тож структурно неперевірний на цій машині без встановленого MSIX (третя
  неперевірена-наживо фіча в тому самому батчі поряд із T-222/T-223 — забагато).
- (c) кастомне `tao`-вікно біля трею — обране користувачем. Перевірено (advisor-запит) чи можна
  обійтись зовсім без нової залежності: `tao` сам не вміє малювати пікселі (лише вікна+events),
  тож будь-який контент все одно потребує рендер-шару. `softbuffer` 0.4.8 — маленький, безпечний
  (жодного `unsafe` у публічному API), Windows-бекенд без Linux-специфічних дефолтних фіч
  (`default-features = false` вимикає `kms`/`x11`/`wayland`) — обраний. Новий SECURITY.md-рядок.

**Фікс (рендер-половина, `crates/dnsqb-tray/src/nudge_popup.rs`):**
- Статичне RGBA-зображення 540×108, згенероване `assets/gen-icon.py::make_browser_nudge_popup`
  (той самий Pillow-конвеєр, що й трей-гліфи — `include_bytes!`, без рантайм-рендерингу тексту, без
  залежності від шрифтового стеку в `dnsqb-tray`; T-151 i18n не почато, тому копірайт фіксований
  українською). Розмір підібраний емпірично — виміряно фактичний bbox тексту через Pillow
  (`multiline_textbbox`), а не вгадано на око (перша спроба 340×96 обрізала текст праворуч,
  виправлено на 540×108 після виміру).
- `SharedWindow(Rc<Window>)` — тонка обгортка, що делегує вже безпечні `HasWindowHandle`/
  `HasDisplayHandle` (з `tao::rwh_06`, той самий `raw-window-handle` 0.6, що й `tao` реекспортує —
  жодної нової залежності на цей трейт не додано) — потрібна, бо `Context`/`Surface`
  вимагають володіючого хендла, а `Window` сам не `Clone`.
  Вбудований UI-конвенції прийом ("everything in, no runtime asset I/O") — той самий, що трей-іконки.
- `popup_position(tray_rect, monitor_size) -> (i32, i32)` — чиста функція, розміщує попап над
  трей-іконкою, затиснута в межі монітора; 4 нових тести (центрування, затискання по правому краю,
  затискання по (0,0), монітор менший за попап — не інвертує діапазон затиску).
- `spawn` — нечистий шар: `WindowBuilder` (без декорацій, always-on-top, не в фокусі, поза
  taskbar через `WindowBuilderExtWindows::with_skip_taskbar`), створюється на ТОМУ Ж
  `EventLoopWindowTarget`, що вже веде трей (жодного другого event loop чи потоку) — `main.rs`'s
  замикання вже отримує `target` на кожному тіку. Малює зображення один раз через
  `softbuffer::Surface::buffer_mut`, `present()`. Повертає `None` на будь-якій помилці (вікно,
  монітор, поверхня) — best-effort, ніколи не варте падіння трею.
- `NudgePopup::should_close(event, lifetime) -> bool` — клік будь-де по попапу (немає декорацій чи
  кнопки закриття) або спливання `POPUP_LIFETIME` (8с).
- `main.rs`: `drive_browser_nudge` — одна точка виклику, що поєднує `maybe_offer_browser_nudge` +
  перевірку закриття, аби не роздувати `main()` (advisor + clippy `too_many_lines` це підтвердили —
  `main()` знадобилось додатково винести `bootstrap()` — інстанс-гвард/pid-файл/watcher-safety-net/
  порт — структурний рефакторинг, не косметика, той самий прецедент, що
  `too_many_arguments`/`too_many_lines` завжди фіксяться структурно, ніколи `#[allow]`).

**Advisor-catch #2 (той самий раунд):** оригінальна ідея "чи можна обійтись без нової залежності"
підтвердилась негативно емпірично (`tao` не малює пікселі), але перевірка була варта 10 хвилин —
`softbuffer` виявився мінімально можливим вибором (не повний GUI-тулкіт), і публічний
`TrayIcon::window_handle()`/`rect()` (для позиціювання) підтвердили, що інтеграція з наявною
трей-іконкою можлива без другої іконки-реєстрації.

**Верифікація:** `cargo build`/`clippy --all-targets -D warnings`/`fmt --check`/
`test --workspace --lib --bins` (`dnsqb-tray`: 55 тестів, +13 нових цього кроку: 9 detection + 4
nudge_popup)/`cargo doc -D warnings`/`cargo deny check` (licenses/bans/advisories/sources ok;
`bytemuck`/`objc2-io-surface`/`objc2-quartz-core` увійшли в `Cargo.lock` через `softbuffer`'s
вимкнені `kms`/macOS-фічі — підтверджено `cargo tree -e no-dev --target x86_64-pc-windows-msvc`,
жоден не компілюється для цієї цілі, лише сам `softbuffer`) — усе зелено.

**Живо перевірено рендер (advisor-запит — колір і читабельність жоден юніт-тест не ловить):**
окремий, одноразовий scratch-проєкт (поза репо, у скретчпеді) з тим самим блітом
RGBA→`0x00RRGGBB`, тим самим `WindowBuilder`/`softbuffer`-конвеєром, тією ж самою реальною
`browser-nudge-rgba.bin` через `include_bytes!` — запущений, зроблено скріншот реального вікна на
екрані. Результат: колір фону/гліфа/тексту коректний (синій гексагон, не червоний — підтверджує,
що канали RGBA не переплутані), текст повністю читабельний, не обрізаний, відповідає Pillow-прев'ю.
**Не перевірено живо в цій сесії:** повний інтегрований тригер через реальний `dnsqb-tray.exe`
(30 хв простою + нуль запитів + справжня трей-іконка для позиціювання) і поведінка фокуса
(`with_focused(false)`) під час реальної роботи користувача — обидва відкладені до наступної живої
MSIX-сесії разом із T-222/T-223.

**Запит:** підтверджено 2026-09-12 (Батч 4.7.A аудит): поле вводу коду блокованої країни
(`#geoip-body`) приймає лише сирий дволітерний ISO 3166-1 alpha-2 код — жодної підказки/пошуку за
назвою країни ніде в UI.

**Фікс:** один нативний `<datalist id="geoip-country-list">` (без нового npm/Cargo-пакета —
браузерна автопідказка з коробки), побудований один раз при завантаженні модуля (`COUNTRY_NAMES`
не змінюється в рантаймі) і повторно приєднуваний до `addRow` при кожному `renderGeoip()` виклику
(той самий DOM-вузол просто переноситься, не перебудовується — `geoipBody.textContent = ""`
відв'язує його від старого батька, але не знищує). `COUNTRY_NAMES` (код → англійська назва, 249
записів) згенеровано скриптом через `pycountry` (canonical ISO-codes дані), не передруковано
вручну — та сама причина, що глобальне правило про великі текстові блоки: ризик мовчазного
усічення/переставлення рядків при ручному передруку 249 значень. **Англійська, не українська,
назва — свідомий вибір:** не було перевіреного джерела українських назв ISO 3166-1 під рукою, а
хибний переклад гірший за коректну англійську; об'єкт структуровано як один плаский `COUNTRY_NAMES`
(той самий "стане locale-lookup при T-151" патерн, що й T-159's `FIELD_HELP`).

**Знайдено й виправлено під час живого тесту (advisor-worthy self-catch, не окрема знахідка):**
перший прохід лишив старий `input.maxLength = 2` незмінним — це мовчки відрізало будь-яку введену
назву країни до 2 символів ще ДО того, як браузер встиг зіставити її з datalist, тобто повністю
унеможливлювало пошук за назвою (можна було вводити лише вже готовий код). Виявлено саме живим
тестом у Chrome (`fill("Ukr")` → реальне значення поля виявилось `"Uk"`), не читанням коду.
Виправлено підняттям ліміту до 64 символів (достатньо для найдовшої назви країни) — валідація
формату лишається на `submitAdd`'s власному regex-і, `validate_country_code` на сервері й далі
єдиний реальний гейт.

**Живо підтверджено в Chrome** (chrome-devtools MCP, скретч-інстанс власного dev-білду, ізольований
`LOCALAPPDATA`, порт 8443): `fill("Ukraine")` → поле зберегло повний текст (без усічення) →
`evaluate_script` підтвердив datalist містить 249 `<option>`, `UA` → `"UA — Ukraine"` → введення
`"UA"` → «Додати» (озброює) → «Підтвердити додавання» → `GET /admin/geoip` показав
`blocked_countries: ["UA"]`, `persisted: true` → «Видалити» → знову порожньо. Повний цикл
додавання/видалення підтверджено, не лише сама підказка.

**Документація:** UI-SPEC.md §3.5 (нотатка про `<datalist>`+пояснення вибору англійської назви).
Без змін у Rust-коді — увесь фікс у `crates/dnsqb-service/ui/main.js` (переembeddений через
`include_str!` при наступному білді). Повний build/clippy/fmt/test (804 passed) — зелено.
...
### T-227 — адаптивний дефолт зон бульбашки + чесна корекція застарілої "Знахідки" (Батч 4.7.B, 2026-09-13, full plan-режим)

**Запит користувача мав три пункти** (2026-09-12): (1) дефолтний список країни — з регіону
встановлення, не фіксований набір; (2) завжди доступний преналаштований набір «глобальний,
науковий/освітній тощо» зверху списку вибору; (3) можливість пошуку й **лінивого**
(за запитом) довантаження списку іншої країни.

**Стаж-корекція, знайдена ДО імплементації (найцінніша частина цієї задачі):** початкова
"Знахідка" в TASKS.md стверджувала, що `GovernmentTopN`/`SciEdu` мають код, але жодного
розповсюдженого датасету — тобто пункт (2) нібито не реалізований. Пряма перевірка коду це
спростувала: `data/topn/` уже мав усі 11 закомміченних файлів з `.sha256`-сайдкарами
(`ua/us/de/pl/gb/global/gov-ua/gov-us/gov-pl/gov-gb/edu`), `topn_download::AVAILABLE_TOPN_LISTS`
уже перелічувала всі 11, а `dispatch.rs`/`admin_ui.rs` уже прокидали їх у `/admin/ui`'s
zone-picker (`admin_ui.rs`'s власні тести стверджують рендер саме з `available_lists`, не
хардкод). **Пункт (2) уже був повністю зданий до появи T-227.** Виправлено дві застарілі згадки:
сам запис T-227 (TASKS.md) і T-135's перехресне посилання, яке повторювало ту саму помилку.

**Пункт (3) свідомо НЕ будується.** Користувач підтвердив: без live-курації взагалі —
«юзер не буде довбатися з цим, максимум вибере відповідну країну чи готовий набір» — і це
узгоджується з DECISIONS.md 2026-09-09 (курація — офлайн `curate_topn` example +
`topn-curate.yml` `workflow_dispatch` → артефакт → людський PR, свідомо не runtime/client-тригер,
щоб уникнути і rate-limit ризику, і неревʼюваної якості даних).

**Пункт (1) — реально збудована частина.** Спершу розглядався `sys-locale` (BCP-47 локаль), але
advisor-раунд (до написання коду) виявив дві проблеми з початковим планом:
- `GetUserDefaultGeoName` напряму (через crate `windows`) — `unsafe fn`, несумісний з
  `#![forbid(unsafe_code)]` цього проєкту; `forbid` не можна локально обійти `#[allow(...)]`.
- `sys-locale` насправді читає `GetUserPreferredUILanguages` (мову інтерфейсу Windows), не
  регіон — користувач з українським регіоном, але англомовним інтерфейсом отримав би `en-US`,
  саме той mismatch, який ця фіча мала виправляти.

Знайдено третій шлях: реєстр `HKCU\Control Panel\International\Geo`\`Name` — те саме значення,
яке пише панель «Параметри → Регіон → Країна чи регіон», уже готовий 2-літерний ISO 3166-1 код
(підтверджено на dev-машині: `Name = "UA"`, поряд зі старим числовим `Nation = 241`). Читається
через `winreg` 0.55.0 (`[target.'cfg(windows)'.dependencies]`, новий прямий рантайм-залежність —
раніше був лише транзитивним build-dep через `embed-resource`) — увесь `unsafe` FFI інкапсульовано
в самому `winreg`, `forbid(unsafe_code)` лишається інтактним у кожному крейті проєкту. Другий
advisor-раунд (після знахідки реєстрового шляху) підтвердив підхід і виявив blind spot: у
`dispatch.rs` є ОДНА функція-вирва `rating_filter_status_view`, викликана з обох
`AdminStatusResponse`-білдерів — підказку треба рахувати саме там, не в одному з двох
call-сайтів, інакше бейдж і статус розійшлися б залежно від того, який маршрут відповів.

**Збудовано:**
- `crates/dnsqb-service/src/install_region.rs` (новий, приватний модуль) — чиста серцевина
  `normalize_region_code`/`suggested_list` + один `#[cfg(windows)]`/`#[cfg(not(windows))]`
  нечистий шар `detect_system_region` (Фаза 6 seam — на не-Windows повертає `None`, видимо, а не
  вкомпільовано). OS-виклик винесено з `AppState::new` у `orchestrate::build_geoip_init`
  (impure shell), а не всередину конструктора — інакше кожен юніт-тест читав би реальний реєстр
  цієї машини замість injected-значення.
- `GeoipInit.system_region: Option<String>` (нове поле, попри назву структури — той самий
  "grab-bag для параметрів AppState::new" патерн, що вже містив `rating_filter_config`/
  `rating_filter_zone`) → `AppState.system_region` (звичайне поле, без `RwLock` — регіон ОС не
  змінюється, поки процес живий).
- `RatingFilterStatusView.suggested_list: Option<String>` (`#[serde(default)]`,
  `ADMIN_DTO_SCHEMA_VERSION` 2→3) — рахується в `rating_filter_status_view` лише поки
  `config.lists` порожній; ніколи не пише в `[rating_filter].lists` самостійно.
- `admin_ui.rs`'s `main.js` zone-picker — маленька inline-підказка ("Рекомендовано для вашого
  регіону: Україна.") + кнопка «Додати», **ніколи не пречекнутий чекбокс**. Це не точковий вибір
  дизайну — мід-раунд advisor-фідбек прямо попередив: пречекнутий чекбокс + повний-replace
  `POST /admin/rating-filter` означає, що клік лише по `enabled`-перемикачу мовчки закомітив би
  підказку як `lists`, той самий клас бага, що й "always-on попередження ≡ відсутність
  попередження" (СПЕК §8.1), тільки іншим шляхом. Підказка ховається, щойно `picked.size > 0`
  (клік по будь-якій зоні, не лише по кнопці підказки).

**Живий self-catch під час ручного смоук-тесту (не з `MAIN_JS.contains(...)`):** перший CSS-варіант
(`.notice.rf-suggestion { display: flex; ... }`) мав вищу specificity, ніж стандартне
`[hidden] { display: none }` браузера — підказка лишалася видимою навіть після встановлення
`hidden`, що й підтвердив живий клік у реальному Edge (застосунок → скретч `LOCALAPPDATA`,
`Add-AppxPackage`-незалежний debug-білд, `thisisunsafe` для self-signed сертифіката,
`ui_click`/`ui_find` через Windows MCP): «Додати» справді додав «Україна» до вибраних зон, але
рядок-підказка лишився зверху. Виправлено звуженням правила до `.notice.rf-suggestion:not([hidden])`
— юніт-тести (`MAIN_JS.contains("picked.size > 0")` тощо) перевіряють лише що JS-логіка існує,
не що CSS дозволяє їй реально ховати елемент; жоден із них цю різницю не ловить.

**Повний гейт пройдено:** `cargo build`/`clippy --all-targets -D warnings`/`fmt --check`/
`test --workspace --lib --bins` (826 passed)/`test --workspace --doc` (9, без нового доктесту —
`install_region` приватний модуль, приклади лишаються юніт-тестами)/`test --test conformance`/
`test --test admin_client`/`test --workspace --examples`/`cargo doc` (RUSTDOCFLAGS=-D warnings)/
`cargo deny check` (0 нових duplicate-версій — `winreg` перевикористовує вже наявний у дереві
`windows-sys` 0.59.0 через `rfd`). `cargo tree -e no-dev --target x86_64-pc-windows-msvc -i winreg`
підтвердив живе компілювання для цільової платформи (до T-227 — порожній вивід, лише build-dep).

**Наслідки в доках:** `SECURITY.md` (новий рядок `winreg`), `CLAUDE.md` (Runtime dependencies
bullet), `CONFIGURATION.md` (§`[rating_filter]`, новий абзац про підказку), `TASKS.md` (T-227
позначено `[x]` з виправленою "Знахідкою"; T-135's перехресне посилання теж виправлено).

**CodeQL:** підтверджено 0 відкритих алертів на HEAD перед початком T-227 (перевірка була
відкладена через T-220–223/T-229 і закрита в цьому ж заході).

## Фаза 7 — Публічні блок-лист-бандли (завершені)
...
- [x] T-218 — Опційний, default-OFF шар публічних блок-лист-бандлів поряд із кворумом (мета —
  ...
  **[x] Батч 7.4, частина 3 (2026-09-17, plan-mode + advisor до і після реалізації) —
  `/admin/ui`-рендер картки, T-218 закрито.** Користувач дав добро продовжувати, спираючись на
  те, що мокап уже містить конкретний дизайн (Артборд F, 4 кадри) — мокап узятий за затверджене
  рішення для кожного з власних "Відкритих питань", без додумування: картка `#blocklist-
  bundles-body` — сиблінг `#overrides-body` у базовому вигляді (не в `<details>` «Розширені»);
  per-джерело чекбокси; групування «Реклама й трекери»/«Безпека й загрози» (не кворум-категорії
  SECURITY/ADS_TRACKERS/ADULT_CONTENT); без confirm-кроку при увімкненні; без окремого бейджа
  під hero; список джерел — власний `<details class="bl-sources">`, закритий за замовчуванням.
  Advisor-рев'ю плану **перед** реалізацією зупинило дворядковий Fork-B, скопійований з
  `renderRatingFilter`: `sources: Some([])` — задокументований у `KNOWN-LIMITATIONS.md` стан, де
  `active` лишається `true` на застарілому наборі (`refresh_all_sources` виходить раніше, ніж
  торкається bundle) — знадобились три гілки опису, не дві, інакше картка постійно брехала б
  "завантажується за кілька секунд". Той самий прохід виловив ще три класи помилок: (1) знятий,
  але ще фізично активний (до завершення наступного циклу) source мав власний текст рядка,
  відмінний від справді "не відстежується" (і відмінний від того самого стану при **цілком**
  порожньому виборі, де жоден наступний цикл не прийде взагалі); (2) orphan id (поза статичним
  клієнтським каталогом) рендериться як власний нескластерований рядок, не зникає мовчки й не
  веде до голого "HTTP 400" на наступному непов'язаному кліку; (3) `entry_count` виключено з
  рендеру повністю (pre-dedup, не сумовний — той самий клас, що T-66 вже виправляв для `gov-*`
  зон), об'єднаний `hagezi-nrd-dga`-рядок показує статичний факт "2 джерела", не суму. Закриваючий
  advisor-рев'ю (перед комітом, після ручної browser-перевірки через chrome-devtools MCP) довиловив
  ще один клас: **"Активно, N джерел" рахував розмір каталогу (7), не фактично обраний набір** —
  та сама фальшива-лічба помилка, яку `zoneMeta()` вже відмовився робити для `gov-*`; виправлено
  на живий підрахунок рядків із хоча б одним обраним id (число тепер рухається, коли знімаєш
  джерело); заразом виправлено відсутній відкат чекбокса й `disabled` на час запиту (те, що вже
  мав master-перемикач, per-джерело чекбокси — ні). Ручна browser-перевірка (chrome-devtools MCP,
  самопідписаний сертифікат прийнято вручну через інтерстиціал) підтвердила: розміщення картки
  (між СПІСКИ ВИКЛЮЧЕНЬ і Розширені), увімкнення без confirm-кроку, Fork-B нотис, групування з
  об'єднаним nrd/dga-рядком, `POST`-тіло master-перемикача (`sources:null`, незмінне) і
  per-джерела чекбокса (`sources` — явний масив без обох знятих id, довели, що пара nrd/dga
  знімається разом). Гілки "порожній вибір" і "orphan-рядок" лишились лише кодо-верифіковані
  (`loaded` не встиг наповнитись живим циклом у смоук-сесії) — не пройдені вживу браузером,
  зазначено чесно, а не як повний прохід. Повільна відповідь сервера під час смоук-тесту (реальне
  фонове завантаження ~111 MB із живих джерел + накопичені паралельні браузерні ретраї) —
  підтверджена пряма поведінка вже наявного `blocklist_updater` (Батч 7.2/7.3), не бага цього
  батчу. Змінено лише `ui/*` (index.html/main.js/style.css) + 5 нових smoke-тестів у
  `admin_ui.rs` (`MAIN_JS`/`INDEX_HTML.contains(...)`) — жодного нового DTO-поля, маршруту чи
  state machine, тож `diagrams/README.md`'s ritual не спрацьовує (перевірено, не чіпалось).
  `cargo build`/`test --workspace --lib --bins` (872)/`clippy --workspace --all-targets -D
  warnings`/`fmt --all -- --check`/`doc --no-deps --document-private-items`/`--test
  conformance`/`--test admin_client`/`--workspace --doc` — усі зелені, увесь workspace.
  DECISIONS.md 2026-09-17 ("Батч 7.4 (частина 3)"). **T-218 і `## Фаза 7` повністю закриті** —
  жодного відкритого підпункту не лишилось.

- [x] T-232 — **`blocklist_updater::hash_source_body`+`finalize` виконувались синхронно всередині
  async-таска (`refresh_all_sources`, T-218 Батч 7.2), не через `tokio::task::spawn_blocking` —
  вимір 2026-09-17 підтвердив, що це реальна проблема, не лише мережа; фікс реалізовано того ж
  дня.** Виникло під час ручної
  browser-перевірки Батчу 7.4 частина 3 (сервер відповідав повільніше під час фонового рефешу);
  цей запис — результат окремого дослідження за запитом користувача 2026-09-17, з реальними
  завантаженими файлами (122 MB, 8 джерел, той самий `<app-data>/blocklists/*.txt`).
  - **Виміряно (release-збірка, не debug — debug дав оманливо песимістичні 182s):** хешинг
    восьми джерел + фінальний `sort_unstable`+`dedup` над об'єднаним ~5.8M-елементним
    `Vec<u64>` — **12.5-13.8s сумарно**; `write_atomic` восьми файлів — **0.4-0.6s** (диск
    не вузьке місце, CPU-парсинг — так). Найважчий одиничний сегмент (без мережевих `.await`
    між ним і сусідніми) — `hagezi-nrd`, **5.7s**. Джерело витрат — `push_normalized` →
    `normalize_domain` (IDNA/punycode через `hickory-proto::Name`) на кожен рядок (3.18M рядків
    у hagezi-nrd).
  - **Дискримінуючий експеримент (яке саме питання T-232 просив перевірити перед фіксом):**
    окремий tokio-рантайм із контрольованою кількістю `worker_threads`, конкурентний "пробер"
    на `sleep_until` із фіксованим абсолютним дедлайном (не відносний `sleep` — перша спроба
    мала методологічну ваду: відносний таймер губить затримку, бо перевимірює себе від моменту
    власного пробудження; спіймано й виправлено в тому ж дослідженні). Результат:
    - `worker_threads=1` → макс. затримка пробера **12.94s** (≈100% застою на весь час хешу).
    - `worker_threads=2` → **16.9ms**.
    - `worker_threads=default` (12 ядер на dev-машині) → **17.8ms**.
  - **Чесний висновок:** на будь-якій реальній сучасній Windows-машині (≥2 логічних ядра —
    практично весь парк, на який націлений MSIX) синхронний хешинг **не** створює відчутного
    застою запитів — вчорашнє 11.5s-сповільнення коректно пояснюється реальним мережевим
    трафіком/TLS-реконектами, як і було діагностовано тоді. Але код і далі порушує
    `~/.claude/rules/rust.md`'s "жодної sync CPU-роботи в async-таску" **структурно** — ризик
    не нуль, а точно виміряно наскільки він росте, якщо `tokio::main`'s дефолтний
    `worker_threads = available_parallelism()` колись опиниться рівним 1 (обмежене
    контейнерне/VM середовище, не типовий десктоп-таргет, але дешево захиститись).
  - **Той самий клас в інших місцях (перевірено за запитом — "чи не схожі місця"):**
    `spawn_blocking` в усьому дереві використовується лише двічі (`cert_watch.rs:46`,
    `dispatch.rs:3676`, обидва — `certutil`). Синхронна робота всередині async-тасків є ще тут:
    - `paths::write_atomic` (`File::create`+`write_all`+**`sync_all`**) — 6 викликачів
      (`cache_persist`, `log_persist`, `personal_zone_persist`, `zone_removal_persist`,
      `blocklist_updater`, `topn_updater`). Виміряно швидким на SSD (0.4-0.6s/122MB) —
      `sync_all` під навантаженням на HDD/мережевому диску не піддається виміру на цій машині.
    - `geoip_updater::try_one_release` — `decompress_bounded` (gzip) + `extract_mmdb_from_tar_gz`
      + `checksum_matches` + `GeoipReader::from_bytes` + `persist_atomically`, усе синхронне.
      Реальний `geoip.mmdb` — лише 7.6 MB (на два порядки менше блок-листів) — сьогодні
      негайно, але `MAX_GEOIP_DECOMPRESSED_BYTES` = 256 MB — це стеля, не поточний розмір.
    - `log_persist`/`cache_persist`/`zone_removal_persist`/`personal_zone_persist` — той самий
      клас (AEAD seal + `write_atomic`), але на 60s-циклі (не 24h) з обмеженими розмірами
      (лог ≤1000 записів, кеш ≤10 000, `MAX_TRACKED_DOMAINS`=2000) — той самий клас, інший
      каденс, низький пріоритет.
    - `topn_updater` — файли по 10-14 KB — негайно, не пріоритет.
  - **Реалізовано (2026-09-17, план+advisor-цикл, окремий коміт):** три call-сайти обгорнуто в
    `tokio::task::spawn_blocking` — (1) `refresh_one_source`'s успішний шлях (нова приватна
    `write_and_hash_blocking` — write+utf8+hash одним блокуючим хопом, не двома); (2)
    `refresh_all_sources`'s fallback-гілка на last-known-good (нова `read_and_hash_blocking`,
    `JoinError` там колапсує в порожній `Vec` — той самий результат, що й відсутній файл); (3)
    фінальний `finalize(&mut domains)` — на `JoinError` цикл повністю відкидається (не
    застосовується не-`finalize`-ований `Vec` — `matches_domain`'s `binary_search` вимагає
    сортування, тож попередній, ще валідний бандл лишається чинним замість часткового
    результату). Новий варіант `BlocklistRefreshError::Join` + `label() == "join"` — перевірено
    безпечним для UI (`main.js`'s `rowStatus` лише перевіряє truthiness `last_error`, не
    конкретне значення). `BLOCKLIST_FETCH_TIMEOUT`'s doc-коментар виправлено: `tokio::time::timeout`
    не може скасувати вже запущений `spawn_blocking`-таск, тож константа тепер обмежує лише
    fetch+ініціацію, не завершення хешу.
  - **Повторний вимір (той самий дискримінуючий експеримент, крізь нові `spawn_blocking`-обгортки
    — перша спроба помилково зміряла старий прямий виклик, спіймано й виправлено в тому ж
    дослідженні):**
    - `worker_threads=1` → **17.3ms** (було 12.94s — падіння ~750x, більше не корелює 1:1 з
      тривалістю хешу — точний критерій успіху з плану).
    - `worker_threads=2` → 16.8ms (було 16.9ms — без змін, як і очікувалось).
    - `worker_threads=default` (12 ядер) → 34.2ms (було 17.8ms — трохи вище через
      dispatch-накладні на `spawn_blocking`-пул, не регресія). Разова wall-clock цифра
      hash+finalize з цього самого прогону (16.5s проти band 12.3-13.8s з усіх інших прогонів)
      **перевірена окремо** (3 повторні прогони, той самий сценарій): 13.3-14.4s — у межах смуги,
      підтверджено шум машини (паралельні release-збірки того ж сеансу), не наслідок фіксу.
  - **Свідомо не зачеплено цим фіксом** (задокументовано в плані, advisor погодив): `geoip_updater`
    (7.6 MB, стеля 256 MB — не пріоритет), `topn_updater` (10-14 KB — не пріоритет), 4 персистери
    на 60s-циклі (обмежені розміри — низький пріоритет), `write_atomic`'s `sync_all` під
    HDD/мережевим диском (не піддається виміру на цій машині). Кожне — окрема майбутня задача зі
    своїм план+advisor-циклом, якщо колись знадобиться, не мовчазне розширення скоупу цього фіксу.

- [x] T-233 — **Немає жодного runtime-захисту від скомпрометованого/зіпсованого публічного
  blocklist-джерела (T-218 Фаза 7)** — заведено 2026-09-17 за прямим запитом користувача,
  окреме дослідження в тій самій сесії, що T-232. **Підтверджено, не сама лише гіпотеза:**
  `MAX_BLOCKLIST_BYTES` (128 MB) — єдина межа, і то лише на розмір HTTP-відповіді, не на
  кількість/якість записів усередині. Жодне з 7 джерел не публікує `.sha256`-сайдкар
  (`data/blocklists/CANDIDATES.md` §3, звірено 2026-09-13 — та сама нотатка вже тоді визнавала
  "Фаза 7 потребує власного контролю цілісності, не прямого переюзання `data/topn/`'s
  sha256-контракту", але цей контроль так і не був побудований — Батчі 7.1-7.4 реалізували
  fetch/hash/config/UI, не integrity-гейт).
  - **Класи атак (дослідження 2026-09-17):**
    1. **Over-blocking / DoS через домішку легітимних доменів** — найдешевша й найреальніша
       атака. Блок-бандл лише блокує, ніколи не allowlist-ить, тож єдиний напрямок шкоди — DoS
       (гірше, ніж без фільтрації взагалі — прямо той клас, проти якого існує SPEC.md/CLAUDE.md
       "Три Б" user-safety leg), не bypass. Не треба ламати парсер — досить, щоб скомпрометоване
       GitHub-репо/CDN-кеш почало віддавати список із домішкою google.com/cloudflare.com/
       amazonaws.com тощо.
    2. **Формат/контент-плутанина** — джерело віддає HTML-сторінку помилки/rate-limit замість
       списку; зараз це мовчки хешується (більшість рядків просто не пройде `normalize_domain`),
       джерело тихо деградує до майже порожнього набору без жодного сигналу чи логу.
    3. **Ресурсне виснаження** — багато коротких рядків у межах 128 MB дає величезний `Vec<u64>`
       (пом'якшено T-232's `spawn_blocking`-фіксом, не усунено — тепер б'є по blocking-пулу, не
       по async-воркеру, але CPU/пам'ять і далі витрачаються).
    4. **Структурна діра, яку сам проєкт не помітив:** `matches_domain`'s суфіксний обхід (Батч
       7.4 частина 1, `blocklist_updater.rs`) явно виключає **голий TLD** (1-label кандидат
       ніколи не доходить до `binary_search`, доведено з самого коду циклу) — **але не виключає
       public suffix із 2+ labels** (`co.uk`, `com.ua`, `github.io`, `blogspot.com`,
       `pages.dev`...). Модуль **свідомо не використовує PSL**, спираючись на задокументоване
       припущення "опубліковані списки вже registrable-only" — припущення про довіру до джерела,
       ніяк не перевірене рантаймом. Один рядок `co.uk\n` у скомпрометованому файлі — і весь
       `.co.uk`-простір заблокований. PSL-файл (`crates/dnsqb-service/examples/
       public_suffix_list.dat`, 334 KB, MPL-2.0) уже є в репо, але прив'язаний лише до
       offline-тулзи `curate_topn` (Фаза 4) — не скомпільований у сам сервіс.
  - **Запропоновані перевірки (без нових зовнішніх залежностей, дослідження вже перевірило
    здійсненність кожної):**
    - **Дельта від попереднього успішного циклу** ("канарка" по кількості) — `entry_count`, що
      впав/стрибнув у кілька разів відносно останнього вдалого фетчу цього ж джерела, підозрілий.
      Самокалібрується per-джерело (NRD природно волатильний день-у-день — `~49 MB`/день,
      Multi PRO стабільний — `~222k` записів за `CANDIDATES.md`, реально зміряно 226255 у цій
      сесії — узгоджується), не треба хардкодити абсолютні межі для кожного з 8 джерел окремо.
    - **Відсоток успішного парсингу** — скільки не-коментар-рядків реально стало валідним
      доменом; різкий обвал (з ~95%+ до, скажімо, <50%) = формат змінився/вміст підмінили,
      незалежний сигнал від чистої лічби (рівна лічба сміттєвих рядків теж підозріла).
    - **Canary-перевірка проти вже наявного `data/topn/global.txt`** (822 живих топ-домени,
      реально вже фетчиться `topn_updater` у той самий `<app-data>` — **нуль нових
      залежностей**): якщо свіжозавантажене джерело раптом містить купу цих доменів — майже
      напевно компрометація; легітимний ad/malware/NRD/DGA-список цього структурно не робить.
    - **PSL-перевірка** (єдина, що потребує нової вартості — перенести/включити PSL-файл у сам
      `dnsqb-service` бінарник, +334 KB): відкидати запис, що сам є public suffix (не лише голий
      TLD, який `matches_domain` і так уже нейтралізує).
  - **Не заведено як точковий фікс — це новий рантайм-механізм** (нова логіка валідації в
    кількох місцях `blocklist_updater.rs`, можливий новий bundled-ресурс), тож власний
    план+advisor-цикл перед реалізацією, не мовчазне розширення T-232's скоупу. Некритична, не
    блокує жодну іншу задачу — але задокументовано як живий гап, не гіпотеза.
  - **Звужено 2026-09-17 (той самий "звужено, не закрито" узор, що T-51/T-56/T-221) — дві з
    чотирьох перевірок зроблено, окремим планом+advisor-циклом (advisor підтвердив: чотири
    перевірки різні за формою й ризиком зміни, одним комітом не варто; PSL-питання окремо
    вирішив користувач через AskUserQuestion — відкласти, не бандлити мовчки):**
    - **Відсоток успішного парсингу** — `blocklist_download::validate_hashes` +
      `MIN_PARSE_SUCCESS_RATIO` = 0.5, калібровано проти реальних 122 MB завантажених файлів
      (`hash_plain_domains`/`hash_adblock_domains` тепер рахують і `candidates`, не лише
      `out.len()`): реальний parse-success сьогодні 98.6%+ (adblock, найгірший з восьми) до
      ~99.99% (plain-domain) — 0.5 лишає ~48 п.п. запасу, це "джерело явно не те, чим себе
      називає" trip wire, не тонка якісна планка.
    - **Canary-домени** — `BLOCKLIST_CANARY_DOMAINS` (12 глобально популярних доменів,
      скомпільованих у бінарник, **не** залежних від `data/topn/global.txt` — той файл існує на
      диску лише коли `[rating_filter].enabled=true`, default OFF, перевірено відсутнім на
      реальній машині 2026-09-17), `MIN_CANARY_HITS_TO_REJECT` = 2 (не 1 — толерантність до
      одного випадкового збігу). Нуль exact-line збігів проти реальних восьми файлів, перевірено
      `grep -Fx` перед кодом.
    - **Advisor-catch, виправлено в тому ж коміті (не окрема задача):** `write_and_hash_blocking`
      до цього писав файл на диск (`write_atomic`) ДО хешування — gate, доданий після цього
      виклику, "спрацював" би, але цикл, що провалює gate, усе одно вже встиг переписати
      `<id>.txt` зіпсованим текстом, отруївши той самий last-known-good fallback, який мав
      захищати. Виправлено порядком: хешувати й валідувати над `body` в пам'яті спершу, писати на
      диск лише при `Ok`. Дискримінуючий тест
      (`write_and_hash_blocking_does_not_poison_an_existing_last_known_good_file`) ловить саме цю
      регресію.
    - **Частина 2 (дельта-перевірка) — зроблено 2026-09-17, окремий план+advisor-цикл (той самий
      узор — advisor підтвердив розбиття на окремі задачі перед стартом, потім два раунди
      advisor-catch перед комітом).** Новий персистентний `<app-data>/blocklists/<id>.count` —
      простий текстовий файл з accepted-count останнього gate-схваленого успішного запису, той
      самий стиль простоти, що `.sha256`-сайдкари `topn_download`, записується атомарно через
      наявний `paths::write_atomic`; власний mtime файлу = момент останнього успішного циклу
      (той самий патерн, що `load_blocklist_bundles_from_disk` уже використовує для
      `last_updated`). Чиста `blocklist_download::delta_verdict(current, previous: Option<(usize,
      Duration)>)` володіє усіма рішеннями (floor/grace/множники) — той самий "clocks injected"
      розподіл, що `cache_persist_dto::to_json`/`reachability::next_probe_delay` вже
      застосовують, тож граничні тести не залежать від підробленого mtime на диску.
      - **Advisor-catch #1 (перед комітом): один глобальний відносний поріг некоректний одночасно
        на обох кінцях** — реальні виміряні розміри восьми джерел спани ~2500× (1 251 у
        `hagezi-hoster` до 3 181 194 у `hagezi-nrd`). Виправлено абсолютною підлогою
        `MIN_COUNT_BASELINE` = 5000 — нижче цього перевірка пропускається повністю (той самий
        "немає статистично значущої бази" принцип, що `candidates == 0` у ratio-перевірці);
        `hagezi-hoster`/`hagezi-dyndns` сьогодні не отримують дельта-захисту, це чесно
        задокументовано, не приховано.
      - **Advisor-catch #2: Контекст переоцінював, що ловить ця перевірка.** Дельта ловить лише
        **валовий об'ємний зсув** (заміна/обвал), НЕ ловить T-233's власну "найдешевшу й
        найреальнішу" атаку — домішку сотень-тисяч легітимних доменів у мільйонний список (1000
        доменів у `hagezi-nrd`'s 3.18M = 0.03% дельти, нижче будь-якого розумного порогу). Той
        клас і далі ловить лише canary-перевірка (частина 1), обмежена конкретними 12 доменами.
      - **Реалізує уточнення користувача про час від останнього успішного циклу через
        `DELTA_CHECK_GRACE_PERIOD` = 7 днів** (`elapsed > grace` → пропуск) — простежений сценарій
        (advisor-запит): `[blocklist_bundles]` вимкнено на місяць → перший цикл після ввімкнення
        бачить застарілий `<id>.count` → пропускає gate → пише свіжу базу → другий цикл порівнює
        нормально. Задоволено самою конструкцією `delta_verdict`, не окремим спецкейсом.
      - **Ціла арифметика замість `f64`-порогу** (`current.saturating_mul(5) < previous` замість
        `current < previous as f64 * 0.2`) — advisor-catch: уникає другого `#[allow(clippy::
        cast_precision_loss)]`, межа доказова з самого рядка через `saturating_mul`, не через
        "nrd лише 3.18M то ×5 не переповниться".
      - **Async-безпека (запит користувача mid-turn):** увесь новий I/O — усередині
        `write_and_hash_blocking`, яка вже й так виконується цілком на `spawn_blocking`-пулі
        (T-232) — жодного нового `.await`-виклику файлової системи з async-таска.
      - Записаний count-файл ІГНОРУЄ провал власного запису після успішного запису `<id>.txt`
        (`tracing::warn!`, не `Err` — інакше вже успішний цикл прочитався б як провалений).
      - **CI-урок цієї сесії, застосований тут:** `cargo doc --workspace --no-deps
        --document-private-items -j 1` тепер обов'язково локально перед пушем (минулого разу
        частину 1 пропустили цю команду й зловили CI-фейл на `docs`-джобі через зламане
        intra-doc посилання; тут два таких посилання спіймано й виправлено локально до пушу).
    - **Частина 3 (PSL-фільтр) — зроблено 2026-09-18, окремий план+advisor-цикл (той самий
      узор).** Перенесено вже наявний, вже перевірений `struct Psl` (`parse`/`registrable`) з
      `examples/curate_topn.rs` у новий бібліотечний модуль `src/public_suffix.rs` (`pub`) —
      разом із самим `public_suffix_list.dat` (`git mv` з `examples/` у `src/`, зберігає
      історію); `curate_topn.rs` тепер лише імпортує з крейту, жодного дубльованого коду.
      - **Advisor-catch (перед реалізацією): дослідження, не апріорна ICANN/PRIVATE-теорія.**
        Порахував точний збіг рядків усіх 8 реальних закешованих `<id>.txt`
        (`%LOCALAPPDATA%\dns-quorum-filter\blocklists`, ~4.1M рядків) проти ICANN- і
        PRIVATE-секцій PSL: **лише 4 ICANN-збіги на весь корпус** (`pl.ua` в adguard-dns-filter,
        `5g.in`/`6g.in`/`firm.in` в hagezi-tif), жоден не в hoster/dyndns; **468/1547 (30%!)
        рядків hagezi-dyndns і 8/1251 hagezi-hoster — навмисні bare PRIVATE-суфікси**
        (делегувальні простори — `duckdns.org`, `no-ip.org` тощо, сама мета існування цих двох
        джерел). Висновок: фільтрувати **лише ICANN-секцію**, лишити PRIVATE недоторканою —
        інакше PSL-фільтр гарантовано зламав би саме ці два джерела.
      - **Дизайн: `Psl` тепер тримає ДВА набори правил** — `rules` (обидві секції, як і раніше —
        `registrable()` не змінює поведінку) + новий `icann_rules` (ICANN-only підмножина, лише
        для нового `is_public_suffix`). Дискримінуючий тест на сам рефакторинг:
        `registrable("foo.github.io") == Some("foo.github.io")` (`github.io` — PRIVATE-правило;
        якщо `rules` колись помилково звузити до ICANN-only, лише цей тест впаде).
      - **Нова `Psl::is_public_suffix(&self, normalized: &str) -> bool`** — свідомо НЕ
        `registrable(..).is_none()`: дешевша (макс. 1 виділення на wildcard-пробу проти циклу
        `.join()`-ів по всіх позиціях — викликається на кожен рядок кожного з 8 джерел, до ~5.5M
        рядків/цикл) і звіряється лише проти `icann_rules`. `blocklist_download::push_normalized`
        відкидає запис, що сам є ICANN public suffix, **до** хешування; лічильник `candidates`
        інкрементується безумовно (як і для будь-якого рядка), тож PSL-відкидання все ж трохи
        зсуває ratio-gate — але вимірений реальний ефект (4 збіги на ~4.1M рядків) незначний
        проти `MIN_PARSE_SUCCESS_RATIO` = 0.5 (advisor-catch на закритті: перша версія
        doc-коментаря стверджувала, що це взагалі не впливає на ratio — виправлено).
      - **Impure-шар:** три викликачі `hash_source_body` (`write_and_hash_blocking`,
        `read_and_hash_blocking`, `load_blocklist_bundles_from_disk`) кожен парсить
        `Psl::parse(PSL_DAT)` заново — свідомо, не memoized (~16.5K рядків — мікросекунди проти
        вже наявного ratio/canary/delta-навантаження). Усі три вже або в `spawn_blocking`, або в
        синхронному не-async контексті — жодного нового `.await`-виклику з async-таска.
      - **Відомий scope gap, задокументований, не прихований:** `is_public_suffix` не звіряється
        проти PSL exception-правил (`!city.kawasaki.jp`) — напрямок помилки лише
        під-фільтрування (менше блок-покриття), ніколи over-blocking, тож прийнято без повного
        exception-обходу на hot path.
      - **Advisor-catch на закритті: одноразовий розрив `<id>.count`-бази при переході.** Вісім
        файлів на диску написані ДО частини 3 — їхні `<id>.count`-бази лічать нефільтрований
        `hashes.len()`. `read_and_hash_blocking`/`load_blocklist_bundles_from_disk` тепер
        застосовують PSL-фільтр при читанні (не grandfathered, на відміну від ratio/canary/delta
        — ті три застосовуються лише при записі), тож перший цикл ПІСЛЯ оновлення побачить
        `hashes.len()` менше на щонайбільше 4 записи, ніж стара база. Перевірено арифметично:
        зачеплені лише `adguard-dns-filter` (1 збіг з ~181K прийнятих) і `hagezi-tif` (3 збіги з
        ~2.47M) — обидва далеко за `MIN_COUNT_BASELINE`, а зсув на 1-3 записи не наближається ні
        до ×5 (`MAX_COUNT_GROWTH_MULTIPLIER`), ні до ÷5 (`MIN_COUNT_RETENTION_DIVISOR`) — розрив
        не здатний сам по собі спрацювати `delta_verdict`.
  - **Закрито 2026-09-18** — усі три сплановані частини (ratio+canary, delta, PSL) реалізовані,
    кожна своїм план+advisor-циклом. Єдиний залишок — **ін'єкція жмені легітимних доменів у
    великий список** (структурна межа всіх чотирьох гейтів разом, T-233's власний початковий
    threat model) — винесено в окрему беклог-задачу **T-234** (`## Поза фазами / бэклог`),
...
2026-09-19, plan-mode + advisor (три раунди advisor-catch на плані перед затвердженням, деталі
нижче).** Третій з 7 узгоджених батчів Фази 5 (`TASKS.md`'s "Батч-план узгоджено з користувачем
2026-09-18"). Бекенд (`cctld_block.rs`, `[cctld_block]` config, `POST /admin/cctld-block`,
`CctldBlockStatusView`) уже повністю зроблено Батчем 5.1 — цей батч лише `ui/*` +
`admin_ui.rs`-тести, жодних Rust pipeline/config-змін.

**Відкрите питання перед кодом (TASKS.md's власна вимога): ccTLD ≠ пряме передрукування GeoIP's
ISO 3166-1 alpha-2 код-набору.** Перевірено емпірично (Node, той самий рушій ICU/V8, що й
Chrome): `Intl.DisplayNames(locale, {type:'region'})` коректно резолвить `UK`→"United Kingdom"
(CLDR-аліас на `GB`) і `EU`→"European Union" без жодного винятку; `SU` резолвиться в "Russia" —
оманливо, `.su` (СРСР) досі окремий, живий простір, не синонім `.ru`. Рішення: `CCTLD_CODES =
GEOIP_COUNTRY_CODES.concat(["uk", "eu", "su"])` + один явний `if` для `su`, а не окрема мапа
винятків. `.tp` (East Timor, застарілий, замінений на `.tl`, вже в ISO-списку) свідомо не
доданий у каталог підказок — IANA вивела з експлуатації, немає живої причини пропонувати.

**Реалізація:**
- `main.js` — `COUNTRY_NAMES` (249-рядковий `{code: English name}`) замінено на
  `GEOIP_COUNTRY_CODES` (лише коди) + `regionLabel(code)` (кешується по `CURRENT_LOCALE`,
  інвалідується природно при зміні локалі — без окремого "скинути кеш" кроку). GeoIP-датаlist
  (`geoipCountryDatalist`) тепер перебудовується на кожен `renderGeoip()` (`populateGeoipCountryDatalist()`),
  не будується один раз при завантаженні модуля.
- Нова картка `#cctld-block-body` — UX-зразок `#rating-filter-body` (вільнотекстовий
  `role="combobox"`/`listbox`, чипи з `×`, dirty-state «Зберегти»), **не** GeoIP-датаlist. І
  рядок меню, і чіп показують код **і** назву разом (`ru — Росія`) — пряма вимога T-118, на
  відміну від rf-зразка (лише назва). `su` отримує хардкоджений український виняток
  ("Колишній СРСР (окремий простір від .ru)").
- **Попередження про грубість евристики — armed, не постійний банер** (Три Б, T-77's
  arm-on-add прецедент): арм відбувається на "Зберегти" лише коли новий набір **додає** код;
  чисте видалення постить одразу без арму. `resetArming()` скидається на будь-яку зміну
  `picked` (не лише на "Скасувати") — інакше видалення щойно доданого коду лишає підтвердження,
  чий текст описує стан, якого вже нема.
- **UI-можливість = серверна можливість:** `validate_cctld_code` приймає будь-який синтаксично
  валідний 2-літерний код, не лише каталог — `Enter`-хендлер має typed-code fallback (лоуеркейс
  перед `toggleCode`, `input.value` очищається **до** виклику, не після). Коди поза каталогом
  (наприклад вручну вписані в `resolver_config.toml`) лишаються видимими й видаляємими —
  `displayCctldCodes()`-еквівалент, той самий "ніколи не ховати мовчки" патерн, що rating-filter's
  `displayCodes()`.
- **CSS: dual-class замість дублювання під новим префіксом.** `.bl-*` (Батч 7.4) отримав власний
  префікс, бо був структурно іншим віджетом; тут комбобокс структурно ідентичний `.rf-*`-версії,
  тож замість дублювання ~60 рядків правил елементи прокидаються з **обома** класами
  (`class="rf-combo cc-combo"` тощо) — стиль успадковується безкоштовно від наявного `.rf-*`,
  `.cc-*` лишає розмітку чесно позначеною як ccTLD-блокова, без жодної правки `style.css`, окрім
  одного документаційного коментаря.
- `index.html` — `#cctld-block-body` усередині `<details>` «Розширені», одразу перед
  `#geoip-body`. `admin_ui.rs` — `ADVANCED_SECTION_IDS` + 9 нових тестів (та сама текстова-
  асершен-планка, що rating-filter/blocklist-bundles), включно з `!MAIN_JS.contains("COUNTRY_NAMES")`
  (завершена міграція, той самий прецедент, що `!MAIN_JS.contains("FIELD_HELP.")`).

**Advisor-catch'і на плані (три раунди, усі застосовані до коду):**
1. Попередження мало бути постійним у першому чернетковому плані — суперечило CLAUDE.md's
   власному "an always-on warning ≡ no warning" (T-77). Виправлено на armed-на-додаванні (вище).
2. Rf-зразок рендерить лише назву зони, T-118 вимагає код+назву разом — пропущено в першому
   чернетковому плані, виправлено явним рядком у §3.
3. Введений typed-код не лоуеркейсився й не чистив `input.value` — `RU`/`ru` розійшлись би в два
   чіпи, `pickedMatchesSaved()` ніколи не зійшовся б. Виправлено.
4. `refreshGeoip()`/`refreshCctldBlock()` мали додатись у `renderTranslatedCards()` **без**
   видалення наявного окремого модульного виклику `refreshGeoip();` — подвійний фетч на
   першому завантаженні. Перевірено читанням bootstrap-IIFE перед зміною (не припущенням);
   модульний виклик прибрано, лишився лише виклик з `renderTranslatedCards()`.
5. `resetArming()` мала викликатись лише з "Скасувати" — пропускала скидання на `toggleCode`/`×`.
   Виправлено (сценарій: арм → прибрати щойно доданий код → "Підтвердити" все одно постить крізь
   застарілий текст попередження).
6. CSS-дублювання під новим `.cc-*`-префіксом (за зразком `.bl-*`) було б **більшим** диффом, ніж
   розширення — `.bl-*` виправдав власний префікс структурною відмінністю (чекбокси замість ×),
   тут структура ідентична. Реалізовано ще простіше за пропозицію advisor'а (comma-селектор) —
   dual-class у самому JS, нуль правок `style.css` крім одного коментаря.
7. `CLAUDE.md` — свідомо не чіпається цим батчем: Documentation map власний розподіл (`UI-SPEC.md`
   — "screen inventory"/"коли екран змінюється", `CLAUDE.md` — "architecture/commands change")
   ставить нову картку в юрисдикцію `UI-SPEC.md`, а глобальне правило вимагає парного видалення
   на кожен допис у `CLAUDE.md` — дешевше не чіпати файл узагалі цим батчем.

**Порядок оголошень у `main.js` (баг, спійманий до коміту, не advisor'ом):** перший прохід
поставив секцію ccTLD-блоку (з `const CCTLD_CODES = GEOIP_COUNTRY_CODES.concat(...)`) **перед**
оголошенням `const GEOIP_COUNTRY_CODES = [...]` у файлі — `const` не хойстяться, це кинуло б
`ReferenceError` при завантаженні сторінки (temporal dead zone). Виловлено власним смоук-тестом
(`node vm.runInContext` з мінімальним DOM-стабом — реальний браузер не використано на цьому
кроці, лише перевірка порядку виконання модуля), не живим Chrome — виправлено переміщенням
`GEOIP_COUNTRY_CODES`/`regionLabel()` перед секцією ccTLD-блоку; повторний прогін підтвердив
`GEOIP_COUNTRY_CODES.length === 249`, `CCTLD_CODES.length === 252`, і коректні мітки для
`ru`/`su`/`uk`/`eu`.

**Diagram ground-truth ritual прогнано.** `diagrams/ui-navigation.md`'s вузол `Advanced` мав
рядок-заглушку "Ф5: ccTLD-блок — список заборонених TLD..." (написану наперед, до Батч 5.1/5.3,
Батч 5.1's власний запис вище явно залишив цей вузол незмінним) — оновлено на фактично зроблене
(combobox+чіпи, armed-попередження лише при додаванні). `UI-SPEC.md` §3.3/§3.6 — рядок ccTLD-
блоку оновлено з "текстове поле зі списком тегів" (стара чернетка) на реальну картку.
`diagrams/rating-filter.md` перевірено — не згадує ccTLD, не зачіпається.

**Живий смоук у Chrome (chrome-devtools MCP, scratch `%LOCALAPPDATA%`, порт 8443, self-signed
сертифікат прийнято вручну через "Перейти на сайт (небезпечно)") — браузер сам був на японській
мові (`navigator.language` цієї машини), випадковий, але цінний третій тест-локаль на додачу до
uk/en:** `#cctld-block-body` відрендерилась одразу перед `#geoip-body` в «Розширені»; пошук `ru`
показав `"RU — ロシア"` в меню; вибір + «Зберегти» на **додаванні** коректно заармив попередження
(`confirmRow.hidden === false`, текст "Скасувати"/"Підтвердити блокування"); підтвердження
реально постило `POST /admin/cctld-block [200]`, чіп пережив `F5`; видалення щойно збереженого
коду + «Зберегти» **не** заармило (`confirmRowHidden === true` протягом усього виклику,
чиста де-ризикована дія); typed-code fallback (`zz`, поза каталогом) додав чіп напряму через
`Enter` і коректно показав фолбек-мітку `"zz — 不明な地域"` (Intl.DisplayNames' власний "Unknown
Region", не голий код); GeoIP-датаlist показав `"IS — アイスランド"` тощо, відсортовані
за японською локалізованою назвою. Жодної консольної помилки (окрім преекзистуючого
`favicon.ico` 404, задокументованого ще в Батчі 5.2).

**Один реальний баг, виловлений саме цим живим смоуком, не advisor'ом і не жодним із 9 нових
тестів:** `CCTLD_CODES = GEOIP_COUNTRY_CODES.concat(["uk", "eu", "su"])` показував коди з
`GEOIP_COUNTRY_CODES` у верхньому регістрі (`RU`, успадковано від GeoIP-картки власної
конвенції), а три вручну дописані (`uk`/`eu`/`su`) — у нижньому — видима, ніде не заассерчена
неконсистентність у тій самій картці. Виправлено на `GEOIP_COUNTRY_CODES.map(code =>
code.toLowerCase()).concat([...])` — ccTLD-суфікс пишеться нижнім регістром за конвенцією
(`.ru`, не `.RU`), і сервер завжди повертає `blocked_codes` у нижньому регістрі
(`validate_cctld_code`), тож нижній регістр — правильна сторона, не GeoIP-картки. Тест
`cctld_codes_reconcile_the_documented_deltas_from_geoip` оновлено під нову форму виразу.

Повне верифікаційне зведення (після цього виправлення): `cargo build --workspace --locked`,
`cargo test --workspace --lib --bins --locked` (960, було 951 — 9 нових тестів), `cargo clippy
--workspace --all-targets -- -D warnings`, `cargo fmt --all -- --check`, `cargo test --test
...
**Батч 5.4, коміт 2 — `#timeout-config-body` (T-151), 2026-09-20.** Другий комміт Батчу 5.4.
Мігрує `renderTimeoutConfig()` на `t()`, заводить `warning.notPersisted` (спільний ключ для
9 родинних "не збережено на диск"-попереджень — наступні коміти лише перевикористовують) і
`common.helpAriaLabel`. 4 нові ключі × 37 локалей.

**Безкоштовний побічний ефект, не окрема робота:** `common.helpAriaLabel` замінив хардкоджену
`"Довідка"` усередині спільної DOM-функції `helpDetails()` (main.js, викликається з
`cardHeading()`) — це одразу перекладає aria-label кнопки довідки на **чотирьох** уже
змігрованих картках (`#overrides-body`, `#cache-config-body`, `#log-body`,
`#providers-body`), жодна з яких сама не редагувалась цим коміт, бо всі вони вже проходять
крізь цю саму функцію. Той самий шаблон `<div class="card-heading-row">...aria-label="Довідка"...`
у картках, що будують розмітку вручну через `innerHTML` (а не через `cardHeading()`), лишається
на свій власний майбутній комміт — вони просто перевикористають уже заведений ключ.
...
підтверджено відсутність кириличних літералів у змігрованому діапазоні (лише коментарі/символи
лишились). Server-side перевірка через scratch-інстанс (порт 8443, той самий підхід, що й
коміт 1): `.../admin/ui/i18n/ar.json` повернув усі 4 нові ключі арабською (42 ключі, збіг з
`en`); `.../admin/ui/main.js` підтвердив 4 нові `t()`-виклики й нуль входжень старих
українських літералів (`"Поведінка при збої"`, хардкоджений `aria-label="Довідка"`). Жива
перевірка перемикання мови в реальному DOM Chrome лишається відкритою (розширення недоступне
цієї сесії), як і в коміті 1.

---

**Батч 5.4, коміт 3 — `#overrides-body` (T-151), 2026-09-20.** Третій комміт. Мігрує
`renderOverrides()`/`renderOverridesError()`/`overrideListItem()` на `t()`. Заводить
`common.delete` (спільна кнопка "Видалити" — Explore-звіт знайшов її незалежно продубльованою
в `overrideListItem`/`geoipListItem`/`providerRow`; наступні два комміти лише перевикористовують).
10 нових ключів × 37 локалей.

**Уточнення до GLOSSARY.md-конвенції, не відхилення від неї:** `"Allowlist"`/`"Blocklist"` —
GLOSSARY.md вже мав правило "перекладати ідіоматично, не як фіксовані назви" (T-236, писане для
`fieldHelp.overrides`-речення); цей комміт застосовує те саме правило до заголовків `<h4>` над
списками (`overrides.allowlistHeading`/`overrides.blocklistHeading`, укр. "Дозволені
домени"/"Заблоковані домени") і до конфлікт-нотатки, а не лишає їх голими англійськими словами,
як вони фактично жили в коді дотепер.

**Перевірка:** `cargo test --workspace --lib --bins` (954, без регресій), `cargo clippy
--workspace --all-targets -- -D warnings`, `cargo fmt --all -- --check` — усі зелені. Грепом
підтверджено відсутність кириличних літералів у змігрованому діапазоні (лише коментарі
лишились). Живий Chrome-смок і server-side curl-перевірка цього разу пропущені для економії
часу — наступна планова живa перевірка (коли розширення Chrome стане доступним) покриє коміти
3-N разом, не кожен окремо.

---

**Батч 5.4, коміт 4 — `#cache-config-body` (T-151), 2026-09-20.** Четвертий комміт. Мігрує
`renderCacheConfig()`/`renderCacheConfigError()` + модульний const `CACHE_CONFIG_FIELDS` (5
міток полів) на `t()`. Перевикористовує вже заведені `warning.notPersisted`/`error.generic`. 10
нових ключів × 37 локалей (`TTL`/`BLOCK` лишені технічними термінами без перекладу, per
GLOSSARY.md).

**Перевірка (об'єднана для коммітів 3+4, per план — "після кожних кількох коммітів"):**
`cargo test --workspace --lib --bins` (954, без регресій), `cargo clippy --workspace
--all-targets -- -D warnings`, `cargo fmt --all -- --check` — усі зелені. Грепом підтверджено
відсутність кириличних літералів у обох змігрованих діапазонах. Server-side перевірка через
scratch-інстанс (порт 8443): `.../admin/ui/i18n/zh.json` повернув усі нові ключі коректною
китайською (62 ключі, збіг з `en`); `.../admin/ui/main.js` підтвердив нові `t()`-виклики.
...
**Батч 5.4, коміт 9 — `#filter-controls-body` (T-151), 2026-09-22.** Дев'ятий комміт. Мігрує
`CATEGORY_META` (name/sub → `nameKey`/`subKey`), `renderFilterControls()`, `flipCategory()`/
`flipAllCategories()`. 15 нових ключів × 37 локалей, з них **два перші плюрал-ключі поза
`zoneDomainCount`** — `filterControls.fanoutPartiesClause`/`fanoutChecksClause`.

**`pluralUk()` видалено.** Ручний українсько-специфічний one/few/many-плюралізатор у реченні
фан-ауту (найскладніша інтерполяція файлу: 3 плюрал-слоти в одному реченні) замінено двома
`tPlural()`-викликами — кожен дає повну клаузу "дієслово + {n} + іменник" з власними формами на
локаль, зовнішнє речення збирається через `t("filterControls.fanoutSummary", {partiesClause,
checksClause})`. Рівно один `{n}` на плюрал-ключ (`tPlural()` підставляє лише перше входження).

**Тест плюрал-форм узагальнено (advisor-catch п.10 плану, виконано саме тут):**
`every_locale_zone_domain_count_has_its_measured_plural_categories` → `every_locale_plural_key_
has_its_measured_plural_categories` — тепер обходить кожен object-valued top-level ключ словника,
не лише `zoneDomainCount` за іменем. Без цього нові ключі пройшли б повз тест із неправильною
формою категорій, а `tPlural()` мовчки деградував би до `.other`.

**Очікувана поломка тесту:** `cctld_block_surfaces_a_failed_persist` рахував сирий підрядок "не
переживе перезапуск сервісу" (`>= 3`) — міграція карток поступово зводила його до нуля (тепер 2
залишкових, до коммітів 10-14). Перероблено на підрахунок `t("warning.notPersisted")`-викликів
+ перевірку тексту в `en.json`.

**Перевірка:** `cargo test --workspace --lib --bins` (954), `cargo clippy --workspace
--all-targets -- -D warnings`, `cargo fmt --all -- --check` — зелені. Грепом підтверджено
...
**Батч 5.4, коміт 13 — `#blocklist-bundles-body` (T-151), 2026-09-22.** Тринадцятий комміт.
Мігрує `BLOCKLIST_SOURCE_META.desc` (→ `descKey`; `name` — назви продуктів, лишені як є),
`BLOCKLIST_CLUSTER_LABELS` (→ `BLOCKLIST_CLUSTER_LABEL_KEYS`), `blocklistRelativeTime()`,
`renderBlocklistBundles()`/`renderBlocklistBundlesError()`. 28 нових ключів × 37 локалей, з них
**три плюрал-ключі**: `blocklist.activeCount`, `blocklist.updated.hours`, `blocklist.updated.days`.

**`blocklistRelativeTime()` — друга і остання рукописна українська плюралізація (п.3 плану)
переведена на `tPlural()`**; `pluralUk()` пішов у коміті 9. Речення "Активно, N джерел" — окремий
плюрал-ключ з відмінюванням іменника по категоріях кожної локалі. Для `hours`/`days` використано
скорочені одиниці ("год"/"дн", "h"/"d", "ч"/"д"…), тож одна форма повторена по всіх категоріях
локалі — відмінювання іменника там не виникає. Це свідомий компроміс: повні слова ("годин"/"днів")
вимагали б окремої форми на кожну категорію × 37 локалей без виміряної користі для рядка-підказки.

**Перевірка:** `cargo test --workspace --lib --bins` (955), clippy, fmt — зелені; узагальнений
тест плюрал-категорій (коміт 9) вперше пройшов по трьох новых ключах усіх 37 локалей одразу —
жодного ручного звіряння таблиці категорій не знадобилось. Кириличних літералів не лишилось
(лише `—`/`›`). Живий Chrome-смок недоступний; **не перевірено на реальних числах** (n=1, 2, 5, 21)
у браузері, лише структурно.

---

**Батч 5.4, коміт 14 — footer (T-151), 2026-09-22; ЗАКРИВАЄ Батч 5.4.** Чотирнадцятий і останній
комміт. Дві атрибуційні `<p>` (`footer.geoipAttribution`/`footer.cruxAttribution`) — ліцензійні
посилання з юридичною вимогою (CC BY 4.0: DB-IP з точним текстом якоря "IP Geolocation by DB-IP",
CrUX). 2 нові ключі × 37 локалей.

**Рішення (п.5 плану, уточнене):** href-и й тексти якорів — **константи в `main.js`**
(`FOOTER_LINK_VARS`), не частина перекладу; словник несе прозу з токенами `{sapics}`/`{pddl}`/
`{dbip}`/`{ccby}`/`{maxmind}`/`{crux}`/`{psl}`/`{mpl}`. Переклад може переставити слова навколо
токена, але не змінити чи загубити саме посилання. **Новий тест**
`every_locale_footer_attribution_keeps_every_required_link_token` перевіряє, що кожна з 37 локалей
зберегла кожен токен, і що `main.js` володіє обов'язковим посиланням — інакше локаль, що загубила
токен, мовчки втратила б юридично обов'язкову атрибуцію, а український fallback у HTML сховав би це.
Перший `<p>` футера (Apache License 2.0) лишено статичним — там немає прози для перекладу.

**Підсумок Батчу 5.4:** 14 коммітів; `cargo test --workspace --lib --bins` 954 → 956 (+2 тести:
`data-i18n` ключі й footer-токени; плюрал-тест узагальнено, 3 rust-тести переведено з raw-літералів
на ключі); clippy/fmt зелені на кожному коміті. Шість "спіймано по дорозі" багів/дір: порожній hero
у `renderError()` (регрес Батчу 5.2); double-fetch `refreshMaxmind()`; відсутній
`refreshMaxmind()` у `renderTranslatedCards()`; init-time label кнопки browser-setup до готовності
словника; ризик "UI бреше про наслідок кліку" на кнопці uninstall при живому перемиканні мови;
розходження формулювання "не збережено" в `renderMaxmind()`.

**Що лишилось відкритим (чесно):** жоден коміт цього Батчу не перевірено в реальному браузері (див.
TASKS.md); переклади 35 локалей — машинні, без нативного рев'ю (`KNOWN-LIMITATIONS.md`, вже
зафіксовано для T-236, поширюється на всі нові ключі). Батч 5.5 (трей) має звірити назву пункту меню
«Повністю видалити» з `danger.uninstallButton` (див. коміт 11).

- [x] T-241 — **Заведено й закрито 2026-09-22, знайдено під час T-238 (смоук-тест v0.5.0, реальний
  встановлений MSIX, не unit-тест).** Два незалежні знайдені й виправлені дефекти в `dnsqb-tray`:
  1. **Task Manager mojibake.** `build/win_resource.rs` пише згенерований `.rc`-файл через
     `fs::write` — сирі байти UTF-8, без BOM. `rc.exe`/`windres` без BOM читають `.rc` системною
     ANSI-кодовою сторінкою незалежно від того, що `StringFileInfo`-блок усередині сам оголошує
     `"040904b0"` (codepage 1200/Unicode) — це стосується скомпільованого ресурсу, не вхідного
     тексту. Наслідок: em dash `—` (3 UTF-8 байти) у трьох `FileDescription`-рядках (`build.rs`
     кожного з трьох крейтів) читався як CP1252 → `â€"` у Task Manager/провідникові властивостей
     файлу. Підтверджено на вже випущеному `v0.5.0`-бінарнику
     (`(Get-Item ...).VersionInfo.FileDescription` → `DNS Quorum Filter â€" tray icon`), виправлено
     заміною на звичайний ASCII-дефіс у всіх трьох (найбезпечніший фікс — не залежить від того, який
     компілятор ресурсів зрештою використовується, `rc.exe` (MSVC CI) чи `windres`
     (windows-gnu, цей dev-бокс) — обидва однаково коректно читають чистий ASCII без BOM).
  2. **Нативні rfd-діалоги трея (10 місць) — старий `MessageBoxW`-стиль, невидима марка застосунку.**
     Запит користувача: усі GUI-повідомлення (підтвердження встановлення сертифіката, «Повністю
     видалити», пауза/вихід, «Про програму» тощо) мали в заголовку явно «DNS Quorum Filter» +
     номер версії, і мали виглядати "по-сучасному" (Windows 10/11), а не як старий
     `MessageBoxW`. Зроблено: (a) один спільний `fn dialog_title(action) -> String` (замінив 10
     незалежних літералів, серед них один неконсистентний `"dns-quorum-filter"` на About-діалозі);
     (b) один спільний `fn confirm(action, description, level) -> bool` — той самий
     Yes/No/title/buttons шаблон, що повторювався в 7 функціях (`confirm_install_cert` … `confirm_quit`
     + онбординг-привітання), винесений після прямого запитання користувача "чи можна це в одне
     місце" — залишає названі функції з їхніми власними doc-коментарями (WHY), не заміняє на один
     загальний виклик просто з `match`-гілки; (c) `rfd`'s `common-controls-v6` feature (вже наявний
     `windows-sys` у дереві transitively через `rfd`, жодної нової залежності) — вмикає
     `TaskDialogIndirect` замість класичного `MessageBoxW`; (d) **емпірично встановлено, що фіча сама
     по собі не діє** — потрібен ще ComCtl32 v6 маніфест, вбудований у сам `.exe`
     (`build/dnsqb-tray.manifest`, `RT_MANIFEST` ід 1, лише для `dnsqb-tray` — `dnsqb-service`/
     `dnsqb-watcher` headless, без діалогів). **Advisor-catch на власному ж скретч-прогоні (не advisor,
     сесія сама зловила через реальний запуск):** перша версія маніфесту з `<dpiAwareness>
     PerMonitorV2</dpiAwareness>` під спільним `xmlns="…/2005/WindowsSettings"` на батьківському
     `<windowsSettings>` **ламала запуск застосунку взагалі** — `Application`-подія Windows,
     джерело `SideBySide`, id 79: "Activation context generation failed ... dpiAwareness is not
     registered" (PerMonitorV2 визначено лише в схемі 2016 року, не 2005). Виправлено: `dpiAwareness`
     і `dpiAware` кожен з власним явним `xmlns` (2016 і 2005-fallback відповідно), не успадкованим
     від батька. Перевірено на реальній MSVC-збірці (окремий rustup-тулчейн `1.98.0-x86_64-pc-windows-msvc`,
     встановлений саме для цього — dev-бокс сам по собі windows-gnu): (1) `mt.exe` витяг маніфест з
     готового `.exe`, підтвердив вміст; (2) ізольований скретч-крейт з тим самим маніфестом+фічею
     справді падав з "side-by-side configuration is incorrect" до фіксу і запускався після; (3)
     `PrintWindow` на реальному вікні підтвердив: заголовок `"DNS Quorum Filter 0.6.0 — Про
     програму"` (з номером версії), сучасний `TaskDialogIndirect`-рендер.
  **Локальний windows-gnu dev-бокс, окреме застереження (не стосується випущеного бінарника):**
  додавання цього ж маніфесту спричиняє `ld.exe`-попередження `.rsrc merge failure: multiple
  non-default manifests` при збірці на активному `stable-x86_64-pc-windows-gnu` тулчейні — MinGW
  CRT сам лінкує дефолтний маніфест, і другий (мій) конфліктує. **Підтверджено, що це суто
  windows-gnu-артефакт: та сама збірка на MSVC (`+1.98.0-x86_64-pc-windows-msvc`, тому самому
  таргету, що й CI/`release.yml`) — жодного попередження.** Не потребує фіксу (не впливає на
  жоден реальний build target цього проєкту), лишити нотатку тут на майбутнє, якщо хтось знову
  побачить це попередження локально й почне його розслідувати з нуля.

  **Продовження, той самий день, той самий принцип поширений на `/admin/ui`:** запит користувача —
  показати версію застосунку і на веб-сторінці, не лише в трей-діалогах. Новий
  `AdminStatusResponse.app_version` (`ADMIN_DTO_SCHEMA_VERSION` 5→6, `#[serde(default)]`, обидва
  `dispatch.rs`-білдери + фікстура `dnsqb-tray/src/status.rs` — CLAUDE.md-конвенція "кожне нове
  поле = бамп константи + serde(default)"), `main.js::renderAppTitle()` встановлює
  `#app-title` з `status.app_version` щоцикл 2с-полінгу; порожнє значення (перед-T-241 сервіс,
  чи невдалий fetch) лишає заголовок без версії — ніколи не вигаданий номер. Тест
  `main_js_renders_the_app_title_from_status_app_version`.

**Батч 5.5 — i18n трею (T-151), 2026-09-22, plan-mode + advisor (2 раунди: до коду — вибір
архітектури; після інфра-кроку — 3 catch'і).** Шостий з 7 узгоджених батчів Фази 5. `dnsqb-tray`
лишався повністю українським — усі пункти меню, `rfd`-діалоги, About, звіт видалення, tooltip.

- Advisor-catch #1 (перед кодом): `cargo test -p dnsqb-tray --lib` — no-op, `dnsqb-tray` це
  `[[bin]]`-лише крейт, потрібно `--bins` (CLAUDE.md вже документує цю пастку — проєкт уже раз на
  ній горів, `browser.rs`'s тест ніколи не бігав у CI).
- Advisor-catch #2: локаль — **injected `&str`-параметр** (`t(locale, key)`,
  `TrayStatus::tooltip(&self, locale)`), ніколи не з процес-глобального стану — інакше кожен
  наявний Ukrainian-substring-тест у `status.rs` став би залежним від OS-локалі машини, що
  запускає набір, той самий клас "local-green ≠ CI-green", що вже вкусив (T-50, T-241). Тести
  пінять `"uk"` явно.
- Advisor-catch #3: постійні Rust-тести keyset-паритету/валідного JSON у `i18n.rs` (дзеркало
  `admin_ui.rs`'s власних), не лише одноразовий скриптовий чек.
- **Архітектурне рішення: окреме файл-дерево** `crates/dnsqb-tray/i18n/*.json` (37 файлів), НЕ
  спільне з `crates/dnsqb-service/ui/i18n/*.json` — трей (нативні `rfd`-діалоги) і веб-UI різні
  домени, той самий принцип "незалежно типізовані списки локалей", що вже застосований між
  `admin_ui.rs` і `dispatch.rs`. Новий `crates/dnsqb-tray/src/i18n.rs` — той самий
  `macro_rules! i18n_dicts!`-патерн; `t()`/`t_args()` (просте `{name}`-заміщення, без кешування —
  переробляти JSON щоразу дешевше, ніж кеш-інвалідація для людського темпу викликів);
  `detect_locale()` — калька `main.js`'s `resolveLocale`/`detectLocale` (спліт по `-`, lowercase,
  точний збіг, інакше `"en"` — жодного ручного перемикача в треї, лише авто-визначення з ОС).
- **Жодного CLDR-плюрал-крейта** — 2 справді число-залежні tooltip-рядки (старий
  український-лише хардкод "N запит(ів)") переформульовано в plural-invariant "підпис: N" формат
  (`tooltip.noActiveProviderTemplate`/`filteringBaseTemplate`); третій кандидат
  (`degradedSuffixTemplate`, "X/Y останніх запитів мали тайм-аут") виявився вже
  partitive-invariant фразою при перевірці, переформулювання не знадобилось.
- Повна міграція `main.rs`/`status.rs`: кожен пункт меню, кожен `rfd`-діалог (заголовки/описи/
  About/звіт видалення), `trust_store_outcome_uk` узагальнено в `trust_store_outcome_text`.
  2 структурні clippy-фікси (не `#[allow]`): `main()` виніс `build_tray_icon` (line-count lint),
  `refresh_tray`'s 3 "last observed" поля згруповано в `TrayRefreshState` (arg-count lint).
- **Bulk-переклад на решту 35 локалей**: той самий MT-tier, що решта проєкту (самоперевірено
  структурно, не native-review). Фраза «бульбашка» рейтинг-фільтра й роздільник короткого речення
  (em/en-dash чи двокрапка залежно від локалі) взяті **напряму з уже вивірених веб-UI словників**
  (`rating.badge.active`, `hero.NO_PROVIDERS.detail`) для консистентності між застосунком, а не
  складені окремо — прийом вартий повторного використання, коли інша поверхня матиме той самий
  переклад.
- Документація: `GLOSSARY.md` (новий абзац — трей має власний паралельний 37-локальний словник,
  ті самі правила, інші файли), `SECURITY.md` (новий рядок для `sys-locale` 0.3.2, нуль подальших
  крейтів у власному дереві на цільову платформу). Повний локальний гейт зелений після кожного з
  2 комітів (`f3438fd`, `88d16ba`) — 962→968 тестів, fmt/clippy/doc/doctest/audit/deny. CI/CodeQL
  підтверджено зеленими, 0 відкритих алертів.

**Батч 5.6 — CLI `--help` для трьох бінарників (T-235), 2026-09-23, plan-mode + advisor (2
раунди: до дизайну — три зауваги; після — жодних дальших).** Сьомий, завершує узгоджений
7-батчевий план Фази 5. Жоден бінарник не парсив аргументи командного рядка взагалі.

- **Архітектурне ускладнення з TASKS.md вирішено емпіричною перевіркою, не припущенням.**
  `println!`/`eprintln!` у `windows_subsystem="windows"`-білді: (1) коректно виводяться, коли
  процес запущений із наявного термінала (PowerShell, Git Bash — успадковані дескриптори);
  (2) **не падають і не панікують**, коли запущені без жодної консолі — перевірено через
  справжній `ShellExecute`-запуск (`UseShellExecute=$true`, .NET `ProcessStartInfo`, точний
  еквівалент подвійного кліку в Провіднику) з `panic::catch_unwind` навколо друку, результат
  записано у файл: `exit=0`, жодної паніки ні на stdout, ні на stderr. **Висновок: жодного
  `AttachConsole`/`MessageBoxW`/unsafe-коду не потрібно** — просте `println!` безпечне в обох
  випадках.
- Advisor-catch (перед фіналізацією плану): перший тест (`Start-Process -Wait`) міг мати
  неявно виділену консоль для GUI-subsystem дочірнього процесу — недостатньо дискримінуючий.
  Повторено зі справжнім `ShellExecute`-шляхом (`UseShellExecute=true`) — той самий висновок,
  тепер надійно підтверджений.
- Advisor-catch (§7.1 #7-межа): новий спільний модуль перевірено — не додає жодного `pub`
  доступу до оркестрації/`run()`, `dnsqb-watcher` і так лишається неспроможним її викликати.
- **Рішення: спільний `dnsqb-service::cli_help`**, не окреме дерево на бінарник (як у Батчу
  5.5) — **свідоме, задокументоване відхилення** від того прецеденту: CLI-довідка всіх трьох
  бінарників **є** одним доменом (той самий короткий текст-конверт, різниться лише опис на
  бінарник), на відміну від трей-діалогів vs. веб-UI, які справді різні продукти.
  `pub fn help_text(binary: Binary) -> String` — чиста функція форматування рядків.
- `--help`/`-h`/`/?` перевіряється найпершою дією кожного `main()`, до `app_data_dir`/
  `init_logging`/захоплення single-instance-локу — інакше прапорець непридатний саме тоді, коли
  хтось спробує його набрати (застосунок уже запущений).
- **Обсяг: усі три бінарники в одному проході** (TASKS.md називав мінімумом лише
  `dnsqb-watcher`) — розширення тривіальне після появи модуля.
- `sys-locale` (вже вибрана й провітчена Батчем 5.5) промотовано в пряму залежність і
  `dnsqb-service`. **Користувач підтвердив усі 37 локалей одразу** (`AskUserQuestion`), попри
  малий розмір словника (5 ключів) — той самий принцип, що вже застосований до веб-UI/трею.
  2 коміти: інфра+3×`main.rs`+uk/en (`8a5ae2f`), потім bulk-переклад на 35 локалей — той самий
  Python-скрипт-патерн, структурні asserts + постійні Rust-тести keyset-паритету. Повний
  локальний гейт зелений: 968 тестів, fmt/clippy/doc/doctest/audit/deny. Живий смок: усі три
  зібрані бінарники з `--help` у PowerShell — текст показується, процес виходить одразу, без
  жодного побічного ефекту (жодного app-data, жодного локу).

**T-240 — фікс регресії Батчу 5.4: форма «Додати власний DoH-провайдер» не перекладалась на
живому `setLocale()`, 2026-09-23.** Не частина 7-батчевого плану Фази 5 (заведено під час T-238),
малий точковий фікс — plan-mode не потрібен.

- Причина (діагностовано в самому тикеті): `customProviderForm()` кешує весь DOM-вузол (T-47,
  щоб перемикання/додавання іншого провайдера не стирало недописаний ввід користувача), і
  `renderTranslatedCards()` лише повторно `appendChild`-ить той самий вузол — жоден рендер-шлях
  ніколи не перечитував `t()` на вже існуючих полях.
- Фікс: нова `retranslateCustomProviderForm()` — оновлює `placeholder`/`aria-label` на трьох
  input'ах, `aria-label` + текст `<option>` на обох select'ах (через `opt.value`, не index-у
  паралельний масив), заголовок і текст кнопки; **не чіпає жодного `.value`** — недописаний ввід
  переживає перемикання мови. Викликається з `renderTranslatedCards()` одразу після
  `refreshProviders()`. Друга знахідка того самого тикета (той самий кешований вузол застигав і
  на `aria-label`, не лише на видимому тексті) закрита в тому самому фіксі, не окремим проходом.
- Регресійні тести (`admin_ui.rs`): `retranslateCustomProviderForm()` додано до переліку
  обов'язкових викликів у `cctld_block_and_geoip_are_reachable_only_from_render_translated_cards`;
  нова `custom_provider_form_is_retranslated_without_rebuilding_or_touching_value` — асертує
  відсутність `createElement` (не перебудовує вузол) і відсутність запису в `idInput.value`/
  `urlInput.value`/`nameInput.value` (читання `opt.value` для select'ів — окремий, дозволений
  випадок), плюс наявність усіх 5 `aria-label`-рефрешів. Жодних нових ключів словника — увесь фікс
  повторно використовує вже наявні (`providers.*`, `overrides.addButton`).
- Повний локальний гейт зелений: 969+64+3 = 1036 тестів (0 falied), fmt/clippy чисті.
  **Не перевірено вживу в браузері** цього проходу — спроба зі scratch-інстансом
  (`LOCALAPPDATA` перенаправлено на тимчасову теку, окремий порт 18443) впала на нативному
  Windows-діалозі підтвердження довіри сертифіката (`certutil -addstore -user Root`), який
  недоступний для автоматизації ні через Chrome DevTools (interstitial-сторінка блокує
  attach), ні через PowerShell/Bash (модальний GUI-діалог). Побічний ефект спроби: `taskkill
  /IM dnsqb-service.exe` без PID зачепив і реальний встановлений інстанс (той самий образ) двічі
  — обидва рази `dnsqb-watcher` коректно й автоматично перезапустив його (лог:
  `respawned dnsqb-service` 22:26:08 і 22:29:41), підтверджено живим на порту 8443 після. Лишається
  відкритим до наступної сесії з ручним кліком на діалозі, чи до T-238-подібного проходу.
