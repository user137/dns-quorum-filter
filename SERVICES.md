...
- **Перевипустити сертифікат** (T-69) — `dnsqb_service::rotate_certificate`: згенерувати новий
  ...
  `cert.pem`/`key.pem` → встановити новий. Порядок "очистити перед записом" вимушений спільним
  CommonName. Новий сертифікат діє **лише після ручного перезапуску `dnsqb-service`** (процес
  тримає TLS-конфіг від старту); до перезапуску tooltip показує `Unreachable`, хоча фільтрація
  працює. За нативним confirm-діалогом, що називає це наперед.
