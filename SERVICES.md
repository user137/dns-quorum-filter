...
poll-лупом — `certutil` блокує). Seed `true` (червоний лише коли `certutil` *довів* недовіру;
`cert.pem` відсутній = «невідомо», лишається попереднє). Каденс — бекоф `15 → 60 → 300` с поки
`!trusted`, `300` с коли `trusted`; cert-пункти меню (`Встановити`/`Видалити`/`Перевипустити`/
`Повністю видалити`) викликають `request_recheck()` після `certutil`-мутації.
