...
6. Запускає **фоновий reachability-проб-таск** (T-152, Батч 3.4) — раз на ~30 s (частіше поки
   ...
   Apple); стан `OFFLINE` — лише коли **всі** впали. Публікується на `AppState`, читається
   резолвером (офлайн → миттєвий `SERVFAIL` замість повного per-query fan-out) і `GET /admin/status.
   network`. Власний `reqwest::Client`, **не** підключений до `/health` чи watchdog-каналів (обрив
