...
4. Приймає з'єднання, термінує TLS (`rustls`), і на кожен DoH GET/POST-запит прогонює конвеєр
   allowlist → blocklist → cache → **рейтинговий фільтр (крок 5, T-124, якщо `[rating_filter]`
   увімкнений)** → quorum (SPEC.md §5.3) через `dispatch::resolve_doh_request`.
5. Завантажує вже наявну `GeoIP`-базу (`geoip.mmdb` у тому ж `%LOCALAPPDATA%`-каталозі), якщо
   попередній запуск її залишив, і паралельно запускає фоновий апдейтер (T-75, T-226(б)) —
   перевіряє джерело (`user-country` за замовчуванням від 2026-09-12, або MaxMind GeoLite2, якщо
   оператор задав креденшели; DB-IP Lite лишається в коді як шлях відкату, але жодне конфіг-поле
   чи маршрут не обирає його сьогодні) раз на 24 години (спочатку одразу при старті), TLS +
   `.sha256`/`.sha1`-звірка (для `user-country` — підтверджено реально існуючий сайдкар; для
   DB-IP/MaxMind — опортуністично) + gzip CRC32/структурна валідація перед атомарною заміною
   файлу. Відсутність бази при першому запуску — не помилка, `GeoIP`-фільтр (T-76, ще не
   підключений у конвеєр) просто без ефекту, доки перший апдейт не завершиться успішно.
...
завершується асинхронно — `orchestrate::serve_until_shutdown`'s accept-луп (T-210: перенесено
з `main.rs`, яке відтоді 3-рядковий шим) припиняє приймати нові з'єднання
...
- **`/admin/ui`** — `#protection-hero` дістав cert-гілку (T-188): `GET /admin/cert-status`
  ...
  серверно). Картка «Підключення браузера» (T-189) має блоки кроків per-браузер (Chromium /
  Firefox / інший) — усі в статичному HTML, `main.js` розкриває один за `navigator.userAgent`
  (+`navigator.brave.isBrave()`); кожен рядок налаштувань браузера з кнопкою «Копіювати».
  Автоматичне прописування DoH у браузер — поза обсягом (T-99 / T-134).
...
меню (`Встановити`/`Видалити`/`Перевипустити`) викликають `request_recheck()`
