...
| `sys-locale` 0.3.2 (T-151 Батч 5.5, `dnsqb-tray`) | Sole caller: `i18n::detect_locale` — the tray's own OS-display-language auto-detect (no manual switcher exists in the tray). This is exactly the signal T-227's `winreg` row above rejected it for (UI display language, not region) — the opposite need here, so that earlier rejection doesn't apply; picking it over hand-rolling `GetUserDefaultLocaleName` avoids a second `unsafe extern "system"` FFI surface in a `forbid(unsafe_code)` crate | Zero further crates in its own dependency tree on this target (`cargo tree -p sys-locale` prints only itself) — no new duplicate-version or license-allowlist entries. Read-only OS query, one `Option<String>` result; `detect_locale`'s own post-processing (split/lowercase/lookup) is pure and has no fallible external call of its own to fail loudly on |
