...
- **Opt-in encrypted on-disk stores (T-146/T-97/Батч 4.5 T-108/T-138).** Four files, all off by
  default, all `XChaCha20Poly1305` via `encrypted_file.rs`, all `%LOCALAPPDATA%\dns-quorum-filter`:
  `query-log.enc` (browsing history) / `cache.enc` (which domains resolved, `Allow` only) /
  `zone-removals.enc` (the T-108 lazy-hygiene overlay — which registrables quorum already
  blocked) share one `persistence-key` Credential Manager entry (`FileKind` in the AEAD header
  keeps them distinct); `personal-zone.enc` (which sites *this specific user* visits often/
  regularly, T-138) uses its own, deliberately **separate** `personal-zone-key` entry — a higher
  privacy tier than the other three, so a compromised shared key can't also unlock the
  per-user browsing-frequency profile. No admin route/toggle for any of the four (hand-edit
  `resolver_config.toml` only) — each gets a passive `/admin/ui` indicator while its flag is on.
  Same orphan-file handling for all four: a missing key with a file present renames it aside
  (`.orphaned-<unix_ts>`) rather than overwriting or attempting recovery.
...
| `aws-lc-sys` 0.44.0 (← `aws-lc-rs` 1.18.0 ← `rcgen` + `rustls` + `rustls-webpki`) | Not a chosen crate — the transitive C-crypto backend behind the `aws_lc_rs` feature picked for `rcgen`/`rustls` (over `ring`, to avoid a second crypto impl in the binary). Links into all three `.exe`s. `ring` is **not** in the ship graph (`cargo tree -e no-dev --target x86_64-pc-windows-msvc -i ring` → nothing) | **The largest `unsafe` C surface in the shipped binary** (vendored AWS-LC via C + generated bindings). Accepted: it is the crypto core `rustls` has no pure-Rust production alternative for, first-party `#![forbid(unsafe_code)]` is intact, and the alternative (`ring`) is the same class of risk. Vetted at each `Cargo.lock` bump like any other dep |
...
| `softbuffer` 0.4.8 (T-229, `default-features = false`) | Paints the browser-nudge popup's pre-rendered static image onto a second `tao` window created on the same event loop — the only alternative that avoided this was a Windows balloon tip, rejected because `tray-icon` 0.21 doesn't expose the private `uID` its own icon registered under, so attaching a balloon to it needs a second `Shell_NotifyIcon` registration (a new dependency either way, plus either hand-rolled `unsafe` FFI or a heavier GUI-toolkit crate). Its own default features (`kms`/`x11`/`wayland` + dlopen variants) are Linux-only windowing backends, irrelevant on this Windows-only target, hence disabled | No `unsafe` in this crate's own code — `softbuffer`'s public API is fully safe; the `SharedWindow` newtype in `nudge_popup.rs` only delegates already-safe `HasWindowHandle`/`HasDisplayHandle` trait methods, `forbid(unsafe_code)` intact. Pulled `bytemuck`/`objc2-io-surface`/`objc2-quartz-core` into `Cargo.lock` (its disabled `kms`/macOS features) — confirmed **lockfile-only, never compiled for this target** (`cargo tree -e no-dev --target x86_64-pc-windows-msvc -i <crate>` → empty for all three; only `softbuffer` itself appears), same `keyring` row's non-Windows-store precedent, not a new live liability |
...
| `proptest` 1.11.0 (dev) | `default-features = false` excludes `fork`/`timeout` — no process-spawning transitive deps | dev-only; the real property tests: `overrides::parse_pattern`, `wire::decode_wire_message`, a fuzz pass over `/admin/config` POST bodies, and the `ROUTES`-driven `serve_never_panics_on_arbitrary_input_for_any_documented_route` (every route not in `FUZZ_EXCLUDED_ROUTES`, `/dns-query` POST included) |
...
| `chacha20poly1305` 0.11.0 (+`aead`/`poly1305`/`universal-hash`/`chacha20`/`cipher`) | Pure Rust, no C toolchain — chosen over promoting `aws-lc-rs` (user decision, 2026-09-03). Sole use: `encrypted_file::{seal,open}` for the four opt-in encrypted stores — query log, cache, the T-108 zone-removal overlay, and (Батч 4.5) the personal learned zone | SIMD `unsafe` contained in `chacha20`/`poly1305`, `forbid(unsafe_code)` intact. `chacha20` was pinned off a yanked 0.10.1 to 0.10.2 via `cargo update` — a real fixed issue, not hypothetical. |
...
| `embed-resource` 3.0.11 (**build-dep**; +`vswhom`/`vswhom-sys`/`winreg` on the msvc target, `memchr` on gnu) | T-177 — embeds the app icon + `VERSIONINFO` into the three `.exe`s so they are identifiable in Task Manager, not bare names. Chosen over `winresource`: it emits `cargo:rustc-link-arg-bins` (per-executable, non-propagating), whereas `winresource` on the gnu toolchain emits a `rustc-link-lib` that leaks the `dnsqb-service` resource into the tray/watcher binaries and collides with theirs | Runs only at build time, never linked into a shipped binary. `unsafe` FFI (`vswhom-sys` — locates the Windows SDK's `rc.exe`) executes on the build host only; `#![forbid(unsafe_code)]` in every shipped crate is intact. Each `build.rs` writes a generated `.rc` to `OUT_DIR` and panics on a `Failed` compile — a resource-compiler-less contributor build degrades to a `cargo:warning`, CI (with `rc.exe`) produces the real thing |
| `winreg` 0.55.0 (T-227, `[target.'cfg(windows)'.dependencies]`, `dnsqb-service`) | Sole caller: `install_region::detect_system_region`, reading `HKCU\Control Panel\International\Geo`'s `Name` value (confirmed present and holding a 2-letter ISO code on this dev box) to suggest a matching rating-filter zone. Chosen over calling `GetUserDefaultGeoName` directly via the `windows` crate — that function is `unsafe fn` and this project's crates carry `#![forbid(unsafe_code)]`, which cannot be locally `#[allow]`-ed around; `sys-locale` was also considered and rejected — it wraps `GetUserPreferredUILanguages` (UI display language), not the region setting, so a user running Windows in a non-native display language would get the wrong country from it. Already present pre-T-227 as a `embed-resource` build-dep on this target (row above) — this is its first appearance as a **direct runtime** dependency (`cargo tree -e no-dev --target x86_64-pc-windows-msvc -i winreg` now prints `dnsqb-service`, not empty), and reuses the same `windows-sys` 0.59.0 already pulled by `rfd` (no new duplicate version per `cargo deny check`) | `unsafe` Win32 registry FFI is contained in `winreg` itself, `forbid(unsafe_code)` intact in every crate of this project. Read-only, one key, one value, `.ok()?` on every fallible step — absent key/value, wrong type, or non-2-letter content all fold to `None` (a UX nicety, never a required signal, so no error path is worth surfacing) |
| `sys-locale` 0.3.2 (T-151 Батч 5.5, `dnsqb-tray`; **T-235 Батч 5.6 promoted it to `dnsqb-service` too**) | Sole caller: `i18n::detect_locale`/`cli_help::detect_locale` — OS-display-language auto-detect (no manual switcher in either the tray or `--help`). This is exactly the signal T-227's `winreg` row above rejected it for (UI display language, not region) — the opposite need here, so that earlier rejection doesn't apply; picking it over hand-rolling `GetUserDefaultLocaleName` avoids a second `unsafe extern "system"` FFI surface in a `forbid(unsafe_code)` crate. `dnsqb-service`'s own `cli_help` module needed the exact same OS-language signal as the tray's `--help` text is shared across all three binaries (T-235's own design: one dictionary, not three) | Zero further crates in its own dependency tree on this target (`cargo tree -p sys-locale` prints only itself) — no new duplicate-version or license-allowlist entries. Read-only OS query, one `Option<String>` result; `detect_locale`'s own post-processing (split/lowercase/lookup) is pure and has no fallible external call of its own to fail loudly on |
