...
| Відхилені з'єднання (backstop) | `AdminStats.rejected_connections: u64` | §1.1, T-169 | лічильник | обчислюваний — кумулятивна кількість з'єднань, закритих на стелі одночасності (`admission::ConnectionGate`, `[limits].max_concurrent_connections`). Live-лічильник, не з логу (відхилене з'єднання не створює `LogEntry`). DTO-поле готове; окрема UI-картка — Backend-before-UI, ще не реалізована. `0` після аптайму = backstop жодного разу не спрацював (здоровий стан) |
