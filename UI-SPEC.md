...
| Поле | Тип | Джерело | Контрол UI | Що приймає / валідація | Фаза |
|---|---|---|---|---|---|
| Рейтинговий фільтр «бульбашка» — перемикач | `bool` | §5.3, T-127 | **окремий, візуально виділений**, НЕ звичайний чекбокс поруч з іншими | **дефолт OFF, без винятків**; явне попередження при увімкненні: "буде недоступна переважна більшість інтернету" | Ф4 |
| Рейтинговий фільтр — індикатор активності | `bool` (дзеркало перемикача) | §5.3, T-128 | окремий бейдж, завжди видимий (не лише в налаштуваннях) | лише читання, дзеркалить перемикач | Ф4 |
| Зони — N (розмір топ-N) | `u32` | §5.3, T-111 | числове поле | розмір курованого топ-N з per-country списку | Ф4 |
| Зони — країни | `Vec<String>` | §5.3, T-111 | multi-select per country | які країни активні для курованого топ-N | Ф4 |
| Зони — персональний список (§5.1.1) | `bool` | §5.1.1, T-138 | switch, **дефолт OFF** | увімкнути локально навчений персональний список зон | Ф4 |
...
| `set_rating_filter(config)` | `RatingFilterConfig` (перемикач + зони: N, країни, персональний список §5.1.1) | Розширені | Ф4 |
