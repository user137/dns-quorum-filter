...
| Списки виключень | Ф1 (+Ф7 T-218) | Високий | редактор allow/blocklist, підсвітка конфліктів; Ф7 — картка публічних блок-лист-бандлів (Артборд F, `mockups/gui-dashboard.html`), не в «Розширені» |
...
| Про застосунок | Ф1 база / Ф2 доповнення | Низький, але юридично обов'язковий вміст | ліцензія, атрибуція DB-IP Lite + `user-country` (T-226(б)) |
...
**Фаза 5, Батч 5.1 (2026-09-19, бекенд лише — "backend before UI"):** `AdminStatusResponse.
cctld_block: CctldBlockStatusView { blocked_codes: Vec<String> }` + `POST /admin/cctld-block`
(`CctldBlockConfigUpdate { blocked_codes }`, повна заміна, немає окремого `enabled` — порожній
список сам по собі й означає "вимкнено") реалізовані тоді. **Батч 5.3 (T-118) додав саму
картку** `#cctld-block-body` — сиблінг `#geoip-body` усередині `<details>` «Розширені», одразу
перед ним (обидва блокують за кодом країни/суфіксу). Combobox-UX `#rating-filter-body`'s
(пошук за кодом і назвою, чипи з `×`, dirty-state «Зберегти»), **не** простішого
`[geoip]`-картки `<datalist>`, як і планувалось. Деталі рядка — §3.6 нижче.
...
| Активне джерело бази | `Option<DatabaseSource>` (`DB_IP_LITE`/`USER_COUNTRY`/`GEO_LITE2`/`OTHER`) | §3.5, T-162, T-226(б) | текст, лише читання | класифіковано на сервері з метаданих **завантаженого** reader-а; рядок у `#geoip-body` |
...
| Атрибуція DB-IP Lite | текст-посилання | Наскрізні вимоги | футер сторінки `#credits` (не футер вкладки — UI однобічний) | **CC BY 4.0** (не CC BY-SA), юридично обов'язково; реалізовано T-81; рядок лишається, бо оновлене встановлення й далі обслуговує вже наявний на диску DB-IP-файл, доки перший `user-country`-рефреш його не замінить (`DatabaseSource::DbIpLite` — класифікація завантаженого файлу, не `GeoipSource`, який більше не обирає DB-IP на жодному коду) |
| Атрибуція `user-country` | текст-посилання | T-226(б) | той самий футер `#credits` | PDDL — юридично не обов'язково, доданий як жест прозорості поряд із дефолтом від 2026-09-12 |
...
> "Видалити"), не `<select multiple>` — той самий шаблон, що й `#overrides-body` (T-47). Поле
> вводу коду країни лишається вільним текстом (сервер усе одно валідує через
> `validate_country_code`), але з T-226(а) має `<datalist>` — назви ISO 3166-1 англійською
> (`pycountry`, не перекладено вручну; передбачувано стане locale-lookup при T-151) підказують
> код при введенні.
...
| Зони доступності — вибір списків | `Vec<String>` (`RatingFilterStatusView.lists`, `available_lists`) | §5.3, T-111 | ⚠️ реалізовано Батч 4.4 — hand-rolled combobox (`input[role=combobox]` + `ul[role=listbox]`, ↑↓/Enter/Esc, `aria-activedescendant`) + стовпчик обраних із лічильником доменів і `×`; чекбокси рендеряться з `available_lists` (сервер), не з константи в JS. Кнопка «Зберегти зони» шле один `POST` (одна перебудова кешу). **T-227:** поки `lists` порожній і нічого локально не обрано, невелика inline-підказка над комбобоксом ("Рекомендовано для вашого регіону: …") + кнопка «Додати» — рендерить `suggested_list`, ховається щойно `picked.size > 0` (клік по будь-якій зоні, не лише кнопці підказки) | коди зі `available_lists` (двобуквені країни + `"global"`); **`lists ⊆ available_lists` — гарантований інваріант** (T-127, `validate_rating_filter_lists` відхиляє код поза `AVAILABLE_TOPN_LISTS`). Мітки країн — клієнтська мапа в `main.js` (i18n сервера тут немає); невідомий код показується сам собою + `main.js` малює обʼєднання (обороняє конфіг, збережений до T-127) | Ф4 |
...
| ccTLD-блок — коди TLD | `Vec<String>` (`CctldBlockStatusView.blocked_codes`) | §5.2, T-118 | ✅ реалізовано Батч 5.3 — картка `#cctld-block-body`, той самий combobox-UX, що `#rating-filter-body` (пошук за кодом і назвою через `regionLabel()`/`Intl.DisplayNames`, чипи з `×`, dirty-state «Зберегти»); типований 2-літерний код, якого нема в меню, теж приймається (Enter-фолбек) | **дефолт порожній**, немає окремого `enabled`. Попередження про грубість евристики — заброньований (armed) крок при збереженні, лише коли новий набір **додає** код (Три Б: постійний банер = відсутність банера); чисте видалення коду постить одразу. `POST /admin/cctld-block {blocked_codes}` (повна заміна) | Ф5 ✅ |
...
| Атрибуція DB-IP Lite, **CC BY 4.0** (не CC BY-SA) | Наскрізні вимоги, T-81 | Ф2 | реалізовано T-81 — футер `#credits` (не окремий екран); обов'язковий сніпет `<a href='https://db-ip.com'>IP Geolocation by DB-IP</a>` + посилання на ліцензію; поряд — атрибуція MaxMind GeoLite2 ("у розширеному режимі", T-80) та `user-country` (T-226(б), PDDL, не юридично обов'язково) |
...
### 3.8 Довідкові підказки полів (T-159)

Власник *сенсу* кожного поля — ця таблиця; власник *перекладеного тексту*, що фактично
рендериться — рантайм-словники `crates/dnsqb-service/ui/i18n/<locale>.json`, 37 локалей (T-151
Батч 5.2 запустив uk+en 2026-09-19; T-236 того ж дня розширив до повного списку — правила
перекладу й виміряна таблиця плюралізації в `ui/i18n/GLOSSARY.md`). Текст нижче — дзеркало
uk.json, не вигадується на льоту в жодному з двох місць.
Механізм — нативний `<details><summary>?</summary><p>…</p></details>` (T-159's варіант 1: нуль JS
понад конструювання елемента, клавіатурна доступність з коробки, працює на тач-екранах на відміну
від hover-тултипа). Реалізовано для п'яти карток, названих у TASKS.md's T-159: Провайдери,
Timeout-режим, Кеш, Списки виключень, Лог-фільтри — по одній підказці на картку, не на кожне
окреме поле всередині (обсяг картки — практична одиниця для 1-2-реченевого тексту).

**i18n (T-151/T-236) — зроблено для цього екрана:** `main.js`'s колишній плаский об'єкт
`FIELD_HELP` видалено; `helpDetails(key)` тепер викликає `t(\`fieldHelp.${key}\`)` — locale-lookup
проти `DICT`, завантаженого з `/admin/ui/i18n/<locale>.json` (37 локалей). Ключі нижче — той самий
`key`, з префіксом `fieldHelp.` у словнику.

| Ключ (`ui/i18n/*.json`) | Картка | Текст (укр.) |
|---|---|---|
| `fieldHelp.providers` | Провайдери-voter'и | «Кожен увімкнений провайдер перевіряє домен незалежно; якщо хоч один каже «блокувати» — кворум блокує (OR-логіка). Більше провайдерів — ширше покриття, але й більше третіх сторін бачать ваші запити.» |
| `fieldHelp.timeoutMode` | Поведінка при збої | «Як трактувати провайдера, що не відповів вчасно. «fail_open» (типово) — пропустити, вважаючи безпечним; «fail_closed» — заблокувати; «degraded» — як fail_open, але явно позначає рішення неповним у лозі.» |
| `fieldHelp.cache` | Кеш | «Мін./макс. час життя запису кешу (обмежує TTL від провайдера), скільки тримати «заблоковано», і скільки записів кеш утримує одночасно, перш ніж почне витісняти найстаріші.» |
| `fieldHelp.overrides` | Списки виключень | «Власні винятки поверх кворуму: allowlist завжди дозволяє, blocklist завжди блокує, незалежно від відповіді провайдерів. При конфлікті між списками виграє allowlist.» |
| `fieldHelp.logFilters` | Лог запитів | «Пошук і фільтри діють лише в межах поточного вікна журналу — останні ~1000 запитів або 24 години; старіші записи вже не зберігаються.» |

Поза межами цього батчу: GeoIP-картка, MaxMind-картка, рейтинг-фільтр «бульбашка» — жоден не
названий явно в T-159's власному прикладному переліку; додати за тим самим патерном, коли
підніметься.
