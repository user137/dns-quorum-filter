...
список сам по собі й означає "вимкнено") **вже реалізовані й доступні**, але **картки UI ще
нема** — T-118 (Батч 5.3, за узгодженим планом чекає на i18n-інфраструктуру Батчу 5.2, бо
показуватиме назву країни через `Intl.DisplayNames`, якій потрібне поняття "поточна локаль").
Коли T-118 буде зроблено — за зразком `#rating-filter-body`'s combobox-UX (пошук за кодом і
назвою, чипи з `×`, dirty-state «Зберегти»), не простішого `[geoip]`-картки `<datalist>`; рядок
цієї таблиці додасться тоді ж, не раніше.
