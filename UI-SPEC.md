...
Власник тексту коротких "?" підказок біля заголовка кожної картки — `main.js`'s `FIELD_HELP`
дзеркалить цю таблицю дослівно, не вигадує текст на льоту. Механізм — нативний
`<details><summary>?</summary><p>…</p></details>` (T-159's варіант 1: нуль JS понад
конструювання елемента, клавіатурна доступність з коробки, працює на тач-екранах на відміну від
hover-тултипа). Реалізовано для п'яти карток, названих у TASKS.md's T-159: Провайдери,
...
**Готовність до T-151 (i18n):** `FIELD_HELP` — один плаский об'єкт `{ключ: текст}` у `main.js`,
а не текст, розсипаний по функціях рендеру — коли i18n дійде до цього екрана, це єдине місце,
яке треба замінити на locale-залежний lookup, а не пошук по всьому файлу.

| Ключ (`FIELD_HELP`) | Картка | Текст (укр.) |
|---|---|---|
| `providers` | Провайдери-voter'и | «Кожен увімкнений провайдер перевіряє домен незалежно; якщо хоч один каже «блокувати» — кворум блокує (OR-логіка). Більше провайдерів — ширше покриття, але й більше третіх сторін бачать ваші запити.» |
| `timeoutMode` | Поведінка при збої | «Як трактувати провайдера, що не відповів вчасно. «fail_open» (типово) — пропустити, вважаючи безпечним; «fail_closed» — заблокувати; «degraded» — як fail_open, але явно позначає рішення неповним у лозі.» |
| `cache` | Кеш | «Мін./макс. час життя запису кешу (обмежує TTL від провайдера), скільки тримати «заблоковано», і скільки записів кеш утримує одночасно, перш ніж почне витісняти найстаріші.» |
| `overrides` | Списки виключень | «Власні винятки поверх кворуму: allowlist завжди дозволяє, blocklist завжди блокує, незалежно від відповіді провайдерів. При конфлікті між списками виграє allowlist.» |
| `logFilters` | Лог запитів | «Пошук і фільтри діють лише в межах поточного вікна журналу — останні ~1000 запитів або 24 години; старіші записи вже не зберігаються.» |
