...
рендериться — рантайм-словники `crates/dnsqb-service/ui/i18n/{uk,en}.json` (T-151 Батч 5.2,
2026-09-19). Текст нижче — дзеркало uk.json, не вигадується на льоту в жодному з двох місць.
...
**i18n (T-151) — зроблено для цього екрана, Батч 5.2:** `main.js`'s колишній плаский об'єкт
`FIELD_HELP` видалено; `helpDetails(key)` тепер викликає `t(\`fieldHelp.${key}\`)` — locale-lookup
проти `DICT`, завантаженого з `/admin/ui/i18n/{uk,en}.json`. Ключі нижче — той самий `key`, з
префіксом `fieldHelp.` у словнику.
