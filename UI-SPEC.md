...
| **[basic]** Секція «Підключення браузера» (T-176) | статичний текст + `AdminStatusResponse.port` | README §"Швидкий старт" / §"Перевірка" | завжди доступна картка вгорі: copy-paste `https://127.0.0.1:<port>/dns-query` + згорнуті покрокові інструкції на браузер; при першому візиті авто-розгорнута (позначка в `localStorage` на admin-origin, без бекенд-сигналу); крок «Перевірити» блокує тестовий домен і показує `ERR_ADDRESS_INVALID` (те, що T-172 робив вручну) | лише читання + кнопки «Копіювати» / «Запустити перевірку» |
