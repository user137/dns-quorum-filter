//! `dnsqb-service` binary orchestration (T-143) — TCP accept loop + TLS
//! termination only; `DoH` GET/POST request/response logic lives in
//! [`crate::dispatch`] (unit-tested there without a live socket). [`run`]
//! itself is deliberately not unit-tested, same "hardcoded real resource,
//! untested by design" precedent as [`crate::paths::app_data_dir`]/
//! [`crate::cert::write_cert_and_key_to_app_data`] — see the manual
//! smoke-test step recorded for T-143 in `TASKS-DONE.md` instead.
//!
//! **T-210 (moved here from `dnsqb-service/src/main.rs`):** that binary is
//! a separate crate linking this lib externally, so keeping this
//! orchestration there forced ~20 internal helpers (`run_*`/`load_*`
//! persistence and updater entry points, `serve`, `AppState`, …) to be
//! `pub` at the crate root for no reason other than that one file. `main.rs`
//! is now a three-line shim calling [`run`]; the helpers below that only
//! [`run`] itself calls no longer need to be reachable from outside this
//! crate at all.
//!
...
//!
//! **§7.1 #7 boundary (T-210):** [`run_service_to_watcher_watchdog`] below is
//! reachable only from [`run`], and only `dnsqb-service`'s own `main.rs`
//! calls [`run`] — `dnsqb-watcher` links this same lib but must never call
//! [`run`] or [`run_service_to_watcher_watchdog`] directly. Before this
//! module existed that boundary was physical (two separate `main.rs`
//! files); now that both binaries link one lib, it is enforced only by "who
//! actually calls this," so it is named here explicitly rather than left to
...
/// `dnsqb-service`'s whole startup + serve-until-shutdown sequence. Called
/// once from `dnsqb-service/src/main.rs`'s `#[tokio::main] async fn main`
/// (T-210) — see the module doc's §7.1 #7 note for why `dnsqb-watcher` must
/// never call this.
pub async fn run() {
...
/// persisted (§7.1 #7). **Reachable only from [`run`]** — see the module
/// doc's §7.1 #7 boundary note.
#[cfg(windows)]
...
/// whatever's already in flight before returning. Split out of [`run`] to
