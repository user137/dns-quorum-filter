//! `DoH` listener bind (T-21). Real HTTP/TLS serving is a later batch — the
//! self-signed leaf certificate itself is generated by `cert::
//! generate_self_signed_cert` (T-48), but the actual `hyper` + TLS listener
//! wiring in `main.rs` is still not done. This module is just the "fixed
//! port, explicit conflict error" contract SPEC.md §1 requires.
