...
`orchestrate.rs`, live-reload on `/admin/reset`) and Батч 7.4 part 1 (pipeline step 2 now
actually consults the bundle, suffix/wildcard match) shipped 2026-09-13 too. **Still 7.4+** —
admin route/DTO for live per-source toggling, an `/admin/status` view, `/admin/ui` rendering.
