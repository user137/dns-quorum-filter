...
kickoff + design (Артборд F) closed 2026-09-13; Батч 7.1 (pure download/parse/hash core) shipped
same day; Батч 7.2 (`blocklist_updater` — network fetch/parse/atomic swap into `AppState`)
shipped 2026-09-13. **Not yet spawned anywhere** (no `orchestrate.rs` caller — see
`blocklist_updater`'s module-table row) and not yet read by the pipeline; `[blocklist_bundles]`
config, admin route, spawn wiring, and pipeline step 2 consumption are all 7.3+.
...
| `blocklist_updater` | T-218 Фаза 7, Батч 7.2 — network fetch + atomic swap, built on `blocklist_download`. Per-source fetch bounded by `MAX_BLOCKLIST_BYTES`, `BLOCKLIST_FETCH_TIMEOUT` = 600s (128 MB cap needs ~1.7 Mbit/s sustained to clear in that window — a 180s draft would have made the cap practically unreachable and silently pinned large sources on last-known-good forever). Raw text atomically written to `<app-data>/blocklists/<id>.txt`; `BlocklistBundleState { seed: RandomState, domains: Vec<u64>, sources: Vec<BlocklistSourceStatus> }` swapped via `AppState.blocklist_bundles: RwLock<Arc<_>>` (same shape as `rating_filter_zone`). `BlocklistBundleState::contains_domain` — exact-match only (not the suffix walk 7.3's pipeline wiring needs — 7.1's closing-review finding #2 on wildcard-vs-exact source semantics is still open). Failed source falls back to its last-known-good disk file, carries the *previous* `last_updated` forward, and still records `last_error` even when that fallback succeeds. **Not spawned anywhere this batch** — no `[blocklist_bundles]` config or admin route exists yet to gate an unconditional ~111 MB/24h refetch (network politeness); `refresh_all_sources`/`load_blocklist_bundles_from_disk` and the new `AppState` accessors have no `orchestrate.rs` caller until Батч 7.3 adds the `loop { refresh; park }` wrapper, wake channel, and `tokio::spawn` |
