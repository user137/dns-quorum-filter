...
kickoff + design (Артборд F) closed 2026-09-13; Батч 7.1 (pure download/parse/hash core) and
Батч 7.2 (`blocklist_updater` — network fetch/parse/atomic swap into `AppState`) shipped same
day; Батч 7.3 (`[blocklist_bundles]` config + `run_blocklist_updater` actually spawned from
`orchestrate.rs`, live-reload on `/admin/reset`), Батч 7.4 part 1 (pipeline step 2 now actually
consults the bundle, suffix/wildcard match), and Батч 7.4 part 2 (`POST /admin/blocklist-bundles`
+ `AdminStatusResponse.blocklist_bundles` per-source status view, `ADMIN_DTO_SCHEMA_VERSION` 3→4)
shipped 2026-09-13/17. **Still 7.4+** — `/admin/ui` rendering (Артборд F is approved but the
mockup's own open questions on per-source granularity/grouping/confirm-flow are not yet resolved).
