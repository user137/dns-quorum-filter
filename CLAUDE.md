...
| `blocklist_download` | T-218 Фаза 7, Батч 7.1 — pure, no-network core for the public blocklist bundles (SPEC.md §5 step 2 extension). `BLOCKLIST_SOURCES: &[BlocklistSource]` (7 logical sources / 8 URLs, `SourceFormat::{PlainDomain,AdblockNetRules}`), `MAX_BLOCKLIST_BYTES` = 128 MB (source-size guard, over-limit fetch in 7.2 keeps last-known-good like `topn_updater`). `hash_plain_domains`/`hash_adblock_domains` — streaming parse-and-hash (no intermediate `Vec<String>`) into a caller `Vec<u64>` via `std::collections::hash_map::RandomState::hash_one` (hashes never persisted — hot-path suffix-walk re-hashes every query, not a 24h-only cost, so hash stability across Rust versions doesn't matter and no new crypto dep is needed). `finalize` = `sort_unstable`+`dedup` for the eventual combined sorted `Vec<u64>` (O(log n) binary-search lookup, chosen over a `HashSet<u128>` for ~3x memory reduction at ~5.5M-entry scale). Not yet wired into `AppState`/pipeline — `blocklist_updater.rs` (network/orchestration/atomic swap) is its own future batch (7.2) |
