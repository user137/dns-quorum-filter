...
`orchestrate.rs`, live-reload on `/admin/reset`) shipped 2026-09-13 too. **Not yet read by the
pipeline** — admin route/DTO for live per-source toggling, an `/admin/status` view, `/admin/ui`
rendering, and pipeline step 2 consumption (`contains_domain` wiring) are all 7.4+.
