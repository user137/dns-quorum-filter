...
**Not started:** Фаза 6 (macOS/Linux port — see "Current phase boundaries" below for the
architectural seam it needs).
...
end through the pipeline (allowlist → blocklist → **ccTLD block, step 3, §5.2, Фаза 5, T-115** →
cache → quorum) plus live GeoIP filtering (SPEC.md §3.5 / §5.3 step 7) and the **rating filter
«bubble» step 5** (§5.3, default OFF; out-of-zone → BLOCK, in-zone → normal pipeline). (There is
no "voter scope" step any more — §5.1 was removed.) Startup also spawns three detached
`#[cfg(windows)]` watchdog tasks (heartbeat pipe
...
| `pipeline` | `handle_query` request flow (takes `UpstreamContext { timeout, baseline_url, serve_baseline_fallback, reachability, filtering_paused, rating_filter, blocklist_bundles }` bundle); `overrides_step` (extracted from `handle_query` for `clippy::too_many_lines`, Батч 7.4) resolves steps 1–2 — allowlist, manual blocklist, then the T-218 public bundle (`blocklist_bundle_hit`/`matches_domain`, `DecisionSource::BlocklistBundle`, **not cached**) — before deriving `(domain, log_domain, qtype)` for the rest of `handle_query`; `invalidate_changed` (cache eviction on override-list reload). Offline (T-152) → `offline_servfail_with_meta` before cache read. `outcome.filters_unreachable` (T-155) → `DecisionSource::BaselineFallback`, never cached. **Step 5 rating filter:** `rating_filter_step` after the cache read, before `resolve` — `RatingFilterView { lists, personal, removed }` snapshots (`personal` — the personal learned zone, checked as a second independent `zone_match` call on a `lists` miss); out-of-zone → `rating_filter_block_with_meta` (`DecisionSource::RatingFilter`, **not cached**); in-zone exact (hygiene-eligible on whichever `ZoneLists` matched) + quorum `Block` → `QueryLogMeta.zone_removal` (lazy-hygiene action signal, not a log field) |
...
| `blocklist_download` | T-218 Фаза 7, Батч 7.1 — pure, no-network core for the public blocklist bundles (SPEC.md §5 step 2 extension). `BLOCKLIST_SOURCES: &[BlocklistSource]` (7 logical sources / 8 URLs, `SourceFormat::{PlainDomain,AdblockNetRules}`), `MAX_BLOCKLIST_BYTES` = 128 MB (source-size guard, over-limit fetch in 7.2 keeps last-known-good like `topn_updater`). `hash_plain_domains`/`hash_adblock_domains` — streaming parse-and-hash (no intermediate `Vec<String>`) into a caller `Vec<u64>` via `std::collections::hash_map::RandomState::hash_one` (hashes never persisted — hot-path suffix-walk re-hashes every query, not a 24h-only cost, so hash stability across Rust versions doesn't matter and no new crypto dep is needed). `finalize` = `sort_unstable`+`dedup` for the eventual combined sorted `Vec<u64>` (O(log n) binary-search lookup, chosen over a `HashSet<u128>` for ~3x memory reduction at ~5.5M-entry scale). **T-233:** `validate_hashes` (parse-success ratio ≥ `MIN_PARSE_SUCCESS_RATIO` + < `MIN_CANARY_HITS_TO_REJECT` exact hits on `BLOCKLIST_CANARY_DOMAINS`) + `delta_verdict` (entry-count vs. the persisted `<id>.count` baseline, `Option<(usize, Duration)>` in → the clocks-injected split keeps floor/grace/multiplier bounds unit-testable) gate each source before `blocklist_updater` ever writes it to disk; `push_normalized` also drops any candidate that is itself an ICANN-section public suffix (`crate::public_suffix::Psl::is_public_suffix`, part 3 — PRIVATE-section entries like `github.io` are left unfiltered, `hagezi-hoster`/`hagezi-dyndns` intentionally list those as delegating-namespace block targets, DECISIONS.md 2026-09-18) — KNOWN-LIMITATIONS.md has what's still open (small-injection gap across all four gates). Sole caller is `blocklist_updater` (below) |
| `blocklist_updater` | T-218 Фаза 7, Батч 7.2 (core) + Батч 7.3 (config + spawn) — network fetch + atomic swap, built on `blocklist_download`. Per-source fetch bounded by `MAX_BLOCKLIST_BYTES`, `BLOCKLIST_FETCH_TIMEOUT` = 600s (128 MB cap needs ~1.7 Mbit/s sustained to clear in that window — a 180s draft would have made the cap practically unreachable and silently pinned large sources on last-known-good forever). Raw text atomically written to `<app-data>/blocklists/<id>.txt`; `BlocklistBundleState { seed: RandomState, domains: Vec<u64>, sources: Vec<BlocklistSourceStatus> }` swapped via `AppState.blocklist_bundles: RwLock<Arc<_>>` (same shape as `rating_filter_zone`). `BlocklistBundleState::matches_domain` (Батч 7.4, renamed from `contains_domain`) — suffix/wildcard walk, resolving 7.1's closing-review finding #2 (DECISIONS.md 2026-09-13): every source is subdomain-inclusive by intent, and the walk structurally never tests a bare TLD candidate. Failed source falls back to its last-known-good disk file, carries the *previous* `last_updated` forward, and still records `last_error` even when that fallback succeeds. **`run_blocklist_updater` (loop { refresh_all_sources; park_until_due }, 24h interval, mirrors `run_topn_updater`) is now spawned from `orchestrate::spawn_public_http_tasks`** — gated by `config::BlocklistBundlesConfig` (`enabled: bool`, `sources: Option<Vec<String>>` — `None` = every currently published source, resolved live each cycle, never frozen into `resolver_config.toml`; `Some(ids)` = an explicit, validated, no-longer-auto-tracking subset; `Some([])` = explicitly inert). Config lives on `AppState` as `RwLock<Arc<BlocklistBundlesConfig>>` + a `Notify` wake handle, same live-snapshot/`POST /admin/reset`-wake pattern as `rating_filter`/`personal_zone` (T-217: every `ResolverConfig{...}` write site in `dispatch.rs` reads the live snapshot, never a stale `PersistTarget` echo). `load_blocklist_bundles_from_disk` still has no caller — the updater's own first cycle (runs before any park) populates the bundle instead of a synchronous startup disk-read of a potentially ~111 MB set |
...
| `query_log` | in-memory ring buffer (`parking_lot::RwLock`); `LogEntry`, `DecisionSource` (9 producible, all 9 the DTO declares: +`BaselineFallback` T-155 — the one whose `voters` is **not** empty; +`RatingFilter` T-124; +`BlocklistBundle` Батч 7.4; +`CctldBlock` Фаза 5, T-116), `LogFilter` search, `clear`; `restore(entries, now)` (T-146 — seeds from `query-log.enc`, re-applies both the 1000/24h bounds) |
...
| `cctld_block` | Фаза 5, Батч 5.1 — SPEC.md §5.2, pipeline step 3. Pure, no-network: `is_blocked(normalized_domain, blocked) -> bool` compares the domain's own last label (`rsplit('.').next()`) against `[cctld_block].blocked_codes` (`config.rs`, lowercased, validated 2-letter entries), case-insensitive. **Deliberately blocks a bare TLD candidate too** (`is_blocked("ru", &["ru"])` → `true`) — unlike `blocklist_updater::BlocklistBundleState::matches_domain`, which structurally never does, because this list is operator-typed, not fetched from an untrusted third party. No `enabled` flag — an empty list already means inert, same convention as `GeoipFilter::blocked_countries`. `pipeline::overrides_step` runs it right after the manual-blocklist/blocklist-bundle checks (step 2), before Cache — same pipeline slot ordering as SPEC.md §5.3, so it also runs above the `filtering_paused` fast path (a pause does not exempt a ccTLD match, same as the blocklist/bundle). `POST /admin/cctld-block` (full-replace, no incremental add/remove — the future Батч 5.3 UI batches picks locally and saves once, the «bubble»'s own pattern, not `/admin/geoip/add`'s) mirrors `/admin/blocklist-bundles`'s shape exactly (`apply_cctld_block_change`, `dispatch.rs`) |
...
in `FUZZ_EXCLUDED_ROUTES`); `POST /admin/blocklist-bundles` (T-218 Батч 7.4 частина 2,
`BlocklistBundlesConfigUpdate { enabled, sources: Option<Vec<String>> }`, full replace; shares
`persist_lock`; validates `sources` (only when `Some`) via
`validate_blocklist_bundle_sources` → `400`; wakes `run_blocklist_updater`; **no** cache-rebuild
branch — see the `dispatch` row for why; **not** in `FUZZ_EXCLUDED_ROUTES`);
`POST /admin/cctld-block` (Фаза 5, Батч 5.1, `CctldBlockConfigUpdate { blocked_codes:
Vec<String> }`, full replace; shares `persist_lock`; validates codes via `validate_cctld_codes` →
`400`; no wake call — this list has no background updater; **not** in `FUZZ_EXCLUDED_ROUTES`);
`POST /admin/uninstall-local-state` (no body fields, never touches
...
- `cargo run --example curate_topn -- lists=ua,global n=1000` — T-107 Фаза 4 curation tool
  ...
  normalises origin→registrable via a bundled pinned PSL (`src/public_suffix_list.dat`,
