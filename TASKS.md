...
- [ ] T-220 — **Знайдено 2026-09-12, НЕ виправлено.** `trust_store.rs`'s `NOT_FOUND_EXIT_CODE:
  ...
  всіх попередніх релізах з часів T-49. Живий стан машини відновлено (сертифікат перевстановлено
  через трей, `/admin/cert-status` → `TRUSTED`). Потребує власного plan+advisor циклу перед фіксом
...
- [ ] T-221 — **Знайдено 2026-09-12, НЕ виправлено.** `[personal_zone].enabled = true` в
  ...
  (недокументований) обхідний шлях. **Виправляє наявний запис "Known limitations" пункт (f)**
