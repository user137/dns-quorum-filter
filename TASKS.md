...
**План виконання Ф4 (2026-09-06, plan-mode + advisor kickoff+closing, збережено для наступних
сесій).** Фаза 4 = SPEC.md §5.1 (+ §5.1.1): топ-N доменів країни виключаються **лише з
Ads/Adult-категорій voters** — Security-quorum лишається живим для них завжди, на кожному запиті.
Задачі T-104–T-114 + T-138. **Це НЕ безпекова фіча** (на відміну від первинної редакції спека, де
топ-сайти були bypass quorum): ставки помилки тут — ads-шум на популярному сайті, не послаблений
захист від malware. Тому дефолт 5.1 "увімкнено" виправданий упевненіше, ніж перша редакція — але
формально це судження (Відкриті питання п.7), підтвердити з користувачем на kickoff.

**Ключова відмінність від Ф1–Ф3:** єдина позиція спека, що вимагає власного **проєктного
CI-конвеєра курації** (не лише клієнтського коду) — fetch Cloudflare Radar Domain Rankings →
публікація версійованого per-country списку доменів + checksum. Клієнтський бік завантаження — той
самий механізм, що GeoIP (T-75: TLS + `.sha256`-sidecar + atomic swap, у CI з Ф2, **фактично вже
обкатаний** — T-105 зводиться до однорядкового підтвердження, не окремого гейта), новий тільки
конвеєр курації.

**Дві незалежні жорсткі передумови всієї фази — перевірити ПЕРШИМИ, у Батчі 4.0, у цьому порядку:**
1. **T-106 (ToS Cloudflare Radar) — дешева, читання документа, РОБИТИ ДО T-104.** Якщо ToS
   забороняє републікацію похідного списку доменів як частини стороннього продукту — **вся
   централізовано-курована модель 5.1 мертва**, не відкладений батч. Виходи: (а) дозволено →
   далі; (б) заборонено → оцінити альтернативне джерело рейтингу (**Tranco** — CC BY 4.0, явно
   дозволяє редистрибуцію, очевидний fallback; **не** Majestic) → якщо жодне не підходить, 5.1
   лишається непобудованою, життєздатний тільки **5.1.1 (T-138)**, який зовнішнього списку не
   потребує взагалі.
2. **T-104 (метрик-гейт) — дорога, жива вимірка, РОБИТИ ПІСЛЯ T-106.** Виміряти реальний
   false-positive rate Ads/Adult-voters **саме на топ-N-доменах** (не Security — там FP не про цю
   фічу). Метод: розширити `examples/phase1_metrics.rs` або новий `examples/topn_fp_probe.rs` —
   прогнати топ-N кількох країн через увімкнені Ads/Adult-voters, порахувати частку заблокованих.
   Manual, не CI (як `phase1_metrics`/`load_test`). **Якщо FP rate уже низький → фаза закривається
   самим записом виміру** (клас T-99/T-164 "closed without code"), не імплементувати "бо було
   заплановано" (SPEC.md §Фазований план Ф4, Відкриті питання п.7). Робити T-104 до T-106 = ризик
   витратити вимірку на фічу, яку не ліцензовано шипнути.

**Батчі (4.0–4.5) — за спільним дизайном, не за кількістю задач. Каденс за батчами:** стояча
каденс (одна задача — один коміт, пауза-звіт між задачами) лишається на рівні задачі; батчинг
міняє лише **plan-mode + advisor kickoff — один на батч** (на першій задачі), бо задачі в батчі
ділять один дизайн. Closing-advisor — перед тим, як останній коміт батча вважається зробленим;
CI-green — після кожного пушу з `.rs`/`.html`/`.css`/`.js`/`Cargo.*`; пауза-звіт — між батчами.
Кожен новий модуль/маршрут — 4 категорії тестів (Happy / Security-Boundary / Misuse-Fool / Error)
+ Concurrency/Recovery.

- **Батч 4.0 — kickoff: дві передумови + дизайн інфраструктури курації + діаграма.** Власний
  plan-mode + AskUserQuestion + advisor. Послідовність: **T-106 → (якщо OK) T-104 → крос-батчеві
  рішення**. T-105 — однорядкове підтвердження "GeoIP download/checksum/atomic-swap переюзовується,
  не пишеться заново", не гейт. **Крос-батчеві рішення в AskUserQuestion (усі — рішення користувача,
  не агента):**
  - **"Поточна країна" — звідки?** (load-bearing, T-109 без цього не спланувати): GeoIP self-lookup
    власної egress-IP користувача (новий outbound-виклик + приватнісна поверхня, якої проєкт
    уникав), поле в `resolver_config.toml`, чи система/locale. Взаємодія з T-111: якщо країна
    конфігурується користувачем, "вимкнути по країні" частково згортається в це саме поле.
  - **Де хоститься курований список:** переюзати `release.yml`-канал проєкту / окремий
    repo-artifact / CDN. Формат: per-country, версійований (версія несе дату — денний рейтинг
    Radar сам змінюється щодня), `.sha256`-sidecar як GeoIP.
  - **Radar API-токен** належить проєкту, **не** вбудований у клієнт (той самий принцип, що
    MaxMind-токен, SPEC §3.5).
  - **Дефолт 5.1 "увімкнено" глобально** — підтвердити (SPEC §5.1 — судження, не факт).
  - **`voter_scope` — 2 чи 3 варіанти enum'а?** Спитати ОДИН раз тут і зафіксувати. Якщо
    3-варіантний (5.1 vs 5.1.1 `SECURITY_ONLY` означають генетично різне — Відкриті питання п.11),
    то T-112 у Батчі 4.2 одразу populate'ить фінальну форму, а не мігрує wire-поле через два батчі.
    `UI-SPEC.md` + `diagrams/ui-dto-model.md` зафіксували 2-варіантну — синхронне оновлення, якщо
    міняється.
  - **Діаграма до коду (ground-truth ритуал):** `diagrams/voter-scope.md` — крок 4 конвеєра
    (SPEC §5.3), склад voters FULL vs SECURITY_ONLY, звідки береться country + top-N list. SOURCES-блок.
  - Виводить сам цей блок у TASKS.md.

- **Батч 4.1 — конвеєр курації (T-107, T-108) — CI/проєкт-only, після T-106 confirmed.** T-107:
  новий workflow або cron-скрипт — fetch Radar Domain Rankings (топ-100 денний, 200+ країн) →
  нормалізація → публікація версійованого per-country списку + SHA-256, формат/канал з рішення
  Батча 4.0. Не клієнтський код; власні concerns як `release.yml` (токен у secret, детермінований
  вивід або зафіксувати денну зміну рейтингу як частину версії, розмір-ліміт, no rust-cache).
  T-108: легка крос-перевірка на етапі курації проти Security-блоклиста (не публікувати
  ads/adult-виняток для домену, що вже явно в Security-блоклисті на момент курації) — гігієна
  списку, **не** захисний механізм (Security живий на кожному запиті).

- **Батч 4.2 — клієнт: механізм voter-scope (T-109, T-110, T-112, T-113, T-114) — справжня
  архітектурна правка конвеєра, власний plan+advisor kickoff.**
  - **T-109:** топ-N домен поточної країни → у quorum-запиті лише Security-tier voters (Quad9,
    Cloudflare Malware, CleanBrowsing Security, DNS4EU Protective), Ads/Adult виключено, навіть коли
    користувач їх глобально ввімкнув. Клієнтський download: переюзати
    `geoip_download`/`geoip_updater`/atomic-swap (T-75) для per-country top-N файлу; нова
    `AppState`-секція за зразком `GeoipState` (`RwLock<Arc<_>>`, reader `Arc::clone`, lock не
    тримається через `.await`). `pipeline.rs`: новий крок voter-scope між cache-read і quorum
    (SPEC §5.3 крок 4) — **чиста функція, що повертає звужений зріз voters як дані**
    (`fn scope_voters(domain, country, &TopNList, &[ProviderEntry]) -> Vec<ProviderEntry>` чи
    аналог), тестована окремо. `quorum::resolve` уже бере `&[ProviderEntry]` — voter-scope звужує
    цей зріз ПЕРЕД викликом, сигнатура не міняється (на відміну від T-72/T-73).
  - **T-110:** лише точний registrable-домен, **без wildcard-збігу** (трекери на піддоменах
    `ads.example.com` не мають вийти з-під Ads-фільтрації разом з apex — прямо суперечить меті).
    Явний тест (T-114).
  - **T-112:** `LogEntry`/`LogEntryView` `voter_scope` — DTO вже існує (`VoterScopeView`, T-54),
    почати **populate** (фінальна 2- чи 3-варіантна форма з рішення Батча 4.0), нового wire-поля не
    додавати. `query_log`-мод-doc оновити.
  - **T-113 — найчутливіший регресійний тест, зробити СТРУКТУРНИМ (T-59 прецедент):** тестувати не
    через поведінку `quorum::resolve`, а через **чисту scope-функцію, що повертає зріз як дані** —
    assert, що Security-tier id завжди присутні в поверненому зрізі для топ-N домену, навіть коли
    Ads/Adult увімкнені глобально. "Проходить бо сьогодні так" ≠ "властивість спостережувана".
  - Плюс 4 категорії тестів на новий voter-scope-модуль + Concurrency (download-swap під час
    live-запиту).

- **Батч 4.3 — UI (T-111) — backend-before-UI.** Backend: admin-маршрут
  (`GET|POST /admin/voter-scope` чи розширення `/admin/config`) + `[voter_scope]`-секція в
  `resolver_config.toml` (cross-field-read + `persist_lock` — рекурентний клас багу T-57/T-77);
  `N` (розмір топ-списку), список країн для фічі, вимкнути повністю (глобально) чи по окремій
  країні (частково згортається в "поточна країна", якщо та конфігурується — див. Батч 4.0). UI:
  картка на `/admin/ui` за зразком `#geoip-body` (T-77), імовірно в `<details>` advanced (T-176
  поділ — це тонке налаштування), `persisted: false` при збої диску. 4 категорії тестів на
  маршрут; `dispatch::ROUTES` +рядок + оновити ROUTES-snapshot-тест (T-59).

- **Батч 4.4 — персональний варіант (T-138) — SPEC §5.1.1, остання, найвища приватнісна планка,
  власний plan+advisor+AskUserQuestion kickoff.** Локальне навчання частоти + регулярності;
  джерело — **лише вже-`ALLOW`-вердикти кворуму** (не allowlist-ALLOW — той термінальний ще до
  voter scope; не заблоковані на жодному кроці). Той самий виняток лише з Ads/Adult, той самий
  `voter_scope`-прапорець. **Окреме, третє шифроване сховище** (не розширення log/cache T-96/T-97)
  — opt-in, дефолт **вимкнено**. Переюзати `encrypted_file` (T-146) + `key_store`; зберігати
  агреговані лічильники (домен → к-сть відвідувань, бітмапа днів у вікні), **не** сирий журнал
  таймстампів. Пасивний спостерігач поверх фінального рішення `handle_query`, не крок конвеєра;
  рахувати користувач-ініційовані навігації, не кожен subresource-fan-out. Два критерії, обидва
  потрібні: (а) частота — домен у персональному топ-N; (б) регулярність — відвіданий у X із
  останніх Y днів. **Вирішити `voter_scope` DTO-прогалину** остаточно (реалізувати рішення Батча
  4.0). Дефолт "вимкнено" + той самий рівень UI-розкриття, що для fan-out-приватності (SPEC §8).
  **Може бути винесений окремо / відкладений** (як T-51/T-56 у Ф3), якщо фаза йде довго — 5.1.1
  дефолт-off, не блокує закриття основної 5.1.

- **Батч 4.5 — закриття фази + РІШЕННЯ про реліз.** Оновити SPEC.md §Фазований план Ф4 +
  Відкриті питання п.7/п.11 (вирішені чи ні), CLAUDE.md phase-line, README §Статус, `diagrams/`
  sync, TASKS-DONE.md. Метрик-гейт T-104 — чесний запис результату (як перенесені Ф1-гейти в Ф3).
  Closing-advisor по фактичному результату всієї фази. **Реліз — один із можливих виходів, не
  дефолт:** якщо шипнуто код → бамп `0.3.0` → `0.4.0` + тег (окремий бамп-коміт + closing-advisor
  перед пушем тега, як T-173); якщо T-104/T-106 закрили фазу без коду → окрема дрібна докс-нотатка,
  без тега. Не пре-комітити тег у плані, чиї перші два гейти можуть скасувати роботу.

**Наскрізні гейти (батч ≠ шорткат):** pure/impure розділення (voter-scope-рішення — чиста fn з
іменованими тестами; download/swap — тонка імпурна оболонка); `#![forbid(unsafe_code)]` цілий;
курований список — той самий TLS+checksum+atomic-swap, що GeoIP, ніякого нового клієнтського
механізму; `voter_scope` — populate наявний DTO (фінальна форма з Батча 4.0), не новий wire-field;
**Security-tier voters НІКОЛИ не звужуються** (T-113 — структурний, найчутливіший); дефолти 5.1
"увімкнено" / 5.1.1 "вимкнено" — підтверджені з користувачем на kickoff, не самовільно.

**Не в цих батчах:**
- **Фаза 5 (рейтинговий фільтр §5.3)** переюзує інфраструктуру топ-N по країнах цієї фази — це
  реальна крос-фазова залежність: Ф5 не стартує, доки Ф4 (принаймні Батчі 4.1–4.2) не готова.
- ccTLD-блок (§5.2) — окрема, легша задача Ф5, від курації не залежить.

**Прогрес (оновлюється по ходу):** — фазу ще не розпочато.
