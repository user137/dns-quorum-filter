...
- [ ] T-233 — **Немає жодного runtime-захисту від скомпрометованого/зіпсованого публічного
  ...
  - **Звужено 2026-09-17 (той самий "звужено, не закрито" узор, що T-51/T-56/T-221) — дві з
    ...
    - **Лишається відкритим у T-233:**
      - **PSL-фільтр записів** — архітектурний форк (і `rating_filter`, і сам `blocklist_updater`
        документують "без PSL" як свідоме рішення); користувач обрав явно відкласти, не
        бандлити +334 KB мовчки.
      - **Ін'єкція жмені легітимних доменів у великий список** — структурна межа й ratio, й
        canary, й тепер дельта-перевірки (див. advisor-catch #2 вище) — жоден із трьох gate це
        не ловить поза списком 12 canary-доменів. Не задача з номером, а стаття
