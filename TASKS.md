...
**Порядок (усе зроблено, Фаза 3 закрита 2026-09-04):** 3.0 → ~~T-101~~ → ~~3.1~~ → ~~3.2~~ →
~~3.3~~ → ~~3.4~~ → ~~3.5: T-146 + T-96~~ → ~~T-97~~ → ~~3.6~~ → ~~3.7: T-100/T-102/T-103~~ →
~~3.8: T-156/T-70~~ → ~~T-167~~. Далі — перенесені Ф1-гейти (T-66 метрики, живий Chrome-DoH-
прохід, `DEFAULT_PROVIDER_IDS`), без номера батчу/фази, user picks.
...
TASKS-DONE.md. **Наступне — T-167 або перенесені Ф1-гейти, без номера батчу/фази.**
...
закрита.** **T-167 (ревізія документації) завершено 2026-09-04.** **Нова задача T-168**
(перфоманс/навантаження, запит користувача 2026-09-05) додана в чергу, без номера батчу.
Наступне — T-168 або перенесені Ф1-гейти, user picks.
...
- [ ] T-168 — (Батч Ф3, після 3.8) **Аналіз перфомансу й стійкості до навантаження — критичні
  ділянки конвеєра + захист від локального resource-exhaustion** (виникло 2026-09-05, запит
  користувача: чи деградує сервіс плавно й прогнозовано під навантаженням, чи просто вичерпує
  ресурси й падає; чи є запобіжники, аналогічні DDoS-захисту, навіть для суто локального
  `127.0.0.1`-використання). Власний plan+advisor kickoff окремо для дизайн-рішень нижче — цей
  запис фіксує скоуп і вже звірені факти, не відповіді (TASKS.md — статус, не обґрунтування).

  Загроза — **не інтернет-DDoS** (слухає лише `127.0.0.1`), а стійкість до одного нестабільного/
  зацикленого локального клієнта чи власного багатопровайдерного fan-out під сплеском запитів.
  Якщо рішення включатиме load-shedding — наперед назвати наслідок для Три Б user-safety:
  SERVFAIL від сервісу — це саме той сигнал, на який браузер може мовчки впасти назад на власний
  DoH/plaintext резолвер (SPEC.md §3.2, той самий ризик, що обґрунтовує NULL-blocking замість
  NXDOMAIN) — дизайн має явно розглянути, чи shedding погіршує це, а не мовчки прийняти.

  **Етап 1 — аналіз складності критичних ділянок** (кандидати, звірені читанням коду):
  - `serve_until_shutdown`'s accept-loop (`main.rs:551-579`) — `tokio::spawn` на кожне прийняте
    з'єднання, без стелі; h2 (`hyper`/`hyper-util` `auto::Builder`, дефолтні налаштування) має
    власний per-connection ліміт одночасних стрімів, але **кількість з'єднань/задач нічим не
    обмежена** — звірити точне дефолтне значення h2's `max_concurrent_streams`, не припускати.
  - Ампліфікація: одночасні запити × увімкнені провайдери = одночасні вихідні `reqwest`-сокети
    (`quorum::resolve`'s `FuturesUnordered` фан-аут) — число, від якого залежить дискусія про
    файлові дескриптори, не голе число з'єднань.
  - `overrides::decision` — suffix-wildcard матчинг: лінійний скан по всіх записах на запит?
  - `moka`'s `invalidate_entries_if`-предикати (T-40) — вже задокументовано, що лишаються на hot
    path до власного sweep'у `moka`; при частих reload'ах override-списку — подивитись накопичення.
  - `query_log::LogFilter` — скан кільцевого буфера на кожен admin-запит.
  - Вже обмежене (довідково, щоб не переоцінити масштаб проблеми): `MAX_MESSAGE_SIZE` (65 535),
    `MAX_ADMIN_BODY_SIZE`, `moka`'s `max_capacity` (10 000), query_log 1000 записів/24h,
    `query_with_timeout` (2s), `UPSTREAM_CONNECT_TIMEOUT` (500ms) — пам'ять здебільшого вже
    обмежена; необмежені — задачі/з'єднання/сокети.

  **Етап 2 — перфоманс-тест**: багато одночасних запитів проти `dnsqb-service`, з **мок
  `DohClient`, не живими провайдерами** (T-133 — ToS автоматизованих запитів до Quad9/AdGuard ще
  не перевірено юридично; живий фан-аут під навантаженням дав би й недетерміновані результати).
  Ймовірна форма — `examples/`-style harness за прецедентом `examples/phase1_metrics.rs`; **не**
  блокуюча CI-джоба (ризик флейків).

  **Етап 3 — деградація й запобіжники** (дизайн-рішення, не вирішено наперед тут):
  - Плавна прогнозована деградація чи crash — виміряти емпірично на етапі 2, не припускати.
  - Чи взагалі має сенс черга очікування, чи bounded-concurrency + негайна відмова —
    `tower::limit::ConcurrencyLimit`/`load_shed` чи `Semaphore::try_acquire` (reject) проти
    `Semaphore::acquire` (черга) — порівняти з тим, як інші DNS/resolver-сервіси реагують на
    перевантаження (Unbound's `num-queries-per-thread` + jostle list, BIND's `clients-per-query`,
    dnsdist's rate limiting, PowerDNS Recursor's `max-mthreads`) — паттерн галузі: **скидати
    навантаження швидко, не будувати глибоку чергу**, бо клієнт (браузер) сам відмовиться від
    запиту за власним коротким таймаутом раніше, ніж черга до нього дійде.
  - Розмір чанка/черги — методологія, не довільне число: **корисна глибина черги ≈ пропускна
    здатність × власний клієнтський таймаут**; усе, що глибше — гарантовано застаріла робота,
    яку порахують і викинуть.

  Гейт виконання: власний plan+advisor kickoff і closing (архітектурна зміна — Agent Discipline,
  global CLAUDE.md). TASKS-DONE.md по завершенні.
