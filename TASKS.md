...
**3.11: T-173 (реліз `v0.3.0`)**. Після 3.11 Фаза 3 закрита **повністю**
...
категорійний маршрут + трей-tooltip'и; макет затверджено; TASKS-DONE.md). **Наступне —
Батч 3.11 (T-173 — реліз `v0.3.0`), і Фаза 3 закрита повністю. План — вище,
...
`/admin/ui`, атомарний `set-category-enabled`, трей-tooltip'и; макет затверджено). Наступне —
**Батч 3.11** (T-173 бамп `v0.3.0` + тег + чернетка MSIX-релізу) → Фаза 3 закрита повністю.
