...
~~3.8: T-156/T-70~~ → ~~T-167~~ → ~~T-168~~. Далі — T-169 (імплементація запобіжника з T-168)
або перенесені Ф1-гейти (T-66 метрики, живий Chrome-DoH-прохід, `DEFAULT_PROVIDER_IDS`), без
номера батчу/фази, user picks.
...
TASKS-DONE.md. **Наступне — T-169 (імплементація запобіжника з T-168) або перенесені Ф1-гейти,
без номера батчу/фази.**
...
3 коміти) — розділено: імплементація запобіжника винесена в **новий T-169**. Наступне — T-169
або перенесені Ф1-гейти, user picks.
...
- [x] T-168 — (Батч Ф3, після 3.8) **Аналіз перфомансу + навантажувальний тест + дизайн-рішення
  ...
  TASKS-DONE.md.
- [ ] T-169 — (Батч Ф3, після 3.8) **Імплементація запобіжника resource-exhaustion** (виникло
  2026-09-05, винесено з T-168). SPEC.md §1.1 — рішення й методологія розміру вже зафіксовані;
  ця задача — код: обмежена одночасність на accept-loop (`main.rs`) з негайною відмовою на
  стелі (не черга), **разом із** дедлайном на TLS-хендшейк (`tokio::time::timeout` навколо
  `acceptor.accept`) і idle/read-таймаутом на з'єднання — інакше гола стеля сама стає DoS'ом
  (slow-loris заповнює її утримуваними з'єднаннями). Окрема менша стеля на одночасні
  quorum-резолюції «у польоті». Конфіг-поле (`resolver_config.toml`, §7.1 #8-стиль — Ф3-дефолт),
  admin-видимий лічильник відмовлених з'єднань. Власний plan+advisor kickoff — тут вирішується
  відкрите питання SPEC.md §1.1 (який сигнал відмови; чи мала черга з коротким таймаутом
  безпечніша за жорсткий reject для SERVFAIL-чутливих клієнтів). Тести на 4 категорії. Якщо
  обереться `tower` — vetting тут. TASKS-DONE.md.
