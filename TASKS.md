...
- [ ] T-70 — **Windows-половина звужена — `trust_store::uninstall()` (T-49) уже є примітивом,
  ...
  **T-67 додав другий артефакт, що деінсталятор мусить прибрати:** приватний ключ у Windows
  Credential Manager (`key_store::delete_private_key`, зараз `#[cfg(test)]` — розгейтити тут).
  Залишений ключ у secure storage після видалення застосунку — той самий клас бага безпеки, що
  й залишений довірений сертифікат (SECURITY.md).
