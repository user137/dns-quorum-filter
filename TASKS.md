...
- [ ] T-141 — Дослідити підтримку HTTP/3 апстрім-провайдерами DoH (Quad9, AdGuard, Cloudflare тощо) і легкий per-upstream capability-детект патерн: пробувати HTTP/3, за відсутності підтримки — падати на HTTP/2, окремо для кожного резолвера (3.6, T-24/T-31); враховувати підтверджену вимогу Quad9 на строгий HTTP/2 (3.1) — HTTP/3-фолбек не повинен її порушити; перевірити поточний статус HTTP/3 у `reqwest`/`hyper` (станом на вибір стеку в SPEC.md "Технічний стек" підтримка була експериментальною/за окремим unstable-фічафлагом) перед оцінкою фічі
- [ ] T-145 — Плейн-текстовий конфіг у стилі ssh_config/my.cnf замість/на додачу до JSON для `resolver_config.json`/`overrides.json` — усі налаштування редаговані вручну без відкриття UI; оцінити TOML як формат (коментарі, секції, сумісний із вже вживаним serde-підходом T-144) проти INI-стилю й проти статус-кво JSON (1, T-144)
