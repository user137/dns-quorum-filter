...
T-194/T-195 з живого прогону `0.3.1` MSIX** (`58c269d` + `45aa0d6`): `certutil` без консольного
вікна (регресія T-191); «Повністю видалити» = стоп застосунку + detached wipe усієї теки
`%LOCALAPPDATA%\dns-quorum-filter` + Параметри Windows (перегляд T-70). Патч-реліз **`v0.3.1`**
...
## Батч 3.14 — кольорові трей-іконки за станом + `is_trusted` наперед + hotfix T-194/T-195
...
**Живий прогін `dist\dns-quorum-filter-0.3.1.msix` знайшов 2 баги → T-194 (`certutil` без вікна,
регресія T-191) + T-195 («Повністю видалити» = стоп + wipe теки, перегляд T-70)**, kickoff+closing
plan+advisor 2026-09-08, коміти `58c269d` + `45aa0d6` — заходять у `v0.3.1` перед тегом, `dist`-MSIX
перезбирається.
...
- [x] T-195 — **зроблено 2026-09-08**, коміт `45aa0d6`. «Повністю видалити» чистило cert + 3
