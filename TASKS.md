...
- **Батч 4.3 — клієнтський рейтинг-фільтр: конвеєр + поведінка + тести (T-124, T-125, T-126,
  T-129, T-130, T-131)**: сам крок конвеєра. Функція членства бере **N-арний `&[ZoneSource]`**
...
- **Батч 4.2 — курація зон: державні + науково-освітні (T-122, T-123)**: ще два `ZoneSource`
  ...
  глобальний ручний список науково-освітніх/некомерційних, версіонований, з changelog. **Після
  4.3/4.4** — редакційний процес потенційно людино-блокований, не має стояти між конвеєром
  курації й фічею, що його споживає. Батч доставляє механізм + процес + мінімальний seed;
  наповнення лишається людині (фіксується в пауза-звіті). Розвилки: PR-шаблон / CODEOWNERS /
  `ZONES-CHANGELOG.md`.
- **Батч 4.5 — персональне локально навчене джерело зон (T-138, §5.1.1)**: ще один `ZoneSource`.
  Навчання частоти ∩ регулярності користувач-навігацій; сигнал — **лише вже-`ALLOW`-вердикти
  кворуму**. Окреме **опційне (дефолт вимкнено)** шифроване сховище — **не** `persistence-key`
  T-96/T-97, окремий ключ (4-й секрет `key_store`). Тільки **додає** домени. Дзеркало
  `log_persist`/`cache_persist` (pure core + тонка імпурна оболонка, `write_atomic`,
  `encrypted_file`). Оновити `SERVICES.md` + `SECURITY.md` (нове сховище browsing-derived даних).
  Останній — доповнює вже робочу «бульбашку», найвища приватнісна планка, ізольований.
...
- [ ] T-107 — Конвеєр курації: fetch чисто-ліцензованого per-country топ-N рейтингу (CrUX CC BY 4.0
  ...
  клієнт (принцип MaxMind). Згортає стару T-121. (5.3)
- [ ] T-108 — Гігієна списку на етапі курації: не включати в зону домен, що явно в
  Security-блоклисті на момент курації, **і** відфільтрувати очевидний adult/gambling-контент
  (T-104 показав, що per-country топ-N його містить) — інакше бульбашка пропускає ці домени повз
  рейтинг-фільтр (5.3)
- [ ] T-180 — (Батч 4.1) Рядок атрибуції джерела топ-N у `<footer id="credits">` `/admin/ui`
  (CrUX CC BY 4.0 — провідний кандидат; юридично обов'язково, окрема задача як T-81 для DB-IP);
  фінальне формулювання після вибору джерела в 4.1 (5.3, Наскрізні вимоги)
- [ ] T-120 — Внутрішньо-Ф4 гейт: курація топ-N готова й обкатана перед клієнтським wiring
  рейтинг-фільтра (T-124) (5.3)
- [ ] T-122 — Курація державних доменів: евристичне кандидування за TLD-патерном (`*.gov`,
...
- [ ] T-124 — Реалізувати рейтинговий фільтр як останній локальний крок конвеєра (після
  Allowlist, Blocklist, ccTLD, Cache; перед Quorum) — чиста scope-функція «домен у зоні?» +
  тонка імпурна оболонка; `decision_source = RATING_FILTER` (5.3)
- [ ] T-125 — Поведінка — лише BLOCK для доменів поза зонами, ніколи force-ALLOW для доменів
  у зоні (5.3)
...
- [ ] T-127 — UI: окремий візуально виділений перемикач з явним попередженням при увімкненні
  (не звичайний checkbox поруч з іншими), імовірно в `<details>` advanced (T-176) (5.3)
- [ ] T-128 — UI: обов'язковий, завжди видимий індикатор активності рейтингового фільтра (5.3, 8)
- [ ] T-129 — Юніт-тест: домен поза зонами блокується без звернення до quorum-моків (перевірка
  виклику, не лише вердикту) (5.3, Наскрізні вимоги)
- [ ] T-130 — Юніт-тест: домен у зоні не отримує force-ALLOW, продовжує звичайний конвеєр
  (регресія на «лише BLOCK») (5.3, Наскрізні вимоги)
- [ ] T-131 — Юніт-тест: user-allowlist працює як виняток навіть з увімкненим рейтинговим
  фільтром (5.3, Наскрізні вимоги)
- [ ] T-138 — Персональне локально навчене джерело зон бульбашки (§5.1.1 переформульовано,
  T-179): локальне навчання частоти + регулярності відвідувань, джерело — лише вже-`ALLOW`-вердикти
  кворуму; окреме (не T-96/T-97) опційне (дефолт вимкнено) шифроване сховище; тільки додає домени
  до зон, нічого не блокує й не розблоковує (5.1.1)
