...
- **Батч 4.6 — закриття Ф4**: бамп `0.3.0` → `0.4.0` (як T-173) + окремий docs-коміт +
  closing-advisor + тег `v0.4.0` → `release.yml` draft. `sinkhole_probe` + `curate_topn` —
  прогнати перед тегом. CLAUDE.md phase-line → «Фаза 4 закрита»; далі Фаза 5.

**Порядок:** ~~4.1~~ → ~~4.3~~ → ~~4.4~~ → ~~4.2~~ → ~~4.5~~ → **4.6**. Один батч за раз, кожен — власний kickoff за
