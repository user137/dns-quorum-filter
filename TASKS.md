...
план", TASKS-DONE.md. T-217/T-218 (тепер під Батч 4.7 нижче) лишаються відкритим бекложем — не блокують закриття Фази 4.

### Батч 4.7 — Позапланові знахідки під час смоук-тесту v0.4.0 + приєднаний беклог (не MSIX-специфічні, крім T-219)

**Перепланування 2026-09-12:** до знахідок смоук-тесту (T-219–T-229) приєднано чотири задачі зі старого бекложу («Поза фазами»), що торкаються тієї самої поверхні (`/admin/ui`, рейтинг-фільтр «бульбашка») — T-217, T-218, T-159, T-136 — щоб не вести дві паралельні купи дрібних UI/конфіг-задач по тому самому коду. Порядок: **4.7.A (дрібні точкові фікси, без окремого plan+advisor циклу) → 4.7.B (потребують власного plan+advisor циклу — security-sensitive / архітектурні) → 4.7.C (великі фічі чи рішення поза кодом, окремий kickoff, дизайн не почато)**. Кожна задача — свій коміт; пауза й звіт між задачами, той самий каданс, що й решта сесії.
...
- [ ] T-224 — **Знайдено 2026-09-12, під час огляду `/admin/ui` користувачем.** Кнопка
  «В allowlist»/«В blocklist» у рядку лога (`main.js:1252-1274`, T-46) на успіх ставить
  `btn.disabled = true` і `btn.textContent = "✓ Додано"` (`main.js:1257-1267`), але жодна
  дія не скидає це назад. Список overrides (`#overrides-body`) — окремий цикл
  fetch/render, не на 2с-опитуванні (`main.js:1104`); видалення того самого домену через
  «Видалити» в списку overrides викликає лише `refreshOverrides()`, ніколи
  `refreshLog()` (`main.js:1466`). Результат: після видалення запису зі списку кнопка в
  рядку лога лишається мертвою («✓ Додано», disabled) — користувач не може повторно
  додати той самий домен з лога, доки не переробить пошук/фільтр лога вручну (що
  перебудовує рядки з нуля). Дрібний, чітко локалізований фронтенд-фікс: або (a) не
  лишати `disabled=true` на success-шляху, або (b) викликати `refreshLog()` теж після
  успішного add/remove. Не MSIX-специфічний, не потребує окремого plan+advisor циклу
  (точковий UI-фікс, не архітектурна зміна).
- [ ] T-225 — **Запит користувача 2026-09-12.** Прохання зробити `chrome://settings/security`
  (`index.html:72`, `<code id="chromium-settings-url">`) клікабельним посиланням, що саме
  відкриється, а не текстом для копіювання вручну. **Технічно неможливо як прохання
  сформульоване:** Chrome блокує навігацію на `chrome://`-URL з `<a href>` звичайної
  сторінки (захист від фішингу зі сторонніх сайтів) — це поведінка самого браузера,
  незалежно від реалізації посилання. Реалістична заміна — кнопка «Скопіювати» поруч із
  вже наявним `<code>` (clipboard API, немає потреби у новій залежності). Дрібний UI-фікс,
  не потребує plan+advisor циклу.
- [ ] T-226 — **Запит користувача 2026-09-12, дві частини.** (а) Підтверджено: при пошуку
  GeoIP-країни немає підсказки/випадного списку з назвою країни — лише сирий
  ISO-код, перевірений `validate_country_code` (`dispatch.rs:2328`, `config.rs`); сама назва
  ніде в UI не показується. Дрібний, реалізовний UI-гап. (б) Прохання використовувати
  сторонній GitHub-мірор (`P3TERX/GeoLite.mmdb`) як дефолтне джерело замість/на додачу до
  DB-IP Lite. **Не UX-рішення — питання довіри до джерела даних, потребує окремої
  оцінки перед вибором:** SECURITY.md уже класифікує GeoIP-фіди як дані, що впливають на
  рішення блокування, з вимогою atomic-swap + integrity-check саме тому, що це шар поза
  контролем проєкту (Три Б, «lower-layer safety»); GeoLite2 має ліцензійні обмеження на
  редистрибуцію (MaxMind EULA), і сторонній мірор — додатковий supply-chain-ризик, не
  покритий поточними рядками `SECURITY.md`/`deny.toml`. Рішення (чи взагалі, і якщо так —
  як дефолт чи як опція) — за користувачем/окремим review, не одноосібний вибір
  виконавця задачі.
- [ ] T-228 — **Запит користувача 2026-09-12.** Коли мережа офлайн (`AdminStatusResponse.
  network == OFFLINE`), елементи `/admin/ui`, що вимагають негайного завантаження (пікер
  списків бульбашки, MaxMind-перевірка ключа, будь-яка кнопка «оновити зараз»), мають
  блокуватися/показувати причину, а не мовчки провалюватись. **Підтверджено:** у
  `main.js` немає жодної перевірки офлайн-стану для таких контролів — лише
  hero-іконка вгорі показує OFFLINE, решта форми лишається активною. Дрібний, чітко
  локалізований UI-фікс (читати вже наявне поле статусу, `disabled`+тултip на
  відповідних контролах), не потребує plan+advisor циклу.
...
- [ ] T-220 — **Знайдено 2026-09-12, НЕ виправлено.** `trust_store.rs`'s `NOT_FOUND_EXIT_CODE:
  i32 = 17` (T-49, "confirmed empirically" через POSIX-truncating шелл) не відповідає реальному
  коду, який бачить сам скомпільований Windows-білд: `certutil -store` на порожній збіг завершується
  з `0x80090011`/`-2146893807` (`ExitStatus::code()` повертає це значення без відтинання), не `17`
  (`17 = -2146893807 mod 256`, POSIX-шеллове відтинання). `confirmed_thumbprints_for_common_name`
  (єдиний спосіб `uninstall_loop` підтвердити "стор порожній") тому ЗАВЖДИ повертає
  `Err(ListFailed(...))` на завершальній ітерації — тобто **`uninstall()` не може повернути `Ok(())`
  у реальності НІКОЛИ**, навіть якщо щойно успішно видалив усі відповідні записи. Блок-радіус:
  `cert_rotation::rotate_certificate` (живо відтворено — трей-сценарій "Перевипустити сертифікат"
  провалився з "could not confirm whether any matching certificates remain" при справді порожньому
  стор) і `local_state::remove_all`→`remove_cert` ("Повністю видалити", `POST
  /admin/uninstall-local-state" — сертифікатний артефакт репорту завжди дасть `Failed`, навіть коли
  видалення реально відбулось). **Не MSIX-специфічний і не новий у v0.4.0** — той самий код шлях у
  всіх попередніх релізах з часів T-49. **Найчистіше живе підтвердження (сценарій 23 смоук-тесту,
  2026-09-12):** під час реального «Повністю видалити» на цей самий `-delstore` виклик з'явився
  нативний Windows CryptUI-діалог («ВИДАЛИТИ наведений сертифікат із кореневого сховища
  сертифікатів?») — підтверджено вручну («Так»), сертифікат РЕАЛЬНО видалено з `CurrentUser\Root`.
  Одразу після цього звіт `remove_all` показав «Сертифікат: НЕ ВДАЛОСЯ видалити» — прямий доказ,
  що код репортує `Failed` навіть коли видалення щойно підтверджено власноруч. (Сам факт появи
  цього CryptUI-діалогу — окреме спостереження, не нова знахідка: одна машина, немає порівняльного
  випадку, встановлення вже видалено — не перевірити повторно цієї сесії; лишити для того, хто
  візьметься за фікс T-220.) Живий стан машини не потребує відновлення — застосунок повністю
  видалено в рамках цього ж сценарію 23. Потребує власного plan+advisor циклу перед фіксом
  (security-sensitive код без автоматизованого покриття реального стора).
...
- [ ] T-222 — **Знайдено 2026-09-12, НЕ виправлено. `self_uninstall.rs`'s app-data wipe-хелпер —
  ...
  (ЛОГІЧНИЙ шлях `%LOCALAPPDATA%\dns-quorum-filter`) і вставляє його в PowerShell-скрипт,
  спавнений як окремий, НЕПАКОВАНИЙ `powershell.exe`-процес (`DETACHED_PROCESS |
  CREATE_BREAKAWAY_FROM_JOB`). MSIX-віртуалізація `%LOCALAPPDATA%` прозора лише для самого
  пакованого процесу — непакований дочірній процес бачить логічний шлях як буквально
  НЕІСНУЮЧИЙ (`Test-Path 'C:\Users\<user>\AppData\Local\dns-quorum-filter'` → `False`,
  перевірено емпірично з непакованого шелу на реальній машині). Скрипт-хелпер
  (`build_wipe_script_for`) сам це маскує: `while ((Test-Path -LiteralPath '{quoted}') -and $j
  -lt 30) { Remove-Item ... }` — умова хибна на першій же ітерації, тож `Remove-Item` НІКОЛИ не
  виконується жодного разу, цикл миттєво завершується, нічого не видаливши, і НЕ повідомляє про
  це — спав пройшов успішно (`Ok(_child)`), тож лог каже лише `"app-data directory wipe
  scheduled"`, без жодного натяку на провал. **Модульний doc-коментар `self_uninstall.rs`
  (рядки 4-12) явно називає цей хелпер ЄДИНИМ механізмом стирання `%LOCALAPPDATA%\dns-quorum-
  filter`** («no DNS-QF process can delete that directory while running... So a detached helper
  does it») — це не резервний рівень захисту, а єдиний задуманий шлях, і він структурно
  непрацездатний на пакованому білді. **Чому валідація (`build_wipe_script_for`'s власна
  перевірка `dir.starts_with(localappdata)` + `file_name() == "dns-quorum-filter"`) це не
  ловить:** логічний шлях справді добре сформований і справді лежить під `%LOCALAPPDATA%` —
  проблема не у формі шляху, а в тому, що він нерозв'язний у просторі імен дочірнього процесу;
  ніяка перевірка форми шляху цього не виявить. **Живо підтверджено 2026-09-12 (сценарій 23
  смоук-тесту):** після повного «Повністю видалити» через трей — лог показав `"app-data
  directory wipe scheduled"`, але реальний фізичний каталог (26 файлів: `cert.pem`,
  `resolver_config.toml`, `geoip.mmdb`, `logs/`, `topn/`, і — оскільки в цьому прогоні сценарій
  22a вручну увімкнув `persist_query_log`/`persist_cache`/`[personal_zone]` — також
  `query-log.enc`/`cache.enc`/`personal-zone.enc`) лишався АБСОЛЮТНО незмінним ще довгий час
  (60+ с) після завершення хелпера. **Важливе уточнення щодо серйозності (advisor-catch, не
  перебільшувати):** на дефолтному встановленні (`persist_query_log`/`persist_cache` = `false`
  за замовчуванням) залишковий набір файлів НЕ містить доменних імен — це `cert.pem` +
  `resolver_config.toml` + `logs/` + `geoip.mmdb` + `topn/`, жодних персональних даних;
  експозиція реальна, але УМОВНА — залежить від того, чи користувач сам вмикав шифровану
  персистенцію (T-97/T-146/T-138). **Чому це НЕ спричинило видимий FAIL у цьому прогоні:**
  Windows' власний AppX-механізм видалення пакета (натискання «Видалити» в «Параметрах») окремо
  й повністю стер увесь `Packages\dns-quorum-filter_8d78tvs37tgae\LocalCache\...` — підтверджено
  емпірично (каталог зник одразу після цього кроку). Це не задумана резервна схема, а випадковий
  побічний ефект AppX-видалення пакета — сам хелпер лишається непрацездатним і не виконав би
  жодної роботи, якби застосунок був непакованим dev-білдом (де AppX-видалення не існує
  взагалі) або якби користувач не довів видалення пакета до кінця в «Параметрах» одразу.
  **Тест `wipe_script_waits_and_bails_before_it_deletes` не ловить це** — той самий клас
  прогалини, що вже задокументований у CLAUDE.md («a test that passes today doesn't prove the
  property its name claims»): тест перевіряє лише ПОРЯДОК підрядків у згенерованому скрипті
  (wait → bail → delete), ніколи не перевіряє, чи шлях взагалі розв'язний. Потребує власного
  plan+advisor циклу перед фіксом (той самий клас, що й T-219: потрібен спосіб дати
  непакованому дочірньому процесу реальний фізичний шлях, а не логічний — можливо, той самий
  прийом, що T-219 вже застосував для `-addstore`, або передача вже розв'язаного фізичного
  шляху як аргументу скрипту замість повторного покладання на віртуалізацію).

- [ ] T-223 — **Запит користувача 2026-09-12, після живого тесту MSIX-оновлення (не баг коду,
  наразі — платформна поведінка).** `Add-AppxPackage`/будь-який MSIX-деплой на нову версію
  відмовляє з `0x80073D02` («need to be closed»), поки хоча б один із трьох процесів
  (`dnsqb-service`/`-tray`/`-watcher`) живий — Windows не закриває класичний Win32-в-MSIX
  застосунок автоматично під час install-оновлення (на відміну від типової UWP-поведінки),
  живо підтверджено сценарієм 24 смоук-тесту: оновлення 0.4.0→0.4.1 успішне лише ПІСЛЯ
  ручного «Вийти з DNS Quorum Filter» через трей. **Мета задачі (сформульована користувачем):
  оновлення має бути прозорим для користувача і максимально тихим** — без ручного закриття
  через трей, без видимого збою деплою. **Напрямок, вартий дослідження (НЕ спроєктовано,
  потребує власного plan+advisor циклу):** `Windows.ApplicationModel.PackageCatalog`'s
  `PackageUpdating`/`PackageInstalling` WinRT-події — desktop-bridge (Win32-в-MSIX) процес
  теоретично може на них підписатися (має package identity) і у відповідь ініціювати
  той самий graceful-shutdown шлях, що вже є в `dnsqb-tray`'s «Вийти»
  (`stop.flag`+`quit.flag` → `run_query_log_persister`'s flush → чистий вихід), без запиту
  користувача. **Невідомо, потребує підтвердження перед проєктуванням:**
  (a) чи ці події справді надійно приходять desktop-bridge Win32-процесу, а не лише
  UWP-застосункам — не перевірено емпірично; (b) скільки часу є на реакцію до того,
  як деплой або продовжить, або зафейлиться повторно; (c) хто саме підписується —
  `dnsqb-watcher` (уже центральний процес життєвого циклу) чи `dnsqb-tray`. Альтернатива,
  якщо (a) не підтвердиться: документований у README/SERVICES.md крок «перед оновленням
  застосунок сам вийде за K секунд бездіяльності» чи подібний тайм-аут-механізм —
  менш «тихий», але реалізовний без нових WinRT-залежностей.
- [ ] T-227 — **Запит користувача 2026-09-12.** Бажана поведінка бульбашки: (1) дефолтний
  список країни — з країни встановлення, не фіксований набір; (2) завжди доступний
  преналаштований набір «глобальний, науковий/освітній тощо» зверху списку вибору; (3)
  можливість пошуку й **лінивого** (lazy, за запитом) довантаження списку іншої країни,
  якщо його ще немає локально, з повідомленням користувачу про факт довантаження.
  **Знахідка під час перевірки:** `ZoneSourceKind` уже має варіанти `GovernmentTopN(cc)` і
  `SciEdu` як частину закритого enum (реалізовані в коді, CLAUDE.md's `rating_filter` row),
  але **жоден із них не має реального розповсюдженого датасету** —
  `topn_download::AVAILABLE_TOPN_LISTS` = лише `ua`/`us`/`de`/`pl`/`gb`/`global`
  (`CountryTopN`/`Global` тільки). Тобто науково-освітній пункт (2) — не написаний код,
  а відсутні дані для вже готового коду. **Адаптивність (1) + lazy-download (3) — це зміна
  архітектури дистрибуції**, не фронтенду: зараз `AVAILABLE_TOPN_LISTS` — контракт
  дистрибуції (видалення коду з нього — breaking change, неопублікований код — навмисний
  `400`, `config.rs::validate_rating_filter_lists`), а курація (`curate_topn` example,
  `topn-curate.yml`) — офлайн `workflow_dispatch` → артефакт → людський PR, не
  користувацький runtime-тригер. Дозволити користувачу самому тригерити довантаження нового
  коду в реальному часі — новий, ще не спроєктований шлях (де відбувається курація? з чиєї
  машини? як перевірити інтегритет без наявного `.sha256`-сайдкара заздалегідь?).
  Потребує власного plan+advisor циклу перед проєктуванням — не точковий фікс.
- [ ] T-229 — **Запит користувача 2026-09-12.** Прохання: постійний знак оклику на
  трей-іконці, якщо застосунок встановлено, але браузер довго не вказаний на нього як
  DoH-провайдер (жодних запитів), з поясненням у тултипі. **Конфліктує з уже прийнятим
  правилом проєкту:** «An always-on warning is functionally identical to no warning»
  (Три Б, SPEC.md §8.1; CLAUDE.md «Recurring patterns», прецеденти T-56/T-57/T-77) — сталий
  бейдж-до-налаштування-браузера саме та форма, яку це правило забороняє. У проєкті вже є
  подієвий механізм для точно такого випадку — `onboarding`-модуль (once-per-process latch
  + `onboarding.seen`-маркер, `dnsqb-tray`) — а не постійний оверлей чи нова іконка.
  Рекомендація (не остаточне рішення): одноразове сповіщення замість беджа; детектувати
  «нуль запитів за вікно» дешево через лог/`AdminStats`, а нову п'яту іконку в
  `gen-icon.py::make_tray_glyph` додавати лише якщо одноразового сповіщення визнають
  недостатнім. Потребує власного plan+advisor циклу (конфлікт з чинним правилом, а не
  чистий бачинг-фікс) і рішення користувача, чи приймати компромісний варіант. **Перехресне
  посилання (2026-09-12, аудит бекложу):** T-56 (Ф1) хоче саме такий сигнал для іншої мети
  (статусний індикатор, не разове сповіщення) і застрягла на T-134's неперевіреному
  canary-механізмі — «нуль запитів за вікно» тут не відрізняє «браузер не налаштовано» від
  «машина простоює», тож не є чистим розв'язанням T-56, лише спільним кандидатом для kickoff'у.

#### 4.7.C — великі фічі / рішення поза кодом, окремий kickoff (дизайн не почато)

- [ ] T-218 — **Перенесено з «Поза фазами» 2026-09-12.** **Запропоновано користувачем 2026-09-11,
...
- Примітка: рішення щодо джерела GeoIP-даних (T-226, частина «б» — сторонній мірор GeoLite2 vs. лише DB-IP Lite/офіційний MaxMind) — теж рішення поза кодом, того самого класу, що й T-136/T-218 вище; сам T-226 лишається в 4.7.A, бо його UI-частина (а) — точковий фікс.
...
- [ ] T-135 — Постійний редакційний процес рев'ю державного та науково-освітнього списків — триває й після релізу, не одноразова задача (Фазований план Фаза 5, Відкриті питання п.8). **Перехресне посилання (2026-09-12, аудит бекложу):** T-227 (Батч 4.7.B) знайшла, що `GovernmentTopN`/`SciEdu` мають готовий код (`ZoneSourceKind`), але жодного розповсюдженого датасету — T-135 передбачає процес рев'ю ЩЕ ДО того, як цей датасет узагалі з'явиться; T-227 — саме та задача, що має його породити.
