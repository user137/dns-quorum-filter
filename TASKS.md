...
4. **Батч 5.4** — решта сайту (T-151, той самий механізм з 5.2) — мультикомітний, один розділ
   `main.js` = один комміт. **Коміти 1-4/~14 (`#app-body`+hero, `#timeout-config-body`,
   `#overrides-body`, `#cache-config-body`) зроблено 2026-09-20** — деталі, advisor-catch'і,
   живий-баг у `renderError()`, спільні `common.*`-ключі — TASKS-DONE.md. Решта коммітів:
   cctld-block, geoip+maxmind, log, providers, filter-controls, browser-setup, danger-zone,
   rating-filter, blocklist-bundles, footer.
