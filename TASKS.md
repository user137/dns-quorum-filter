...
~~T-169~~ → **3.9: ~~T-170~~/~~T-171~~/~~T-174~~/~~T-175~~/~~T-172~~ (лишився closing-advisor батча)** → **3.10: T-173**. Після 3.10 Фаза 3 закрита **повністю**
...
- **Батч 3.9 — закриття відкритих Ф1-гейтів (T-170, T-171, T-172).** Спільна тема = чесно
  ...
  **Спінофи в батчі (за запитом користувача):** T-174 (фікс сигнатури CleanBrowsing → гіпотезу
  ...
  `ERR_ADDRESS_INVALID` + корельовані `BLOCKLIST`-рядки). Лишився тільки closing-advisor на весь
  батч 3.9.
...
отримав секцію «Перевірка: браузер → локальний DoH»). **Наступне — closing-advisor на весь Батч 3.9
→ Батч 3.10 (T-173 — реліз `v0.3.0`), і Фаза 3 закрита повністю. План — вище, «План фінального
закриття Ф3».**
...
локальний DoH»). Наступне — closing-advisor на весь **Батч 3.9** → **Батч 3.10** (T-173 бамп
`v0.3.0` + тег + чернетка MSIX-релізу) → Фаза 3 закрита повністю.
...
- [ ] T-176 — **UX-ревізія `/admin/ui` для нетехнічного користувача** (запит користувача
  2026-09-06; мотив: «програма висить як сервіс, звичайній домогосподарці це не зрозуміло»).
  Дві частини:
  - **Інформаційна архітектура сторінки керування.** Розділити на «просте / розширене»:
    базовий вигляд — увімк/вимк фільтрації, стан («захищено / не захищено»), 2–3 перемикачі
    категорій (malware / ads / adult); усе інше (провайдери поштучно, `timeout_mode`, GeoIP,
    cache TTL, `[limits]`, персистентність, cert-дії) — за прогресивним розкриттям
    (акордеон / випадаюче підменю «Розширені»). Оновити `UI-SPEC.md` §2 (інвентар) + §3
    (поля по екрану, позначити basic/advanced) + `mockups/`.
  - **Помітність запущеного застосунку.** Нетехнічний користувач має бачити «працює й
    захищає» без термінології: трей-іконка присутня + зрозумілий tooltip, Start-menu плитка
    (MSIX, T-156 — вже є, перевірити текст/іконку), простий рядок статусу. Онбординг:
    перший запуск має привести до сторінки конфігурації браузерного DoH (README §"Швидкий
    старт" крок 4 — нині суто текст).
  **Порадитися з дизайнером і отримати від нього рев'ю** (макети + IA) до імплементації —
  це UI-структурна зміна, plan+advisor gate на обох кінцях. Імплементація (HTML/CSS/JS у
  `include_str!`, без бандлера — як зараз) — окремим кроком після затвердження макета.
