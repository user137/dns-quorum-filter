...
- [ ] T-51 — **Звужено 2026-08-29**: Chrome-половина підтверджена емпірично (проба вище — `Root`,
  ...
  сертифікат навіть не потрапляє в Firefox's власну базу.
...
- [ ] T-157 — **Аналіз** (не імплементація) можливості переходу на системний DNS усього ПК через
  ...
  лише браузер — некрashity-safe одноточкова відмова (Три Б, user safety: якщо `dnsqb-service`
  ...
  теоретично дозволяє fallback-правило), (3) чи потрібен новий, не-DoH prото-хендлер (`dispatch.rs`
  ...
  можливий на Windows, чи це лишається відкритим ризиком. Поза межами MVP, дослідницька задача, той
  самий прецедент "research, not implementation", що й T-134/T-141.
