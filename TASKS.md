...
- [ ] T-53 — Явний allowlist адмін-каналу + DTO замість прямої експозиції внутрішніх структур бекенду (8) — **референт змінився T-149**: Tauri-команди більше не існують (`dnsqb-ui` видалено, DECISIONS.md); поверхня тепер — HTTP-маршрути `dispatch::serve`'s `match` (`/admin/status`/`/admin/config`/`/admin/reset`/`/admin/shutdown`/`/admin/ui*`), вже покриті власними тестами `dispatch.rs`, але без формального, окремо задокументованого рев'ю поверхні
...
- [ ] T-55 — CSP без `unsafe-inline`/`unsafe-eval` (8) — **мета вже досягнута інакше, T-149**: вбудований `/admin/ui` з самого початку несе `Content-Security-Policy: default-src 'self'; frame-ancestors 'none'`, без жодного inline script/style; колишнє `dnsqb-ui`'s (Tauri) CSP-обмеження, яке ця задача мала виправити, видалено разом із самим `dnsqb-ui`
- [ ] T-56 — Індикатор стану: чи браузер використовує локальний DoH, чи фільтрація активна, деградовані апстріми, окремий стан "фільтрацію вимкнено — 0 voters" (8, 3.3, 8.1) — `dnsqb-tray`'s tooltip (T-149) — простіший 3-станний попередник, не заміна: без перевірки браузера/watchdog/degraded, ціль лишається веб-UI чи повноцінному майбутньому екрану
- [ ] T-57 — Явне повідомлення в UI про приватність кворуму (N провайдерів бачать нову історію браузингу) (8, Відкриті питання п.4)
- [ ] T-58 — Тести UI↔бекенд каналу за 4 категоріями: smoke, злам/exploit, misuse, fuzz (`proptest`) (8.1) — канал тепер HTTP (T-149), не Tauri IPC; `dispatch.rs`'s наявні тести покривають частину smoke/exploit-категорій за фактом (CSRF-гейт, розмір тіла), формального проходу за всіма 4 категоріями явно не було
- [ ] T-59 — Snapshot-тест на allowlist експонованих маршрутів адмін-каналу (8.1, Наскрізні вимоги) — **референт змінився T-149**: не Tauri-команди, а `dispatch.rs`'s `match`-роутинг
...
- [ ] T-150 — Інсталятор реєструє автозапуск `dnsqb-service` (Startup-ярлик/Run-ключ на Windows) — зафіксовано під час T-149's plan-mode рев'ю: трей (`dnsqb-tray`) ніколи не володіє `dnsqb-service` як дочірнім процесом (навмисно, щоб не прив'язувати headless-ціль, п.12, до графічного трей-супервайзера), тож без цього мишею-only користувач після встановлення пакету не має способу запустити сервіс без терміналу (SPEC.md "Відкриті питання" п.13, DECISIONS.md)
...
- [ ] T-139 — UI: **звужено після T-52** — лічильники (оброблено/заблоковано) вже показуються в дашборді вбудованого веб-UI (`admin::compute_stats`, живцем із логу, без нового сховища; колишній `dnsqb-ui` показував те саме до T-149) — залишок цієї задачі — лише відсоток заблокованого, поки не відображений (6, 8)
