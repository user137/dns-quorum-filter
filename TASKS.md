...
4. **Батч 5.4** — решта сайту (T-151, той самий механізм з 5.2) — мультикомітний, один розділ
   `main.js` = один комміт. **Коміти 1-3/~14 (`#app-body`+hero, `#timeout-config-body`,
   `#overrides-body`) зроблено 2026-09-20** — деталі, advisor-catch'і, живий-баг у
   `renderError()`, `common.helpAriaLabel`/`common.delete`-побічні ефекти — TASKS-DONE.md. Решта
   коммітів: cache-config, cctld-block, geoip+maxmind, log, providers, filter-controls,
