...
- [ ] T-229 — **Запит користувача 2026-09-12.** Прохання: постійний знак оклику на
  ...
  **Прогрес 2026-09-13:** користувач прийняв рекомендацію (одноразове сповіщення, не бейдж) і
  додатково уточнив: сповіщення має бути невелике, спливаюче біля трею, не на весь екран і не
  по центру — це виключає `rfd`'s центровані нативні діалоги (`onboarding`-модуль). Дослідження
  рендер-варіантів показало обидва дорожчі за саму задачу: (a) Windows balloon tip
  (`Shell_NotifyIcon`/`NIF_INFO`) — `tray-icon` 0.21's `TrayIcon::window_handle()` публічний, але
  приватний `uID`, під яким існуючу іконку зареєстровано, ні — тож balloon не можна причепити до
  вже наявної іконки без окремої, другої `NOTIFYICONDATA`-реєстрації; це або новий зовнішній
  crate, або власний `unsafe`-код (проєкт — `#![forbid(unsafe_code)]` всюди) — обидва потребують
  SECURITY.md-рядка чи DECISIONS.md-запису, не просте розширення T-229. (b) кастомне `tao`-вікно
  біля трею — потребує нової рендер-залежності (малювання тексту), якої в цьому крейті зараз
  немає. **Зроблено (детекційна половина, без рендера):** новий `dnsqb-tray/src/browser_nudge.rs`
  — чисті `should_offer_browser_nudge`/`first_seen`/`mark_first_seen_if_absent`/
  `browser_nudge_seen` (дзеркалить `onboarding.rs`'s форму), поріг 30 хв від першого запуску,
  9 нових тестів; підключено в `main.rs` (`maybe_offer_browser_nudge`, per-process latch,
  **не** пише постійний `browser-nudge.seen` — щоб не спалити єдиний живий постріл до вибору
  рендера), поки що лише `tracing::info!` замість реального сповіщення. Повний workspace-гейт
  зелений; CodeQL на HEAD (`66d701e`+цей коміт) — 0 відкритих алертів. **Відкрито:** рендер-вибір
  (a)/(b) — рішення користувача, не техпитання.
