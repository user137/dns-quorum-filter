...
(перезібраний із T-194/T-195) користувач робить разом із тегом `v0.4.0` (Батч 4.6, DECISIONS.md
2026-09-11 — T-190 більше не тегується окремим `v0.3.2`, злитий у цей реліз). **Батч 3.13
(T-188–T-190 + T-193) — зроблено 2026-09-08:** T-188 (`4cb9ed9`) + T-189 (`98ac6c8`/`a5489ef`) +
T-193 (онбординг: `GET /admin/cert-status` + `POST /admin/install-cert` + майстер трея `rfd` +
hero cert-гілка + per-браузер картка; **T-193** — трей-пауза більше не вбиває DNS, служба віддає
нефільтрований baseline); T-190 (бамп + closing-advisor + MSIX + тег) закрито Батчем 4.6 під
тегом `v0.4.0`, не окремим `v0.3.2`. TASKS-DONE.md.
...
HTTP-маршрутів + hero-стану. **Тегається як `v0.4.0` разом із Фазою 4 (Батч 4.6), не окремим
`v0.3.2`** — рішення користувача, DECISIONS.md 2026-09-11 (переглядає план нижче). Порядок
комітів: T-188 → T-189 → T-190. DECISIONS.md 2026-09-08.
...
план", TASKS-DONE.md. T-217 (тепер під Батч 4.7 нижче) лишався відкритим бекложем — не блокував
закриття Фази 4; T-218 з 2026-09-13 живе під власною `## Фаза 7` нижче (kickoff, DECISIONS.md).

### Батч 4.7 — Позапланові знахідки під час смоук-тесту v0.4.0 + приєднаний беклог (не MSIX-специфічні, крім T-219)

**Перепланування 2026-09-12:** до знахідок смоук-тесту (T-219–T-229) приєднано чотири задачі зі старого бекложу («Поза фазами»), що торкаються тієї самої поверхні (`/admin/ui`, рейтинг-фільтр «бульбашка») — T-217, T-218, T-159, T-136 — щоб не вести дві паралельні купи дрібних UI/конфіг-задач по тому самому коду. Порядок: **4.7.A (дрібні точкові фікси, без окремого plan+advisor циклу) → 4.7.B (потребують власного plan+advisor циклу — security-sensitive / архітектурні) → 4.7.C (великі фічі чи рішення поза кодом, окремий kickoff, дизайн не почато)**. Кожна задача — свій коміт; пауза й звіт між задачами, той самий каданс, що й решта сесії. **T-218 з 2026-09-13 вийшов із цього батчу у власну `## Фаза 7`** (kickoff завершено — 8 звірених джерел, конвеєр-рішення, перший дизайн-артборд; DECISIONS.md 2026-09-13) — 4.7.C нижче лишається лише як історична нотатка.

#### 4.7.A — дрібні точкові фікси (без окремого plan+advisor циклу)
...
#### 4.7.C — superseded 2026-09-13: T-218 перенесено у власну `## Фаза 7` нижче

**Цей підрозділ більше не веде T-218** (CLAUDE.md: "не депрекувати мовчки" — заголовок і
попередній розгорнутий аналіз лишаються нижче для історії, самé рішення й перенесення — у
`## Фаза 7`, DECISIONS.md 2026-09-13). Розгорнутий аналіз джерел, три "рішення користувача"
питання, кандидатний список — усе, що було тут, тепер під T-218 у Фазі 7, доповнене звіркою з
першоджерелами й фінальним 8-джерельним набором.

<details><summary>Історичний запис до kickoff'у 2026-09-13 (розгорнути)</summary>

- T-218 (історичний запис, живий чекбокс тепер лише у `## Фаза 7`) — **Перенесено з «Поза фазами» 2026-09-12.** **Запропоновано користувачем 2026-09-11,
  оцінено advisor'ом, дизайн ще не почато.**
...
- Примітка: T-226(б) (джерело GeoIP-даних) вирішено й закрито 2026-09-12 — TASKS-DONE.md.
  T-226(а) (UI-гап: назва країни при пошуку) лишається відкритим точковим фіксом.
...
**Свідомо остання фаза кросплатформного розширення — не "може колись", а планова ціль,
винесена в кінець** (SPEC.md §"Фазований план", Фаза 6; Фаза 7 нижче йде після неї, але не є
кросплатформною роботою). Середовище розробки суто Windows, macOS build/test-доступу немає;
...
- [ ] T-135 — Постійний редакційний процес рев'ю державного та науково-освітнього списків — триває й після релізу, не одноразова задача (Фазований план Фаза 5, Відкриті питання п.8). **Виправлено (2026-09-13):** попереднє перехресне посилання тут (на T-227) стверджувало, що `GovernmentTopN`/`SciEdu` не мають розповсюдженого датасету — це виявилося застарілим/помилковим: `gov-ua`/`gov-us`/`gov-pl`/`gov-gb`/`edu` вже мають реальні закомміченні файли (`data/topn/`) і прокинуті через `AVAILABLE_TOPN_LISTS`. Цей пункт лишається відкритим лише як **постійний** редакційний процес рев'ю вже існуючих списків, не як задача "породити перший датасет".
...
- [ ] T-158 — **Онбординг-майстер: виявлення відсутнього сертифіката/DoH-налаштування браузера при
  ...
  **Звужено 2026-09-12 (аудит бекложу, той самий узор "звужено, не закрито", що й T-51/T-56):**
  (a)'s Chrome/системний-трастстор шлях уже реалізований — `dnsqb-tray`'s `onboarding`-модуль
  (T-188/T-189/T-193, Батч 3.13): `should_offer_onboarding` виявляє непідтверджену довіру,
  `run_setup_wizard` пропонує встановити, на «Так» викликає `ensure_installed`. **Це НЕ закриває
  (a) повністю** — сам текст (a) явно вимагає "для Chrome і для Firefox окремо", а вже готовий
  wizard охоплює лише `CurrentUser\Root` (Chrome/Edge-сумісний шлях); Firefox-половина (власна
  NSS-база, T-132) лишається повністю не автоматизованою й не спроєктованою. Лишається відкритим
  у (a): лише Firefox/NSS-DB-виявлення-і-пропозиція; лишається відкритим у (b): без змін.
...
- [ ] T-166 — **Оновити pinned versions GitHub Actions** (виникло 2026-09-01
  ...
  warning, зникне сам коли runner прибере Node 20. **Скоуп уточнено 2026-09-12 (аудит бекложу):**
  задача 2026-09-01 називала лише `ci.yml`+`codeql.yml`, але `claude-md-size.yml`/`release.yml`/
  `topn-curate.yml` (усі заведені пізніше) теж на `@v4` — треба підняти в усіх п'яти:
  ...
  - `.github/workflows/claude-md-size.yml`, `.github/workflows/release.yml`,
    `.github/workflows/topn-curate.yml` — перевірити кожен на `actions/checkout@v4`/інші
    pinned-версії, підняти разом з рештою.
...
- [ ] T-231 — **Заплановані CI-тести "канарки" на зовнішні джерела даних** (запит користувача
  2026-09-13, мід-турн під час Батчу 7.3): окремий, не-на-кожен-push набір, що запускається раз
  на кілька днів (`workflow_dispatch`/`schedule`-крон), перевіряє, чи зовнішні джерела, від яких
  залежить проєкт (DNS-резолвери-voter'и, блок-лист-бандли, top-N/GeoIP-фіди), не змінили мовчки
  формат відповіді й досі оновлюються/працюють — і сповіщає **лише після кількох поспіль невдач**,
  не з першої (щоб уникнути шуму хибнопозитивних від одноразового мережевого збою).
  **Ключове дизайн-питання, яке вирішує все інше — де живе стан дебаунсу між запусками.**
  GitHub Actions scheduled workflow сам по собі без памʼяті між ранами: варіанти — закомічений
  файл стану в репо (простий, але шумить в git-історії), workflow-артефакт із retention (без
  git-шуму, але має термін життя), запит історії ранів через GitHub API (без окремого стану
  взагалі — лічити останні N ранів того ж workflow), або issue, який джоба сама оновлює
  (людино-читабельний журнал, природне місце для сповіщення). Обрати один, а не список.
  **Набір проб** здебільшого вже існує розрізнено, лишається зібрати в один розклад:
  `cargo run --example sinkhole_probe` (T-175, живий переприцінювач sinkhole-префіксів, вже
  "запускати перед релізом" за CLAUDE.md) — прямий кандидат; 8 URL `BLOCKLIST_SOURCES`
  (`blocklist_download.rs`) — формат+досяжність; `topn_download`'s стабільний-URL-контракт +
  sha256-сайдкари; `geoip_download`'s два джерела; `reachability::MARKERS`; DoH-ендпоінти
  `BUILTIN_PRESETS`. **Мережева ввічливість тут — найгостріший тест правила** (CLAUDE.md
  "Recurring patterns"): набір проб на розкладі, що б'є кожного voter'а й кожен фід, — це якраз
...
- [ ] T-233 — **Немає жодного runtime-захисту від скомпрометованого/зіпсованого публічного
  ...
  - **Звужено 2026-09-17 (той самий "звужено, не закрито" узор, що T-51/T-56/T-221) — дві з
    ...
    - **Лишається відкритим у T-233:**
      - **Ін'єкція жмені легітимних доменів у великий список** — структурна межа ratio, canary,
        delta й тепер PSL-фільтра теж (PSL ловить лише цілий public suffix одним рядком, не
        жменю звичайних на вигляд доменів). Не задача з номером, а стаття
        KNOWN-LIMITATIONS.md.
      - **Ресурсне виснаження (клас 3)** — пом'якшено T-232, не усунено.
