...
Обидва — не задачі з номером, а відкриті рішення перед повноцінним стартом Фази 2 (живий прохід
дешево зробити зараз, cert уже довірений; метрики — переміряти чи додати провайдера, чи прийняти
результат як є). Наступна активна фаза — Фаза 2, ще не почата.
...
**Реальний блокер, вирішений з користувачем 2026-08-29**: T-68/T-70's macOS-половини, T-71
(порт на macOS), T-83 (CI matrix +macOS) усі потребують реального macOS build/test-доступу
(Keychain API, `security` CLI) — це середовище суто Windows. **Рішення користувача: відкласти
все macOS-залежне повністю**, не research-only-прохід, не припущення про доступність. Порядок
нижче побудований навколо того, що реально збирається й тестується в цьому середовищі зараз.
...
**Відкладено повністю, не в цьому плані** (рішення користувача вище): T-68/T-70's
macOS-половини, T-71, T-83. Повернутись, коли з'явиться реальний macOS-доступ.

Каденс не змінюється: одна задача — один комміт, advisor-гейт до і після, пауза й звіт між
задачами (project memory: `feedback_task_by_task_delegation_cadence.md`). Перша задача в роботі:
T-74.
...
- [ ] T-69 — Механізм ротації сертифіката: перевипуск, видалення старого з trust store без
  накопичення (2) — **T-49's `uninstall()` (CN-based, вичерпний) уже прибирає застарілі записи
  після перевипуску, коли викликаний**; ця задача — про те, щоб ротація сама викликала його
  автоматично, не про новий примітив видалення.
