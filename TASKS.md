...
проведено)** → **3.10: T-177 → T-176 (UX нетехнічного користувача — рішення користувача 2026-09-06,
входить у v0.3.0)** → **3.11: T-173 (реліз `v0.3.0`)**. Після 3.11 Фаза 3 закрита **повністю**
...
- **Батч 3.10 — UX нетехнічного користувача (T-177 → T-176).** Спільна тема = застосунок має
  ...
  **Kickoff plan+advisor — один на батч, на T-176** (T-177 — дрібна мех. зміна, окремого gate не
  ...
  1. **T-177 — іконка + `VERSIONINFO` у три `.exe`.** `build.rs` у `dnsqb-{service,tray,watcher}`
     з `winresource` (або `embed-resource` — обрати за чистотою `#![forbid(unsafe_code)]` +
     ліцензією): `VERSIONINFO` (`FileDescription`/`ProductName`/`CompanyName`/`OriginalFilename`,
     версія з `CARGO_PKG_VERSION` — на цей момент ще `0.2.0`, Батч 3.11 підніме) + `IDI_ICON` з
     нового `assets/icon/app.ico` (згенерувати з `assets/gen-icon.py` — новий вихідний формат,
     не hand-edit PNG→ICO). Вет нової build-залежності → `SECURITY.md` + `deny.toml`. Перевірити,
     що `makeappx`/`pack-msix.ps1` не конфліктує зі вбудованим у `.exe` ресурсом. Свій коміт;
     closing-advisor опційно. **Готово:** Диспетчер задач показує іконку + опис для всіх трьох
     процесів; `cargo build --workspace` + CI зелені; MSIX-пак усе ще збирається.
  2. **T-176 — UX-ревізія `/admin/ui` + помітність застосунку.** Дизайн-фаза (skill `design`):
     багатоартбордний canvas — (a) базовий вигляд `/admin/ui` (увімк/вимк фільтрації, стан
     «захищено / не захищено», перемикачі malware/ads/adult), (b) розширений вигляд за
     прогресивним розкриттям (акордеон/підменю «Розширені»: провайдери поштучно, `timeout_mode`,
     GeoIP, cache TTL, `[limits]`, персистентність, cert-дії), (c) трей-tooltip-стани й Start-плитка,
     (d) онбординг першого запуску → крок налаштування браузерного DoH. Ітерація на canvas →
     затвердження користувачем. Далі: оновити `UI-SPEC.md` §2 (інвентар) + §3 (поля, позначити
     basic/advanced) + `diagrams/ui-*` за ground-truth-ритуалом + `mockups/`. Імплементація —
     переписати `include_str!` HTML/CSS/JS `/admin/ui` (без бандлера, як зараз), CSP без
     `unsafe-inline` зберегти; 4 категорії тестів на нову структуру маршрутів/рендер, як T-59
     precedent. **Kickoff plan+advisor до дизайну, closing-advisor після імплементації.**
     **Готово:** `/admin/ui` має basic/advanced розкриття, `UI-SPEC.md`/`diagrams` синхронні,
     тести зелені, canvas затверджено.

- **Батч 3.11 — фінальний реліз Фази 3 (T-173).** Бамп `0.2.0` → `0.3.0`, оновлення "Фаза 3
  ...
  публікує людина. **v0.3.0 тепер несе й T-176/T-177** (UX + іконки) — release-note це відображає.
  Свій короткий kickoff + **обов'язковий closing-advisor перед пушем тега** (тег — точка, яку
  важко відкотити). CI-green на бамп-коміті; після пушу тега — `gh run watch` на `release.yml`.
  **Відкат, якщо `release.yml` падає _після_ приземлення тега** (напр. `repro`-недетермінізм —
  `main.rs` змінювався в T-169 **і** T-177 додає `build.rs`, а `repro` порівнює дві чисті
...
(closing-advisor проведено). **Наступне — Батч 3.10 (T-177 → T-176, UX нетехнічного користувача,
входить у v0.3.0) → Батч 3.11 (T-173 — реліз `v0.3.0`), і Фаза 3 закрита повністю. План — вище,
...
локальний DoH»). **Батч 3.9 закрито 2026-09-06** (closing-advisor проведено). Наступне —
**Батч 3.10** (T-177 → T-176, UX нетехнічного користувача, у v0.3.0) → **Батч 3.11** (T-173 бамп
...
- [x] T-169 — (Батч Ф3, після 3.8) **Імплементація запобіжника resource-exhaustion — зроблено
  ...
  TASKS-DONE.md.
- [ ] T-173 — (Батч 3.11) **Фінальне закриття Фази 3 + реліз `v0.3.0`.** **Бамп `0.2.0` → `0.3.0`
  ...
  T-170/T-171/T-172, UX-батч T-176/T-177 у релізі». Коміт бампу+доків → CI-green. Потім
  `git tag v0.3.0` + `git push origin
  ...
  **чернетка** GitHub-релізу (3 `.exe` + `SHA256SUMS` + `.msix` + `.cer`), публікує людина. `gh run
  ...
  змінився в T-169 **і** T-177 додав `build.rs` → `repro`-недетермінізм можливий):
  `git push origin :refs/tags/v0.3.0`, полагодити, перетегувати — див. «Батч 3.11» вище.
  **Готово:** `v0.3.0` тег на `origin`,
