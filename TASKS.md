...
- [ ] T-218 — Опційний, default-OFF шар публічних блок-лист-бандлів поряд із кворумом (мета —
  ...
  **8 звірених джерел** (2026-09-13, `data/blocklists/CANDIDATES.md` §3): HaGeZi Multi PRO/TIF/
  NRD-DGA/DynDNS/Badware Hoster/Most Abused TLDs (GPL-3.0) + AdGuard DNS filter (GPL-3.0) +
  1Hosts Lite (MPL-2.0) — усі рантайм-фетч без редистрибуції курованого артефакту (окрема рамка
  за T-106's CC BY-NC-блокер). OISD/Steven Black hosts/CoinBlockerLists/OpenPhish виключені
  (ліцензія/склад/свіжість/ToS); URLhaus виключено як нез'ясоване, не остаточно відхилене.
  Жодне джерело не публікує `.sha256`-сайдкар — власний контроль цілісності. 24-годинний
  опитувальний цикл (мережева ввічливість). **Атомарний своп без вікна недоступності** —
  `RwLock<Arc<T>>`-патерн, той самий що `CacheState`/`OverridesState`/`GeoipState`/
  ...
  **Перший дизайн-артборд готовий:** `mockups/gui-dashboard.html`, Артборд F (2026-09-13) —
  розміщення `#blocklist-bundles-body` після `#rating-filter-body`, master-перемикач + 8-джерельний
  чекліст зі статусом, credits-атрибуція. Малює per-джерело статус, якого сьогодні не несе жоден
  DTO — має на увазі майбутній `AdminStatusResponse`-зріз (аналог `RatingFilterStatusView`,
  `#[serde(default)]` + `ADMIN_DTO_SCHEMA_VERSION`-бамп), не вирішений цим kickoff'ом.
  **Гейт: за прямим запитом користувача для цієї задачі код не починається до user-рев'ю
  артборда F** — на відміну від Артборда E, чий статус "чекає затвердження" історично не
  зупинив реалізацію (T-127/T-128 відвантажені попри це); тут гейт явний, не структурний.
  Точна архітектура updater-модуля (`blocklist_download`/`blocklist_updater`, за зразком
  `topn_download`/`topn_updater`) — не вирішена, наступний план+advisor цикл після design-рев'ю.
