...
4. **Батч 5.4** — решта сайту (T-151, той самий механізм з 5.2) — мультикомітний, один розділ
   `main.js` = один комміт. **Коміт 1/~14 (`#app-body` + hero-хвіст) і коміт 2/~14
   (`#timeout-config-body`) зроблено 2026-09-20** — деталі, advisor-catch'і, живий-баг у
   `renderError()`, безкоштовний `common.helpAriaLabel`-побічний ефект на 4 інших картках —
   TASKS-DONE.md. Решта коммітів: overrides, cache-config, cctld-block, geoip+maxmind, log,
   providers, filter-controls, browser-setup, danger-zone, rating-filter, blocklist-bundles,
   footer.
