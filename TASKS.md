...
- [ ] T-225 — **Запит користувача 2026-09-12.** Прохання зробити `chrome://settings/security`
  клікабельним посиланням, що саме відкриється, **а не текстом для копіювання вручну**.
  **Технічно неможливо як прохання сформульоване:** Chrome блокує навігацію на `chrome://`-URL з
  `<a href>` звичайної сторінки. **Факт, перевірений 2026-09-12 (греп + `git log -S` + жива
  a11y-перевірка в Chrome):** кнопка «Скопіювати» поруч із `<code>` — те, що задача сама називає
  реалістичною заміною — **вже реалізована й у продакшні**: `index.html:73`
  (`#chromium-settings-copy`) + `index.html:84` (`#firefox-settings-copy`), обидві підключені
  через `wireCopyButton` (`main.js:2153-2175`, clipboard API), зроблено в T-189 (коміт `98ac6c8`,
  задовго до заведення T-225). Живо підтверджено на скретч-інстанції порту 9443 (та сама сесія,
  де перевіряли T-224): розгорнута інструкція показує URL з кнопкою «Копіювати» поруч. **Не
  вирішено агентом:** сам текст запиту протиставляє "клікабельне посилання" саме копіюванню
  вручну ("а не текстом для копіювання вручну") — тобто наявна копі-кнопка могла й НЕ бути тим,
  чого просив користувач, а не просто пропущеною через згорнуту картку. Рішення, чи наявного
  вистачає, чи потрібне щось понад копіювання (саме посилання технічно неможливе) — за
  користувачем, не одноосібний вибір агента.
