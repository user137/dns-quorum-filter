...
конвеєр на обох знайдених точках. Наступна задача в роботі: **T-79** (`DecisionSource::Geoip`'s
`geoip_country`-поле; сам `decision_source = GEOIP` T-76 вже зробив реально досяжним) — план вище
ставить T-82 (закриваючі тести) одразу після, потім T-77/T-78/T-80/T-81.
