...
проведено)** → **3.10: ~~T-177~~ → T-176 (UX нетехнічного користувача — рішення користувача
2026-09-06, входить у v0.3.0)** → **3.11: T-173 (реліз `v0.3.0`)**. Після 3.11 Фаза 3 закрита
**повністю**
...
(closing-advisor проведено). **T-177 зроблено 2026-09-06** (іконка + `VERSIONINFO` у три `.exe`
через `embed-resource`; TASKS-DONE.md). **Наступне — T-176 (UX-ревізія `/admin/ui`, дизайн через
skill `design`) → Батч 3.11 (T-173 — реліз `v0.3.0`), і Фаза 3 закрита повністю. План — вище,
...
локальний DoH»). **Батч 3.9 закрито 2026-09-06** (closing-advisor проведено); **T-177 зроблено
2026-09-06** (Батч 3.10 крок 1 — іконка + `VERSIONINFO` у три `.exe`). Наступне — **T-176**
(UX-ревізія `/admin/ui`) → **Батч 3.11** (T-173 бамп `v0.3.0` + тег + чернетка MSIX-релізу) →
