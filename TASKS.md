...
- [ ] T-226(а) — **Запит користувача 2026-09-12.** Підтверджено: при пошуку GeoIP-країни немає
  підсказки/випадного списку з назвою країни — лише сирий ISO-код, перевірений
  `validate_country_code` (`dispatch.rs:2328`, `config.rs`); сама назва ніде в UI не
  показується. Дрібний, реалізовний UI-гап, ще не зроблений.
  (T-226(б) — те саме прохання щодо GeoIP-джерела — вирішено й закрито, TASKS-DONE.md.)
