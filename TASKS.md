...
- [ ] T-136 — **Перенесено з «Поза фазами» 2026-09-12.** Merge публічних блок-листів (EasyList тощо) — окрема фіча поза quorum-логікою (Явно поза межами MVP). **Перекривається скоупом T-218** (той самий клас — статичний шар блок-листів поряд із кворумом), який уже має значно детальніший аналіз джерел/ліцензій/дизайн-питань (2026-09-11). Тримати окремо лише якщо вирішено НЕ зводити в один kickoff з T-218 — інакше закрити як «superseded by T-218» при старті тієї задачі. Рішення — за користувачем, не агентом.
