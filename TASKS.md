...
- [ ] T-169 — (Батч Ф3, після 3.8) **Імплементація запобіжника resource-exhaustion** (виникло
  ...
  стелі (не черга), окрема менша стеля на одночасні quorum-резолюції «у польоті», конфіг-поле
  (`resolver_config.toml`, §7.1 #8-стиль — Ф3-дефолт), admin-видимий лічильник відмовлених
  з'єднань. Власний plan+advisor kickoff — тут вирішується відкрите питання SPEC.md §1.1 (який
  сигнал відмови; чи мала черга з коротким таймаутом безпечніша за жорсткий reject для
  SERVFAIL-чутливих клієнтів). Тести на 4 категорії. Якщо обереться `tower` — vetting тут.
  TASKS-DONE.md.
