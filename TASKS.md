...
~~3.5: T-146 + T-96~~ (зроблено 2026-09-03) → ~~T-97~~ (зроблено 2026-09-03) → ~~3.6~~ (зроблено 2026-09-04) → ~~3.7: T-100/T-102/T-103~~ (зроблено 2026-09-04) → **T-167** (ревізія документації) + 3.8 (останній).
3.4 могла йти раніше, якщо watchdog-батч застопориться (він незалежний).
...
**Батч 3.7 (T-100/T-102/T-103, release-інженерія) завершено 2026-09-04, CI-only. Наступне —
T-167 (ревізія документації) і Батч 3.8 (T-156/T-70, останній).**
...
- [x] T-103 — (Батч 3.7) CI release-pipeline — **зроблено 2026-09-04**: тег `v*` → джоба `release`
  ...
  3.8. TASKS-DONE.md.
- [ ] T-167 — (Батч Ф3, після 3.7) **Повна ревізія документації для читача-людини.** Кожен `.md`
  прогнати з `advisor` на: чи відповідає структурі community-стандарту, чи не перевантажений, чи
  його реально прочитають. Конкретно: (a) README — прості, «для домогосподарки», інструкції як
  зібрати й запустити проєкт; (b) повний конвеєр фільтра доступною мовою або діаграмою (ймовірно в
  README) — порядок кроків, що кожен фільтрує, ранг важливості; (c) SECURITY.md — усе після
  таблиці залежностей читається як нерозбірливий лог, реорганізувати й викинути зайве; (d) звірити
  достатність build-пояснень. Скрипти білду/підпису можна міняти, якщо задача цього вимагає.
  Власний plan+advisor kickoff.
