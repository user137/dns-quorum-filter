...
- [ ] T-118 — UI додавання/видалення ccTLD-кодів — **за зразком UX рейтинг-фільтра «бульбашки»**
  ...
  обраний чіп показує назву країни поруч із кодом (не лише голий `.ru`), видалення вже доданого
  коду через кнопку `×` на чіпі, «Зберегти» з'являється лише коли вибір розійшовся зі збереженим
  станом (dirty-state, той самий патерн, що `syncSaveBtn`/`pickedMatchesSaved`). Разом з UI-
  попередженням про грубість евристики (аналогічно GeoIP over-blocking notice) — той самий
  запит користувача, обидва UI-аспекти однієї задачі. **Відкрите питання перед кодом:** ccTLD ≠
