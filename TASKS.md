...
підтверджує. T-77 done (TASKS-DONE.md, 2026-08-30, два коміти) — `GET /admin/geoip`/`POST
/admin/geoip/add`/`remove` + `#geoip-body` на `/admin/ui`, перший живий шлях запису
`[geoip] blocked_countries` без ручної правки TOML. Наступна задача в роботі: **T-78** (UI:
індикатор дати останнього оновлення GeoIP-бази), потім T-80/T-81.
