...
- [ ] T-233 — **Немає жодного runtime-захисту від скомпрометованого/зіпсованого публічного
  ...
  - **Звужено 2026-09-17 (той самий "звужено, не закрито" узор, що T-51/T-56/T-221) — дві з
    ...
    - **Advisor-catch, виправлено в тому ж коміті (не окрема задача):** `write_and_hash_blocking`
      ...
      регресію.
    - **Лишається відкритим у T-233 (не в цій частині):**
      - **Дельта-перевірка проти минулого успішного циклу** — `previous` сьогодні живе лише в
        пам'яті `AppState`, тож на першому циклі після кожного рестарту (найважливіший момент)
        порівнювати нема з чим; потребує нового персистентного per-джерело sidecar-файлу.
        **Уточнення користувача 2026-09-17 (важливе для дизайну, не для цієї частини):** діф
        сам по собі може бути істотним, якщо джерело довго не оновлювалось (сайдкар-джерело
        мовчало кілька циклів/тижнів через збій чи вимкнений `[blocklist_bundles]`, потім
        відновилось) — поріг має враховувати **час від останнього успішного циклу**, не лише
        сире значення дельти, інакше легітимне повернення застарілого джерела після паузи
        прочитається як компрометація.
      ...
      - **Ресурсне виснаження (клас 3)** — пом'якшено T-232, не усунено; поза скоупом цієї
        частини.
