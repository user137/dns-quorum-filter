...
6. **Батч 5.6 — зроблено 2026-09-23** — CLI `--help` (T-235) для всіх трьох бінарників.
   Архітектурне питання вирішено **емпіричною перевіркою, не припущенням**: `println!`/
   `eprintln!` у `windows_subsystem="windows"`-білді (1) коректно виводяться, коли процес
   запущений із наявного термінала (успадковані дескриптори); (2) **не падають і не панікують**
   без жодної консолі — перевірено через справжній `ShellExecute`-запуск (точний еквівалент
   подвійного кліку в Провіднику) з `panic::catch_unwind`, результат записано у файл: `exit=0`,
   без паніки. Висновок: **жодного `AttachConsole`/`MessageBoxW`/unsafe-коду не потрібно** —
   `--help`/`-h`/`/?` перевіряється найпершою дією кожного `main()`, до `app_data_dir`/
   `init_logging`/захоплення single-instance-локу (advisor-catch: інакше прапорець непридатний,
   доки застосунок уже запущений). Новий спільний модуль `dnsqb-service::cli_help` (не окреме
   дерево на бінарник, як у Батчу 5.5 — **свідоме, задокументоване відхилення** від того
   прецеденту: CLI-довідка всіх трьох бінарників **є** одним доменом, на відміну від
   трей-діалогів vs. веб-UI), той самий макрос-патерн і `t()`, малий словник (5 ключів,
   `crates/dnsqb-service/cli-help-i18n/*.json`), користувач підтвердив усі 37 локалей одразу
   (`AskUserQuestion`, попри малий розмір ключів). `sys-locale` промотовано в пряму залежність
   і `dnsqb-service` (уже була в `dnsqb-tray` з Батчу 5.5). 2 коміти: інфра+3×`main.rs`+uk/en,
   потім bulk-переклад на 35 локалей — той самий патерн, що Батч 5.5.
