...
- [ ] T-237 — **Заведено 2026-09-19, запит користувача ("так само як я зробив на pakko").**
  Ненав'язливе посилання на донат — README (GitHub) + десь у самому застосунку. **Відкрите
  питання перед кодом, уточнити з користувачем:** платформа — Buy Me a Coffee (як сформульовано
  в запиті) чи Ko-fi (те, що сибл-проєкт `windows-archiver-wrapper`'s власний
  `docs/TASKS_DONE.md`, T-F93, фактично показав — платформу там теж міняли з BMC на Ko-fi
  посеред задачі, "не оригінальний скоуп"); і сам URL/handle — жоден із двох не існує в цьому
  репозиторії, вигадувати не можна. **Розміщення (адаптація Pakko-прецеденту під реальні поверхні
