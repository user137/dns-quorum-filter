...
(3) `DEFAULT_PROVIDER_IDS` = `quad9`+`adguard`, не SPEC §3.4/§3.5-й "лише Security". Батчі 3.9–3.10
...
2026-09-05. Наступне — Батч 3.9 (T-170/T-171/T-172 — закриття Ф1-гейтів) → Батч 3.10 (T-173 —
реліз `v0.3.0`), і Фаза 3 закрита повністю. План — вище, «План фінального закриття Ф3».**
...
Наступне — **Батч 3.9** (T-170 `DEFAULT_PROVIDER_IDS`, T-171 переміряти quorum-coverage, T-172
...
- [x] T-169 — (Батч Ф3, після 3.8) **Імплементація запобіжника resource-exhaustion — зроблено
  ...
  TASKS-DONE.md.
- [ ] T-170 — (Батч 3.9) **`DEFAULT_PROVIDER_IDS` — рішення + зміна.** Дефолтний активний набір
  першого запуску (`upstream.rs:224`, `= ["quad9", "adguard"]`) досі не SPEC §3.4/§3.5-й "лише
  Security" (Quad9 Filtered + Cloudflare Malware). **Kickoff-AskUserQuestion (обов'язково — це
  зміна того, що фільтрується при першому запуску, Три Б, не побічний ефект):** (а) лишити
  `quad9`+`adguard` і закрити пункт «прийнято як є»; (б) перейти на SPEC-й `quad9` +
  `cloudflare-malware`; (в) розширити на 3+ Security-tier presets (`+cleanbrowsing-security` /
  `+dns4eu-protective`). Якщо зміна: `DEFAULT_PROVIDER_IDS` + fresh-install тест (`upstream.rs`
  ~450, перевіряє `len` == `DEFAULT_PROVIDER_IDS.len()` і що кожен `id` — валідний preset) +
  CONFIGURATION.md `[[providers]]`-дефолт + README + SPEC §3.4/§3.5 (позначити вирішеним) +
  **DECISIONS.md** (реверс неявного shipped-дефолту). Малий, один коміт. **Gate = сама
  kickoff-AskUserQuestion** (вибір робить користувач); окремого advisor-проходу на T-170 не
  треба — його покриває батч-closing після T-172. **Готово:** константа = вибір користувача,
  тести/доки узгоджені, DECISIONS.md-запис (якщо змінили) або TASKS-DONE.md-нотатка «прийнято
  `quad9`+`adguard`, чому».
