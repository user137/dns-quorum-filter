...
задачі T-120–T-131 тепер там. Тут лишається ccTLD-блок + i18n + (заведено 2026-09-18) CLI
`--help`.
...
2. **Батч 5.2 — зроблено 2026-09-19** — i18n-інфраструктура сайту. Плаский JSON-словник на
   ...
   **T-236 (той самий день, поза нумерованим 7-батчевим порядком) — зроблено 2026-09-19** —
   запит користувача розширити мовний `<select>` з uk+en до повного списку; реальний перелік із
   36 культур `.resx`-набору сусіднього проєкту `windows-archiver-wrapper` (не вигаданий 36-мовний
   список, який відхилило рішення вище) + `en` = 37 локалей, той самий pilot-набір ключів (вісь
   локалей, не вісь ключів — Батч 5.4 нижче лишається окремою віссю). `ui/i18n/GLOSSARY.md` —
   правила перекладу + виміряна таблиця `Intl.PluralRules`-категорій. Деталі, advisor-catch'і,
   технічний підхід (макро замість 37-разового дублювання) — TASKS-DONE.md.
3. **Батч 5.3 — зроблено 2026-09-19** — ccTLD UI (T-118) + міграція GeoIP `COUNTRY_NAMES`
   (249 рядків) на `Intl.DisplayNames` в тому самому батчі — один механізм на обидві фічі за
   одну правку. Картка `#cctld-block-body` (combobox-UX `#rating-filter-body`'s), armed-попередження
   лише при додаванні коду, typed-code fallback поза каталогом, `CCTLD_CODES = GEOIP_COUNTRY_CODES
   ∪ {uk, eu, su}` (звірка різниці ccTLD/ISO-3166-1, `su`-виняток бо `Intl.DisplayNames` мапить
...
4. **Батч 5.4 — зроблено 2026-09-22** — решта сайту (T-151, той самий механізм з 5.2), 14 коммітів,
   один розділ `main.js` = один комміт (`#app-body`+hero, `#timeout-config-body`, `#overrides-body`,
   `#cache-config-body`, `#cctld-block-body`, `#geoip-*`, `#log-body`, `#providers-body`,
   `#filter-controls-body`, browser-setup, `#danger-zone-body`, `#rating-filter-*`,
   `#blocklist-bundles-body`, footer). ~250 нових ключів × 37 локалей, механізм `data-i18n`/
   `data-i18n-html`/`data-i18n-attr` для статичного `index.html`, обидві рукописні українські
   плюралізації (`pluralUk()`, `blocklistRelativeTime()`) замінено на `tPlural()`, тест плюрал-форм
   узагальнено, назви зон рейтинг-фільтра — через `Intl.DisplayNames`. Деталі, advisor-catch'і і
   живі баги — TASKS-DONE.md (записи "Батч 5.4, коміт N"). **Не перевірено вживу в реальному
   браузері** (розширення Chrome було недоступне всі сесії Батчу) — лише server-side + структурні
   тести; смок перемикання мови на всіх картках лишається відкритим до Батчу REL/наступної сесії.
5. **Батч 5.5 — зроблено 2026-09-22** — i18n трею (T-151, Rust-сторона). Власний план+advisor-цикл
   (2 advisor-раунди: перед кодом — вибір архітектури; після інфра-кроку — 3 catch'і: `--bins` не
   `--lib` для тестового гейту, injected `locale: &str` не глобальний стан, постійні Rust-тести
   keyset-паритету, не лише скриптовий чек). Рішення: **окреме файл-дерево**
   `crates/dnsqb-tray/i18n/*.json` (37 файлів), не спільні з `ui/i18n/*.json` — трей і веб-UI різні
   домени (нативні rfd-діалоги vs. веб-сторінка), той самий принцип що незалежне типування списку
   локалей у `admin_ui.rs`/`dispatch.rs`. Новий `crates/dnsqb-tray/src/i18n.rs` — той самий
   `macro_rules! i18n_dicts!` патерн, `t()`/`t_args()`/`detect_locale()` (калька
   `resolveLocale`/`detectLocale` з `main.js`). `serde_json` промотований у прямі залежності,
   `sys-locale` доданий (той самий крейт, що відхилений для GeoIP-регіону T-227 — там *не* та
   ознака була потрібна, тут *саме* та — детектування мови ОС для tooltip/меню/діалогів). **Число-
   залежні рядки** (3 у `status.rs`, стара "N запит(ів)"-хардкод-калька) переформульовано в
   "підпис: N"-форму — без CLDR-плюрал-крейта, обходить plural-agreement для будь-якої мови.
   `trust_store_outcome_uk` (точковий фікс тієї ж сесії) узагальнено в `trust_store_outcome_text`.
   2 коміти: інфра+main.rs+status.rs (uk/en), потім bulk-переклад на 35 локалей (Python-скрипт,
   структурні asserts + постійні Rust-тести keyset-паритету, той самий MT-tier що решта проєкту).
   Поза скоупом (наступні батчі): CLI `--help` (5.6), `nudge_popup` (5.7).
6. **Батч 5.6** — CLI `--help` (T-235), споживає словник з 5.5. Перше питання власного
   план-циклу: чи достатньо `debug_assertions`-розділення (debug — console-subsystem, довідка без
   жодного FFI) саме собою, перш ніж чіпати `AttachConsole`/`MessageBoxW` (обидва `unsafe fn` —
   конфлікт із `forbid(unsafe_code)`, вирішити safe-wrapper-крейтом чи вузьким задокументованим
   винятком, не мовчки).
7. **Батч 5.7** — `nudge_popup` живий текст-рендеринг (T-151, найризикованіший, свідомо
   останній) — заміна pre-baked PNG на `tao`/`softbuffer` text-layout, кирилиця+латиниця лише
   (не CJK/RTL цього проходу), джерело шрифту (bundled `.ttf` vs. системний Segoe UI) — рішення
   на старті батчу. Власний план+advisor-цикл, найдетальніший з усіх.

Повний текст плану Батчу 5.4 (порядок коммітів, наскрізні рішення — спільні ключі,
static-vs-dynamic HTML, плюрал-ключі) узгоджено в plan-mode 2026-09-20; **сесійний план-файл
попередньої домовленості (2026-09-18) не пережив між-сесійний розрив, тому не покладатись на
...
- [ ] T-151 — Інтернаціоналізація UI: рядки веб-UI (`/admin/ui`) та `dnsqb-tray` (меню/tooltip) винесені у файли перекладу замість хардкоду в `admin_ui.rs`/`main.js`/Rust-рядках трея, підтримка щонайменше української й англійської, вибір мови — автовизначення з ОС + ручний перемикач в UI; SPEC.md/UI-SPEC.md наразі не називають жодної мовної вимоги — новий, не раніше зафіксований скоуп. **Батч 5.2 (i18n-інфраструктура + pilot-переклад FIELD_HELP/HERO_PRESENTATION) зроблено 2026-09-19; T-236 (той самий день) розширив мовний `<select>` з uk+en до 37 локалей для того самого pilot-набору ключів** — решта сайту (Батч 5.4 зроблено 2026-09-22), трей (**Батч 5.5 зроблено 2026-09-22**, 37 локалей, окреме файл-дерево — деталі вище), `nudge_popup`
  (Батч 5.7) ще попереду
- [ ] T-240 — **Заведено 2026-09-22, знайдено під час T-238 (смоук-тест v0.5.0, живий
  `chrome-devtools`-прохід усіх 37 локалей, не node-стаб).** Батч 5.4 регресія: форма «Додати
  власний DoH-провайдер» (`main.js`, `customProviderForm()`) не перекладається на живому
  `setLocale()` — плейсхолдери, опції категорії/сигнатури блокування й кнопка «Додати» застигають
  у мові першого рендеру картки «Провайдери» й не оновлюються на жодному наступному перемиканні
  мови. Причина — `customFormNode` кешує весь DOM-вузол (`if (customFormNode) return
  customFormNode;`), навмисно, щоб toggle/add/remove іншого провайдера не стирав недописаний ввід
  користувача (T-47) — але той самий кеш блокує й ретрансляцію. Самі словники (перевірено на
  `ar.json`) мають правильний переклад цих ключів — проблема суто в кешуванні DOM, не в перекладі.
  Контрастний приклад того самого файлу: `buildLogFilterRow()` перебудовує весь вміст щоразу (і
  тому коректно перекладається); `geoipCountryDatalist` кешує вузол, але явно перебудовує
  `<option>`и через `populateGeoipCountryDatalist()` на кожен виклик. Мінімальний фікс — окрема
...
- [ ] T-235 — **CLI `--help` для трьох бінарників, заведено 2026-09-18 (запит користувача,
  ...
  немає в жодному з трьох бінарників — не задача перекладу наявного, задача реалізації з нуля.
  **Мінімум — `dnsqb-watcher`** (запит користувача явно): це єдина точка входу для старту всієї
  групи процесів (`ensure_sibling_running(Tray)` → `ensure_sibling_running(Service)` — CLAUDE.md).
  `dnsqb-service`/`dnsqb-tray` — розширення того самого патерну, якщо є сенс, не обов'язково в
  першому проході.
  **Архітектурне ускладнення, яке треба розв'язати перед кодом, не в процесі:** усі три бінарники
  збираються з `windows_subsystem = "windows"` під `not(debug_assertions)` (T-181/watcher's
  `#[tokio::main]`-цитата в CLAUDE.md: "console-subsystem build made the Start-menu tile open a
  terminal whose close killed the group") — реліз-білд **не має консолі взагалі**, тож
  `println!("--help text")` нікуди не виведеться в типовому запуску подвійним кліком. Потребує
  або (а) `AttachConsole`/`AllocConsole` win32-виклику, коли процес фактично запущений із
  вже-наявного термінала (виявити як?), або (б) показ довідки через `MessageBoxW`, коли консолі
  нема — обидва варіанти нові для проєкту, жодного прецеденту в `RUST-GOTCHAS.md`. Мова довідки —
  та сама i18n-система з T-151, не окрема захардкожена копія.
...
## Батч REL — Реліз-підготовка (T-237/238/239, після Фази 5)

**Заведено 2026-09-19, запит користувача — виокремлено з загального бэклогу в свій батч
("щоб не загубити"), той самий прецедент, що Батч 4.7 для позапланових смоук-тест-знахідок.
Не нумерується як частина Фази 5 (`5.x`) — ці три задачі не про ccTLD/i18n/CLI, а про
пост-Фаза-5 реліз-цикл; випадковий номер фази собі не привласнюють.**

**Послідовність, уточнено з користувачем (`AskUserQuestion`, 2026-09-19): спершу довести Фазу 5
до кінця (Батчі 5.4-5.7), і лише тоді братись за ці три** — не перемикатись на них зараз.
Порядок усередині батчу, коли настане час: T-237 (найменша, незалежна) → T-238 (смоук, ловить
регресії перед тим, як щось публікувати) → T-239 (публікація — залежить від чистого T-238-проходу
й окремого kickoff-раунду, описаного в її власному пункті).

- [ ] T-237 — **Заведено 2026-09-19, запит користувача ("так само як я зробив на pakko").**
  Ненав'язливе посилання на донат — README (GitHub) + десь у самому застосунку. **Платформа
  уточнена з користувачем (`AskUserQuestion`, 2026-09-19): Ko-fi, не Buy Me a Coffee** —
  свідомо той самий вибір, що на `windows-archiver-wrapper` (T-F93, який теж мінявся з BMC на
  Ko-fi посеред задачі) — консистентність між проєктами користувача, не оригінальне
  формулювання запиту. **Досі відкрите:** сам URL/handle (Ko-fi-акаунт для цього застосунку,
  ще не існує ніде в репозиторії — уточнити з користувачем перед кодом, не вигадувати).
  **Розміщення (адаптація Pakko-прецеденту під реальні поверхні
  цього проєкту, не 1:1 копія):** Pakko — WinUI3, `HyperlinkButton` у тому самому рядку, що
  GitHub/Privacy Policy лінки в About-діалозі, свідомо БЕЗ підвищеної візуальної ваги
  (`DECISIONS.md` T-F93: donate-лінк вагоміший за trust-лінки читався б як нав'язливість для
  security/довіра-чутливої аудиторії — той самий аргумент застосовний і тут, DNS-фільтр — так
  само security-інструмент). Тут немає WinUI３/`HyperlinkButton` — `dnsqb-tray`'s "Про програму"
  (`show_about_dialog`, `crates/dnsqb-tray/src/main.rs`) — це `rfd::MessageDialog`, plain-text,
  без клікабельних елементів (URL там і зараз — рядок тексту, не кнопка); реальна клікабельна
  поверхня — `#credits` footer на `/admin/ui` (`index.html`, вже має реальні `<a href>` на
  Apache License/GeoIP-атрибуції) — природний кандидат на клікабельний donate-лінк тим самим
  стилем/вагою, що вже наявні там лінки. Немає окремого "About"-екрана в web UI (UI-SPEC.md §3.7:
  "окремого екрана 'Про застосунок' немає") — перевірити разом, чи це привід створити такий
  екран, чи достатньо footer + tray-текст.
- [ ] T-238 — **Заведено 2026-09-19, запит користувача.** Повний функціональний смоук-тест усього
  застосунку через MCP-автоматизацію (Windows MCP/AutoIt/chrome-devtools), підготовка до
  наступного релізу — той самий формат, що вже одного разу зроблено для v0.4.0:
  `plans/msix-smoke-test-v0.4.0.md` (сесійний файл у репо, НЕ закомічений — 23 пронумерованих
  сценарії по категоріях A–G, `[ОБОВ'ЯЗКОВИЙ]`/`[РОЗШИРЕНИЙ]`-маркування, уже включає повний
  Фаза-0-очищення + Фаза-1-інстал + функціональний прохід; один реальний баг (T-221, персональна
  зона не переживає рестарт) і кілька advisor-catch'ів на структуру самого сценарію деградації
  кворуму вже задокументовані там). **Наступний прохід — не переписувати з нуля, а розширити
  цей чекліст** на все, що приземлилось у Фазі 5/7 і не існувало на момент v0.4.0: ccTLD-блок
  (`#cctld-block-body`), блок-лист-бандли (`#blocklist-bundles-body`), i18n (37-локальний
  перемикач, RTL), і що встигне доробитись до CLI `--help`/Батчів 5.4-5.7. **Найкраще
  розміщення в часі — безпосередньо перед наступним version tag**, не посеред Фази 5 (той самий
  принцип, що вже описаний у README/CLAUDE.md для release-gate роботи) — не почато цим запитом,
  зафіксовано в беклозі до слушного моменту. **Частково пройдено 2026-09-22** (v0.7.0, установлений
  MSIX-пакет, `plans/smoke-test-v0.5.0.md` — сесійний, не закомічений): мережева ввічливість,
  ADULT_CONTENT toggle, MaxMind фейкові креди, наскрізний i18n/a11y-прохід — усе пройдено, 2 реальні
  баги знайдено й виправлено (кеш не інвалідувався при зміні провайдерів/категорій — `bcc8275`;
  8 полів форм без `id`/`name`/`aria-label` — `bd77dae`). **4 пункти навмисно відкладено до
  наступного проходу T-238** (кожен упирається в дію самого користувача, не в мою автоматизацію):
  клік «Так» на трей-діалозі паузи (5 підходів автоматизації вичерпано, правило 3 спроб); реальна
  фільтрація через справжній браузер (зміна secure-DNS налаштувань — поза межами дозволеної
  браузерної автоматизації); деградація кворуму/офлайн fast-path через `hosts` (потребує UAC +
  явного «так», відкат прописати ДО зміни); «Повністю видалити» (найдеструктивніше, окреме
  підтвердження, єдиний код-шлях без автотестів).
- [ ] T-239 — **Заведено 2026-09-19, запит користувача.** Публікація в Microsoft Store. **Не
  ...
  Pakko — публікаційний досвід користувача там уже є, за прямим посиланням користувача), чи
  зарезервовано ім'я застосунку в Store, безкоштовна модель чи платна (за наявним ADR/SPEC.md
  проєкт наразі не декларує жодної моделі монетизації), вимоги вікового рейтингу (IARC), URL
  privacy policy (SPEC.md/README поки не мають опублікованої політики приватності — GitHub-репо
  ≠ política), publisher identity/CN у MSIX-маніфесті (`packaging/pack-msix.ps1`'s підписання —
  Store-сабміт може вимагати CN, що збігається з Partner Center-акаунтом, не довільний
  self-signed/ефемерний, який зараз використовує `release.yml`), скріншоти/опис/категорія
  лістингу. **Публікація — публічна й важко зворотна дія** (як і будь-яка публікація в
  CLAUDE.md's переліку "Explicit permission required") — сам сабміт вимагає явного підтвердження
  в момент дії, не успадкованого від "додав задачу в беклог".

## Поза фазами / бэклог

- [ ] T-234 — **Заведено 2026-09-18, винесено з T-233 (Фаза 7).** Жоден із чотирьох
  ingestion-гейтів `blocklist_download`/`blocklist_updater` (parse-success ratio, canary-домени,
  entry-count delta, ICANN-PSL фільтр) не ловить ін'єкцію **жмені** легітимних доменів у великий
  список — T-233's власний початковий threat model, лишений відкритим у всіх трьох
  план+advisor-циклах T-233. Ratio не зачеплено (ін'єктовані рядки — валідні домени); canary
  ловить лише 12 конкретних скомпільованих доменів; delta не зачеплено на будь-якому реалістичному
  обсязі (1000 доменів у `hagezi-nrd`'s 3.18M = 0.03% зсуву); PSL ловить лише цілий public suffix
  одним рядком, не жменю звичайних на вигляд доменів. Закриття потребує або значно більшого,
  безперервно підтримуваного canary-набору (чи еквівалентного reputation-сигналу), або зовсім
  іншого класу перевірки — жодного плану ще нема, тому в беклозі, не в наступній фазі. Некритична
  (жоден із чотирьох гейтів не деградує до гіршого за без-фільтрації стану — Три Б user-safety
  leg — це DoS через over-blocking, не bypass); повний технічний контекст — KNOWN-LIMITATIONS.md.
