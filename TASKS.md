...
- [ ] T-53 — Явний allowlist Tauri-команд + DTO замість прямої експозиції внутрішніх структур бекенду (8) — T-52 зроблено (`dnsqb-ui`, 3 команди: `get_status`/`set_providers`/`set_timeout_mode`, ще без формального рев'ю поверхні)
...
- [ ] T-139 — UI: **звужено після T-52** — лічильники (оброблено/заблоковано) вже показуються в `dnsqb-ui`'s дашборді (`admin::compute_stats`, живцем із логу, без нового сховища); залишок цієї задачі — лише відсоток заблокованого, поки не відображений (6, 8)
