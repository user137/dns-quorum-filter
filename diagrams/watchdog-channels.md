...
- Чи ділять напрями watcher→service і service→watcher **одну** ~5 s фазу опитування, чи мають
  ...
  ньому.
- Точна форма «тіло-маркер OK» для `/health` (яке саме тіло/поле означає «здоровий») — §7.1 #4
  задає лише «глибша за ‹процес існує›, без upstream»; конкретний DTO — T-86 у Батчі 3.1.
