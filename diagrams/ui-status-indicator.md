...
## Основний індикатор (одна умова показується одночасно; порядок пріоритету — DECISIONS.md 2026-09-02)
...
| 3 | Активних voters = 0 (усі провайдери вимкнені) | §3.3, §8.1 | "Фільтрацію вимкнено — жодного активного провайдера" — явний pass-through, не помилка |
| 4 | Деградація: N з M апстрімів не відповіли, режим `degraded` | §3.3 | "Деградовано: фільтрація неповна (N/M апстрімів не відповідають)" |
| 5 | Жодна з умов 1-4 не виконана | — | "Фільтрація активна" |

Порядок 1→2→3→4: спочатку найгірше (браузер узагалі не фільтрується), тоді інфраструктурний
збій сервісу (недоступний незалежно від конфігурації), тоді явний конфіг-вибір користувача,
тоді часткова деградація. **watchdog вище за 0-voters** — DECISIONS.md 2026-09-02.
...
flowchart TD
    ...
    Check2 -->|Так| Check3{"Активних voters > 0?"}
    Check3 -->|Ні| S3["Стан: фільтрацію вимкнено —<br/>0 активних провайдерів (§3.3, §8.1)"]
...
- **Умова 3 (0 voters)** — з T-149 (`NoActiveProvider`); T-72/T-73 змінило вхід на
  `active_providers: ProviderStatusView[]` (порожній масив = стан).
- **Частина умови 4 (деградація)** — `AdminStats.degraded_window`/`degraded_events`, суфікс до
...
Повна версія індикатора (умови 1–4 як окремі конкуруючі стани з єдиним індикатором, не суфіксом)
