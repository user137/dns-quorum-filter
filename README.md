...
людина. **Проміжний тег `v0.7.0` (2026-09-22) — Фаза 5 ще відкрита:** ccTLD-блок (T-115..118) і
повністю локалізований `/admin/ui` (37 локалей, ~330 ключів, T-151 Батчі 5.2–5.4) вже всередині; трей,
CLI `--help` і `nudge_popup` (Батчі 5.5–5.7) поки лише українською, переклади 35 локалей — машинні.
`v0.5.0` бамп-коміт лишився непротегованим; `v0.6.0` тегувався разом з T-241 (v0.5.0 смоук-тест,
той самий день) — виправлено UTF-8→ANSI «мозаїку» в назвах трьох процесів (Task Manager) і
ввімкнено сучасний `TaskDialogIndirect`-стиль (замість класичного `MessageBoxW`) для всіх
нативних діалогів трея; `v0.7.0` додає версію застосунку в заголовок `/admin/ui` (той самий
принцип, що й трей-діалоги — скріншот сам себе ідентифікує). Поставлено з Фази 3: watchdog-процес зі взаємним heartbeat (`dnsqb-watcher`), опційне
...
2. Якщо не через `-Install`: `Add-AppxPackage dns-quorum-filter.msix -ForceTargetApplicationShutdown`
   (T-223 — без цього прапорця повторний запуск на вже встановленій і запущеній версії провалюється
   з `0x80073D02` "needs to be closed", поки хтось вручну не вийде із застосунку через трей).
...
**Оновлення до новішої версії:** та сама команда з кроку 1 (`.\Trust-TestCert.ps1 -Install`) на
новий `.msix` — оновлює на місці, без ручного виходу з трею (T-223).
