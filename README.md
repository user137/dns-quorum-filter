...
(2026-09-06, дискримінантний негативний контроль — див.
"[Перевірка: браузер → локальний DoH](#перевірка-браузер--локальний-doh)" нижче). Живий,
...
5. Перевірити, що сервіс живий — відкрити `https://127.0.0.1:<port>/admin/ui` **напряму** в
   браузері (не через щойно вказаний кастомний DoH). Що браузер справді резолвить домени саме
   через цю службу — перевіряється окремо, див.
   "[Перевірка: браузер → локальний DoH](#перевірка-браузер--локальний-doh)" нижче.
